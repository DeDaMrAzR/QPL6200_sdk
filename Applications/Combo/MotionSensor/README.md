...
Qorvo&reg; QPG6200 Combo Motion Sensor Reference Application shows how to create a motion sensor to remotely report an occupancy and illuminance with either Thread connectivity, using Matter&trade; or Zigbee 3.0 protocols in two separate applications within one chip. It is using Bluetooth&reg; LE to perform Matter&trade; commissioning. This example can be used as reference for creating your own Matter&trade; or Zigbee 3.0 motion sensor.

Buttons are used as input to trigger device state changes, LEDs are used as output to visualize the device states.

Features of this application are:
1. Simulate the PIR sensor and light sensor by using buttons.
...
| GPIO0| Input (Pull-up)| SW9| Multiple functionalities: <br />- Trigger OTA - (released 0-3s) <br />- Factory Reset (released in 6-9s) <br /> - Application Switch (release after >12s)|
| GPIO29| Input (Pull-up)|SW3| Motion PIR sensor state input: <br />- Press and hold: PIR occupancy detected <br />- Button released: PIR occupancy lost|
| GPIO3| Input (Pull-up)| SW2| Toggle the light sensor illuminance value between `1` and `0xfffe`|
...
| GPIO1| Output| LD8| Motion sensor sensed occupancy state <br />- **On**: Occupied.<br />- **Off**: Unoccupied.|
...
|:----------:|:----------:|:----------:|:---------|
| RESETN| Input| SW1| Reset Button|
| GPI29| Input (Pull-up)| SW3| Toggle occupancy state (occupied/unoccupied).<br />If the state is changed from unoccupied to occupied, device will send OnWithTimedOff command and report illuminance measured value. |
| GPIO3| Input (Pull-up)| SW2| Toggle illuminance measured value (MinMeasuredValue/MaxMeasuredValue). |
| GPIO21| Input (Pull-up)| SW4| Toggle destination of commands (group/bind address). |
| GPIO0| Input (Pull-up)| SW9| Multiple functionalities: <br />- Start Zigbee Touchlink Commissioning (release in 2-4s)<br /> - Start/Stop Touchlink commissioning with reset-to-factory-new request (release in 4-6s)<br /> - Start Zigbee Finding and Binding (6s-8s) <br /> - Perform a factory reset (10s-12s) <br /> - Application switch (>12s)|
| GPIO11| Output | D6| Status LED (Green) |

## Firmware architecture

The Combo Motion Sensor application is composed of:

1. [Application Bootloader](../../Bootloader/README.md).
2. [Matter Motion Sensor application](../../Matter/MotionSensor/README.md) (primary application).
3. [Zigbee Motion Sensor application](../../Zigbee/MotionSensor/README.md) (secondary application).
...
[P][-] ============================
[P][-] Qorvo Motion-Sensor-app Launching
[P][-] ============================
```
...
how to build and program the Combo Motion Sensor example application.

## Testing the example

The Combo Motion Sensor application will not start any advertisement or commissioning at start-up if it is was not commissioned before.
