...
Qorvo&reg; QPG6200 Matter Motion Sensor example shows how to create a motion sensor to remotely report an occupancy and illuminance with Thread
...
creating your own Matter motion sensor.
...
See [Building and flashing the example applications](../../../sphinx/docs/QUICKSTART.rst#working-with-examples) section to get instructions how to build and program the Matter motion sensor example application.

### Testing the example

The Matter motion sensor example will start Bluetooth LE advertising automatically at start-up if it is was not commissioned before
...
Make sure both Light and MotionSensor device are commissioned to the same network.

Assuming that the NodeID of the devices are set to the followings
- Matter Light device: NodeID = `#LightNodeId#`
- Matter MotionSensor device: NodeID = `#MotionSensorNodeId#`

1. Write binding info to the MotionSensor app
```
./chip-tool.elf binding write binding '[{"fabricIndex": 1, "node": #LightNodeId#, "endpoint": 1, "cluster": 6}]' #MotionSensorNodeId# 1
```

2. Confirm that the binding info is written successfully
```
./chip-tool.elf binding read binding #MotionSensorNodeId# 1
```
Output:
```
[1666874049.111186][11048:11053] CHIP:TOO: Endpoint: 1 Cluster: 0x0000_001E Attribute 0x0000_0000 DataVersion: 1511515605
[1666874049.111440][11048:11053] CHIP:TOO:   Binding: 1 entries
...
```
./chip-tool.elf accesscontrol write acl '[{"fabricIndex": 1, "privilege": 5, "authMode": 2, "subjects": [112233], "targets": null}, {"fabricIndex": 1, "privilege": 3, "authMode": 2, "subjects": [#MotionSensorNodeId#], "targets": [{"cluster": 6, "endpoint": 1, "deviceType": null}]}]' #LightNodeId# 0
```
...
[1666874500.590815][11235:11240] CHIP:TOO:         [1]: #MotionSensorNodeId#
[1666874500.590833][11235:11240] CHIP:TOO:       Targets: 1 entries
...
5. Press `SW3` to trigger motion sensor occupied will turn on the Light device.
