...
class PTCDriver(RCCDriverGeneric):
    ...
    def __init__(
        ...
        ptc_commands: List[ConsoleCommand] = [
            ...
            CommandAttributeInt("TXMC_SRCID", ["Set logic channel for multi channel tx"], self.api, "txmc_srcid"),
            ...
            CommandAttributeEnum[PTC.BleRxDataRate](
                "BLERXDATARATE",
                ["Defines the BLE Data Rate that will be used for Rx"],
                self.api,
                "ble_rxdatarate",
                {
                    "1M": PTC.BleRxDataRate.Ble1M,
                    "2M": PTC.BleRxDataRate.Ble2M,
                    "LR": PTC.BleRxDataRate.BleLR,
                    "1M+LR": PTC.BleRxDataRate.BleConcurrent,
                },
            ),
            CommandAttributeEnum[PTC.BleDataRate](
                "BLETXDATARATE",
                ["Defines the BLE Data Rate that will be used for Tx"],
                self.api,
                "ble_txdatarate",
                {
                    "1M": PTC.BleDataRate.Ble1M,
            ...
            CommandAttributeInt(
                "WSET_ARG_CHANNEL", ["Set the channel to which the power will be set"], self.api, "wsetchannel"
            ),
            CommandAttributeInt("WSET_ARG_POWER", ["Set Tx power of a specific channel"], self.api, "wsetpower"),
            CommandAttributeInt("WGET_ARG_CHANNEL", ["Get Tx power of a specific channel"], self.api, "wgetchannel"),
            CommandAttributeInt("PCH_ARG_STATCHANNEL", ["Set channel of statistic print"], self.api, "pchstatchannel"),
            CommandAttributeEnum[self.api.Stats_Dump](
                "STATSDUMP_RXP",
                ["Dump output from RXP command to file"],
                self.api,
                "stats_dump_rxp",
                {
                    "DUMP_OFF": self.api.Stats_Dump.OFF,
                    "DUMP_CSV": self.api.Stats_Dump.CSV,
                },
            ),
            CommandAttributeEnum[self.api.Stats_Dump](
                "STATSDUMP_P",
                ["Dump output from P command to file"],
                self.api,
                "stats_dump_p",
                {
                    "DUMP_OFF": self.api.Stats_Dump.OFF,
        ...
        proto_modes = [
            ...
            ProtoModeAction(
                ...
                ["WSET_ARG_CHANNEL", "WSET_ARG_POWER"],
            ),
            ProtoModeAction(
                "WGET",
            ...
            ProtoModeAction(
                "P",
                ["Print Rx/Tx packet count"],
                self._console_print_statistics,
                ["STATSDUMP_P"],
            ),
            ProtoModeAction(
                "PCH",
            ...
            ProtoModeOnOff(
                ...
                None,
                ["TXMC_SRCID", "TXMC_CHANNEL", "TXMC_PACKETCOUNT", "TXMC_SETTXDATA", "TXMC_PACKETINTERVAL"],
                self._console_print_txmc_configuration,
            ),
            ProtoModeAction(
                "PANT",
                ["Print antenna statistics"],
                self._console_print_antenna_statistics,
            ),
            ProtoModeAction(
                "RANT",
                ["Reset antenna statistics"],
                self._console_reset_antenna_statistics,
            ),
            ProtoModeAction(
                "CHIPTEMP",
                ["Read current chip temperature"],
                self._console_print_mcu_temp,
            ),
            ProtoModeAction(
                "CLOAD_TABLE",
                ["Read load capacitors settings table"],
                self._console_print_cload_table,
            ),
        ]
    ...
    def _indication_message_cb(self, message: BaseIndication) -> None:
        if isinstance(message, RxPacketIndication):
            ...
            rssi = ctypes.c_int8(message.rssi).value
            channel = message.channel
            payload_list = [f"{x:02X}" for x in message.payload]
            stats_dump = message.stats_dump
            self._app.show_user_message(
                f"RX Packet: antenna: {antenna}, rssi: {rssi}, lqi: {lqi}, channel: {channel}, payload: {' '.join(payload_list)}, len: {len(payload_list)}"
            )
            if stats_dump == self.api.Stats_Dump.CSV.value:
                self.api.print_packets_to_CSV(
                    "RXP",
                    PTC.PhyMode.RF4CE.name,
                    [
                        "Antenna",
                        "RSSI",
                        "LQI",
                    ...
                    [f"{antenna}", f"{rssi}", f"{lqi}", f"{channel}", f"{' '.join(payload_list)}"],
                )
    ...
    def _console_print_antenna_statistics(self) -> List[str]:
        value = self.api.get_antenna_statistics()
        return [
            f"Antenna 0:",
            f"  Number of RX packets: {value.ant0_rx_packet_count}",
            f"  Avg RSSI: {value.ant0_avg_rssi}",
            f"Antenna 1:",
            f"  Number of RX packets: {value.ant1_rx_packet_count}",
            f"  Avg RSSI: {value.ant1_avg_rssi}",
        ]

    def _console_reset_antenna_statistics(self) -> None:
        self.api.reset_antenna_statistics()

    def _console_print_mcu_temp(self) -> List[str]:
        value = self.api.get_chip_temperature()
        return [
            f"Chip temperature: {value:.4}°C"
        ]

    def _console_print_cload_table(self) -> List[str]:
        caps_list = self.api.get_capacitor_load()
        temp_start = -30
        c_load_list = list()
        for i, x in enumerate(caps_list):
            temp_str_beg = str(temp_start + (i - 1) * 10) + "°C"
            temp_str_mid = " < T < "
            temp_str_end = str(temp_start + i * 10) + "°C"
            if i == 0:
                temp_str = "T < " + temp_str_end
            elif i == 16:
                temp_str = temp_str_end + " < T"
            else:
                temp_str = temp_str_beg + temp_str_mid + temp_str_end
            c_load_list.append(["cload_comp_" + str(i), temp_str, str(x)])
        return [tabulate(c_load_list, headers=["Parameter", "Temperature range", "Frequency Error [ppm]"])]
