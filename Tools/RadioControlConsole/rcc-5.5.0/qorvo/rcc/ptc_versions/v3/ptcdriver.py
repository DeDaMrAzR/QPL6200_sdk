...
class RxPacketIndication(BaseIndication):
    ...
    def from_api(cls: Type[BaseIndication._IndicationT], value: gpAPIConfirmIndication) -> BaseIndication._IndicationT:
        lqi = value.parList[0]
        antenna = value.parList[1]
        rssi = value.parList[2]
        channel = value.parList[3]
        val = value.parList[6]
        payload = val.to_bytes(length=(max(val.bit_length(), 1) + 7) // 8, byteorder="little")
        stats_dump = value.parList[7]
        return cls(lqi, antenna, rssi, channel, payload, stats_dump)

    def __init__(self, lqi: int, antenna: int, rssi: int, channel: int, payload: bytes, stats_dump: int):
        self.lqi = lqi
...
class AntennaStatistics:
    ant0_rx_packet_count: int
    ant0_avg_rssi: int
    ant1_rx_packet_count: int
    ant1_avg_rssi: int
...
class _BasePTC(abc.ABC):
    ...
    def get_attribute(self, attrib: int) -> int:
        ...
        if (
            ...
            code_str = gpPTCAPI.gpPTC_Result_t[ret.cmdDef.parValues["result"]]
            raise InvalidStateException(f"Wrong reply on get_attribute method ({code_str})")
    ...
    def set_mode(
        ...
        if (
            ...
            logger.debug(ret)
            code_str = gpPTCAPI.gpPTC_Result_t[ret.cmdDef.parValues["result"]]
            error_message = f"Wrong reply on set_mode method ({code_str})"
            action = None
            if ret.cmdDef.parValues["result"] in (gpPTCAPI.gpPTC_ResultTXDataNotSet, gpPTCAPI.gpPTC_ResultInvalidTXDataLength):
                action = [
                    "TX data is not set correctly for the test",
                    "Please make sure to set TX DATA",
                ]

            raise InvalidStateException(error_message, action)
...
def _int8(x: int) -> int:
    """ "
    Convert unsigned 8-bit integer from gpAPI into signed Python type.
    """
    return x - (1 << 8) * (x >= (1 << 7))
...
def _int32(x: int) -> int:
    """ "
    Convert unsigned 32-bit integer from gpAPI into signed Python type.
    """
    return x - (1 << 32) * (x >= (1 << 31))
...
class PTC(_BasePTC, PropertyAttributeDriver):
    ...
    def __init__(
        ...
        indication_message_cb: Optional[Callable[[BaseIndication], None]] = None,
    ) -> None:
        ...
        self._received_packet_counter = CsvPacketCounters()
    ...
    def channel(self):
        """Device active channel (alias for SRCID0)."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeChannelSrcId0)
    def channel_srcid0(self):
        """Device active main channel source."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeChannelSrcId1)
    def channel_srcid1(self):
        """Device active second channel source."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeChannelSrcId2)
    def channel_srcid2(self):
        """Device active third channel source."""
    ...
    @property_attribute_enum(gpPTCAPI.gpPTC_BLE_RxDataRate, BleRxDataRate)
    def ble_rxdatarate(self):
        """BLE Data Rate used for Rx"""

    @property_attribute_enum(gpPTCAPI.gpPTC_BLE_TxDataRate, BleDataRate)
    def ble_txdatarate(self):
        """BLE Data Rate used for Tx"""
    ...
    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone1MaxTemp)
    def xtaltrimtemp1(self):
        """XTALTRIMCAP zone 1 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone1Setting)
    def xtaltrimset1(self):
        """XTALTRIMCAP zone 1 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone2MaxTemp)
    def xtaltrimtemp2(self):
        """XTALTRIMCAP zone 2 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone2Setting)
    def xtaltrimset2(self):
        """XTALTRIMCAP zone 2 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone3MaxTemp)
    def xtaltrimtemp3(self):
        """XTALTRIMCAP zone 3 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone3Setting)
    def xtaltrimset3(self):
        """XTALTRIMCAP zone 3 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone4MaxTemp)
    def xtaltrimtemp4(self):
        """XTALTRIMCAP zone 4 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone4Setting)
    def xtaltrimset4(self):
        """XTALTRIMCAP zone 4 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone5MaxTemp)
    def xtaltrimtemp5(self):
        """XTALTRIMCAP zone 5 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone5Setting)
    def xtaltrimset5(self):
        """XTALTRIMCAP zone 5 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone6MaxTemp)
    def xtaltrimtemp6(self):
        """XTALTRIMCAP zone 6 max temperature"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone6Setting)
    def xtaltrimset6(self):
        """XTALTRIMCAP zone 6 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeXtalTrimCapZone7Setting)
    def xtaltrimset7(self):
        """XTALTRIMCAP zone 7 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeWsetChannel)
    def wsetchannel(self):
        """WSETCHANNEL zone 1 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeWgetChannel)
    def wgetchannel(self):
        """WGETCHANNEL zone 1 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeWsetPower)
    def wsetpower(self):
        """WSETPOWER zone 1 setting"""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeTxmcCurrentSrcId)
    def txmc_srcid(self):
        """Device active channel source - multi channel tx."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeTxmcChannel)
    def txmc_channel(self):
        """Device active channel  - multi channel tx."""

    @property_attribute_bytes(gpPTCAPI.gpPTC_AttributeTxmcData, cached=True)
    def txmc_txdata(self):
        """Transmit packet payload - multi channel tx."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeTxmcPacketInterval)
    def txmc_packet_interval_ms(self):
        """Packet interval in ms - multi channel tx."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributeTxmcPacketCount)
    def txmc_packet_count(self):
        """Number of packets - multi channel tx."""

    @property_attribute_int(gpPTCAPI.gpPTC_AttributePCHStatChannel)
    def pchstatchannel(self):
        """PCHSTATCHANNEL zone 1 setting"""

    class Stats_Dump(Enum):
        OFF = 0
        CSV = 1

    @property_attribute_enum(gpPTCAPI.gpPTC_AttributeDumpRxp, Stats_Dump)
    def stats_dump_rxp(self):
        """RXP output dump flag"""

    @property_attribute_enum(gpPTCAPI.gpPTC_AttributeDumpP, Stats_Dump)
    def stats_dump_p(self):
        """P output dump flag"""

    @property_attribute_bool(gpPTCAPI.gpPTC_AttributeDCDC)
    def dcdc(self):
        """dcdc mode flag."""
    ...
    def execute_custom_command(self, cc_id: int, arg_list: List[int] = []) -> List[int]:
        ...
        if (
            ...
            code_str = gpPTCAPI.gpPTC_Result_t[ret.cmdDef.parValues["result"]]
            raise InvalidStateException(f"Wrong reply on execute_custom_command method ({code_str})")
    ...
    def set_power_to_channel(self, channel: Optional[int], power: Optional[int]) -> None:
        """Writes power value of channel."""
        if channel is not None:
            self.wsetchannel = channel
        if power is not None:
            self.wsetpower = power

        # Mode
        self.set_mode(
            gpPTCAPI.gpPTC_SetTxPowerChannel,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            1,
            0x00,
    ...
    def get_power_of_channel(self, channel: Optional[int]) -> int:
        """Read power value of channel."""
        if channel is not None:
            self.wgetchannel = channel
        # Mode
        ret = self.set_mode(
            gpPTCAPI.gpPTC_GetTxPowerChannel,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            1,
            0x00,
        )
        rval = _int32(ret.cmdDef.parValues["parameters"])
        return rval
    ...
    def get_packet_statistics(self) -> PacketStatistics:
        ...
        stats_dump = struct.unpack("<i", bytearray(list_[25 : 25 + 4]))[0]

        if stats_dump == self.Stats_Dump.CSV.value:
            phy_mode = PTC.PhyMode(self.get_attribute(gpPTCAPI.gpPTC_AttributePhyMode))
            self.print_packets_to_CSV(
                "P",
                phy_mode.name,
                [
                    "Average RSSI",
    ...
    def get_packet_statistics_for_channel(self, channel_list: Optional[List[int]] = None) -> List[PacketStatistics]:
        """Returns a list of dict with raw dump of packet statistics for given channel."""
        if channel_list is None:
            channel_list = [self.pchstatchannel]

        stat_list = []
        for channel in channel_list:
            self.pchstatchannel = channel

            ret = self.set_mode(
                gpPTCAPI.gpPTC_ModePrintPacketCountForChannel,
                0,
                gpPTCAPI.gpPTC_ModeExecution_Single,
                5,
                0x00,
            ...
            # Unpacking data as signed int32, little endian
            list_ = ret.parBytes
            rssi = _int8(struct.unpack("<i", bytearray(list_[5 : 5 + 4]))[0])
            lqi = struct.unpack("<i", bytearray(list_[9 : 9 + 4]))[0]
            rx_packet_count = struct.unpack("<i", bytearray(list_[13 : 13 + 4]))[0]
            tx_ok = struct.unpack("<i", bytearray(list_[17 : 17 + 4]))[0]
            tx_fail = struct.unpack("<i", bytearray(list_[21 : 21 + 4]))[0]
            stat_list.append(ChannelPacketStatistics(channel, rssi, lqi, rx_packet_count, tx_ok, tx_fail))
        return stat_list

    def get_antenna_statistics(self) -> AntennaStatistics:
        """Returns a dict with raw dump of antenna statistics."""
        # Setting mode for printing packet count with 5 extra parameters
        ret = self.set_mode(
            gpPTCAPI.gpPTC_ModePrintAntennaStatistics,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            4,
            0x00,
        ...
        # Unpacking data as signed int32, little endian
        list_ = ret.parBytes
        ant0_rx_packet_count = struct.unpack("<i", bytearray(list_[5 : 5 + 4]))[0]
        ant0_avg_rssi = struct.unpack("<i", bytearray(list_[9 : 9 + 4]))[0]
        ant1_rx_packet_count = struct.unpack("<i", bytearray(list_[13 : 13 + 4]))[0]
        ant1_avg_rssi = struct.unpack("<i", bytearray(list_[17 : 17 + 4]))[0]

        return AntennaStatistics(ant0_rx_packet_count, ant0_avg_rssi, ant1_rx_packet_count, ant1_avg_rssi)

    def reset_antenna_statistics(self) -> None:
        """Resets PTC antenna statistics."""
        self.set_mode(
            gpPTCAPI.gpPTC_ModeResetAntennaStatistics,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            0,
            0x00,
    ...
    def reset_packet_statistics_for_channel(self, statchannel: Optional[int]) -> None:
        """Resets PTC packet statistics for given channel."""
        if statchannel is not None:
            self.pchstatchannel = statchannel
        # Setting mode for printing packet count with 5 extra parameters
        ret = self.set_mode(
            gpPTCAPI.gpPTC_ModeResetCountersForChannel,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            0,
            0x00,
    ...
    def set_xtal_trim_cap(self, sleep_mode: Optional[SleepModes] = None) -> None:
        """Configure the Xtal32M capacitor trimming algorithm."""
        self.set_mode(
            gpPTCAPI.gpPTC_ModeSetXtalTrim,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            1,
            0x00,
    ...
    def enable_tx_multi_channels(self) -> None:
        self.set_mode(
            gpPTCAPI.gpPTC_ModeTransmitMultipleChannels,
            0,
            gpPTCAPI.gpPTC_ModeExecution_On,
            1,
            0x00,
    ...
    def disable_tx_multi_channels(self) -> None:
        self.set_mode(
            gpPTCAPI.gpPTC_ModeTransmitMultipleChannels,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Off,
    ...
    def get_txmc_configuration(self, args: List[str]) -> Optional[TxmcConfiguration]:
        """Read txmc configuration."""
        if len(args) == 1 and int(args[0]) < 3:
            ret = self.set_mode(
                gpPTCAPI.gpPTC_ModePrintTxmcConfiguraiton,
                0,
                gpPTCAPI.gpPTC_ModeExecution_Single,
                4,
                0x00,
            ...
            # Unpacking data as signed int32, little endian
            list_ = ret.parBytes
            channel = struct.unpack("<i", bytearray(list_[5 : 5 + 4]))[0]
            packet_count = struct.unpack("<i", bytearray(list_[9 : 9 + 4]))[0]
            data_length = struct.unpack("<i", bytearray(list_[13 : 13 + 4]))[0]
            packet_interval = struct.unpack("<i", bytearray(list_[17 : 17 + 4]))[0]
            return TxmcConfiguration(channel, packet_count, data_length, packet_interval)
        return None

    def print_packets_to_CSV(self, command: str, phy_mode: str, header_fields: List[str], vals: List[str]) -> None:
        """print packet info to CSV file."""

        # replace symbols that may not be supported by every OS
        timestamp = str(self._starttime).replace(" ", "_").replace(":", "-").replace(".", "-")

        file_path = Path.cwd().joinpath("csv", "%s_%s_%s.csv" % (command, phy_mode, timestamp))
        if not file_path.parent.exists():
            os.mkdir("csv")

        # gets the packet counter for current command and phy mode
        if command == "RXP":
            packet_counter = self._received_packet_counter.rxp_rf4ce
            self._received_packet_counter.rxp_rf4ce += 1
        else:  # P command
            if phy_mode == "BLE":
                packet_counter = self._received_packet_counter.p_ble
                self._received_packet_counter.p_ble += 1
            else:  # RF4CE phy mode
                packet_counter = self._received_packet_counter.p_rf4ce
                self._received_packet_counter.p_rf4ce += 1

        # inserts info about packet numbers
        vals.insert(0, str(packet_counter))
        header_fields.insert(0, "Packet number")
        # writes header row if new csv file is created
        if not file_path.exists():
            with open(file_path, "w", newline="") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(header_fields)
        # writes packet info to csv file
        with open(file_path, "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(vals)

    def get_chip_temperature(self) -> float:
        """Get the current temperature of the chip in °C"""
        ret = self.set_mode(
            gpPTCAPI.gpPTC_ModeGetChipTemp,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            1,
            0x00,
        )
        chip_temp = _int32(ret.cmdDef.parValues["parameters"])
        chip_temp_float = (chip_temp>>8) + ((chip_temp&0x00FF)/255.0)
        return chip_temp_float

    def get_capacitor_load(self) -> list:
        """Get the load capacitor coefficient value"""
        ret = self.set_mode(
            gpPTCAPI.gpPTC_ModeGetCapacitorLoad,
            0,
            gpPTCAPI.gpPTC_ModeExecution_Single,
            17,
            0x00,
        )
        return list(struct.unpack("<17i", bytearray(ret.parBytes[5:(5 + 18 * 4)])))
