...
from .peakconnect import ConnectAndStart, peakConnector
from .threading import ProcessThread


class IndicationCallback(Protocol):
    def __call__(self, dev: "gpAPI_Generic", ind: "gpAPIConfirmIndication") -> None:
        ...
...
# Helper functions
# --------------------------
def ascii2hex(s: bytes) -> str:
    return "".join(["%02x" % ord(c) for c in s])  # the %02 pads when needed


def str2ascii(string: str) -> List[int]:
    return [ord(c) for c in string]


def bytesashex(a: Union[List[int], int]) -> str:
    s = "["
...
def int2Bytelist(inp: int, numBytes: int) -> List[int]:
    ret: List[int] = []
    if inp < 0:
...
class gpAPI_Generic(object):
    ...
    def __init__(
        self,
        cmdDictionary: Dict[str, List[Any]],
        formatParseSpecial: Tuple[
            Optional[Callable[[Any, Any, Any], str]],
            Optional[Callable[[Any, Any, Any], str]],
        ],
        processCallback: Optional[IndicationCallback] = None,
        name: str = "",
        ignoreWaitFunc: bool = False,
    ):
        self.verbose = False
        self.cmdDictionary = cmdDictionary
        self.ignoreWaitFunc = ignoreWaitFunc

        self.name: str = name if name else self.cmdDictionary["gpAPIInfo"][1]
        self.moduleID: int = self.cmdDictionary["gpAPIInfo"][0]
        ...
        self.events = queue.Queue[List[int]]()
        self.acks = queue.Queue[List[int]]()
    ...
    def _indicate(self, ind: "gpAPIConfirmIndication") -> None:
        ...
        if ind.cmdId == gpAPI.Ack_CmdId:
            # Ack normally caught when request was waiting for it
            self.log(ind)

        if self.processCallback is not None:
            self.processCallback(self, ind)

    def getEvents(self, timeout: int = 0, WaitF: Optional[Callable[[int], int]] = None) -> List[Any]:
    ...
    def Request(self, cmdId: int, parList: Any, noAck: bool = False, waitF: Optional[Callable[[float], float]] = None):
        # Format input
        ...
        cmd_def = gpAPICmdDef(
            ...
            self.cmdDictionary,
            parList=par_list,
            formatting=self.formatParseSpecial[0],
        )
    ...
    def waitForAck(self, cmdDef: "gpAPICmdDef", timeout=None, waitF=None):
        if not (self.isAcked):
            return None
        ...
        try:
            ...
            raise TimeoutError("Timeout %d sec - No ack received for %s" % (timeout, cmdDef.cmdName)) from ex

        # We can only process the correct ACK frame at this time. Did the sender generate an unexpected ack?
        assert cmdDef.cmdId == msg[2], "Received ACK for a wrong command id, expected %s got %s" % (
            cmdDef.cmdId,
            msg,
        )
...
class gpAPI(gpAPI_Generic):
    ...
    def new_connection(
        ...
        obj._owns_connector = True
        return obj

    def __init__(
        self,
        peakConnector: peakConnector,
        cmdDictionary: Dict[str, List[Any]],
        formatParseSpecial: Tuple[
            Optional[Callable[[Any, Any, Any], str]],
            Optional[Callable[[Any, Any, Any], str]],
        ...
        super().__init__(cmdDictionary, formatParseSpecial, processCallback, name, ignoreWaitFunc)
    ...
    def ReplacePeakConnector(self, p: peakConnector) -> None:
    ...
    def __str__(self):
        ...
        if self._owns_connector:
            formatted += "Connection : " + str(self.peakConnector.conn_param) + "\n"
        else:
...
class gpAPI_Tunnel(gpAPI):
    ...
    def SetDefaultDestination(self, destination=None):
        if destination is not None and destination not in self.destinations:
            raise ValueError(
                "Default destination '%s' is not set in destination possibilities\n - %s"
    ...
    def setValues(self, parList=None, retList=None):
        ...
        for i in range(len(self.parList)):
            if parList is not None:
                ...
                if isinstance(parList[i], float):
                    raise TypeError("Is float: %d %s. " % (i, repr(parList[i])))
            else:
        ...
        if isinstance(parameter, list):
            if parameterByteSize != 0:
                if parameterByteSize != len(parameter):
                    raise RuntimeError(
                        "%s - %s: List of length:%d Expected:%d"
                        % (
                            self.cmdName,
                            parameter[0],
                            len(parameter),
...
class gpAPIConfirmIndication(object):
    def __init__(self, message: List[int], cmdDef: gpAPICmdDef, formatParseSpecial=None):
        """
    ...
    def parseParamList(self):
        ...
        self.parList: Optional[Union[int, Tuple[int, int]]] = []
        counter = 0
        base_name = " "
...
class peakChannel(commChannel):
    ...
    def Cleanup(self):
        ...
        if self._HandleReceived in self.peakConnector.indicateCallbacks:
            self.peakConnector.indicateCallbacks.remove(self._HandleReceived)

    def _HandleReceived(self, moduleid: int, payload: List[int]) -> None:  # PeakConnector cb
        ...
        # Add to rx list
        if not self.rx.empty():
            self.log("!Message buffered - Queue not empty when adding a new message.\n")

        self.rx.put(payload[:])

    def sendMsg(self, payload: List[int]) -> None:  # gpAPI
