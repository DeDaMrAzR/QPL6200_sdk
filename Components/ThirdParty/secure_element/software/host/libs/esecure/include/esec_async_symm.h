/**
 * @brief eSecure Library: asynchronous symmetric keys management
 * @copyright Copyright (c) 2017-2018 Silex Insight. All Rights reserved
 * @file
 */

#ifndef ESEC_ASYNC_SYMM_HEADER
#define ESEC_ASYNC_SYMM_HEADER
...
/** Compute asynchronously the HMAC signature of the message.
 *
...
/** Create asynchronously a symmetric key randomly.
 *
...
/** Derive asynchronously a symmetric key from seed.
 *
...
/** Derive asynchronously a symmetric key from another symmetric key with HKDF
 *
...
/** Derive asynchronously a symmetric key from another symmetric key with PBKDF2
 *
...
uint32_t esec_derive_kdf2_skey_go(struct esec_mb* mb, struct esec_skey ikm, uint32_t hash_alg, const uint8_t* param,
                                  size_t param_sz, struct esec_key_storage storage, uint32_t len,
                                  struct esec_skey* okm);
...
/** Import asynchronously a symmetric key into a esec_skey.
 *
...
/** Export asynchronously a symmetric key from a esec_skey.
 *
 * After issuing this command, caller must wait a response becomes available
 * by calling ::esec_pop_cmd_response or polling ::esec_pop_cmd_response_if_any
 **/
...
/** Transform asynchronously a symmetric key into a new version with another
 * protected storage
 *
...
/** Delete asynchronously a symmetric key from eSecure
 *
