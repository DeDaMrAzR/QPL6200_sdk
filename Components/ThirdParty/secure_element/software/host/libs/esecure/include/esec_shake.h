...
#ifndef ESEC_SHA3_HEADER
#define ESEC_SHA3_HEADER
...
/**
 * @brief Compute shake operation which is a SHA3 operation with variable length specififed by the user
...
/**
 * @brief Start a shake operation and output intermediate state for context switching
 * @param algo which algo to use ESEC_HASH_ALGO_SHAKE_128 or ESEC_HASH_ALGO_SHAKE_256
 * @param msg message input to start with
 * @param len length of input data
 * @param state_out intermediate state after absorbing the input , length will always be ESEC_SHA3_HASH_STATE_SIZE
 */
uint32_t esec_shake_hash_begin(uint16_t algo, const uint8_t* msg, size_t len, uint8_t* state_out);

/**
 * @brief Update a shake operation using a previous state with new data and output intermediate state for context
 * switching
 * @param algo which algo to use ESEC_HASH_ALGO_SHAKE_128 or ESEC_HASH_ALGO_SHAKE_256
 * @param msg message input to be updated
 * @param len length of input data
 * @param state_in previous state to be updated length must always be ESEC_SHA3_HASH_STATE_SIZE
 * @param state_out new intermediate state after absorbing the new input , length will always be
...
/**
 * @brief Finish a shake operation using a previous state with new data and the final digest of the hash
 * @param algo which algo to use ESEC_HASH_ALGO_SHAKE_128 or ESEC_HASH_ALGO_SHAKE_256
 * @param msg message input to be updated
 * @param len length of input data
 * @param state_in previous state to be updated length must always be ESEC_SHA3_HASH_STATE_SIZE
