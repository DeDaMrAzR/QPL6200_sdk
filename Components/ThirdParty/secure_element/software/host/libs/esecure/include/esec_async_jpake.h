/**
 * @brief eSecure Library: Elliptic Curve JPAKE asynchronous crypto operation
 * @copyright Copyright (c) 2019 Silex Insight. All Rights reserved
 * @file
 */

#ifndef ESEC_ASYNC_JPAKE_HEADER
#define ESEC_ASYNC_JPAKE_HEADER
 ...
 * The password, user_id and remote_user_id must be smaller than the domain.
 *
 * See also esec_ecjpake_generate_r1_go()
 *
 * After issuing this command, caller must wait a response becomes available
 * by calling ::esec_pop_cmd_response or polling ::esec_pop_cmd_response_if_any
 **/
 ...
 *
 * See also esec_ecjpake_generate_r1_go() and esec_ecjpake_generate_r2_go()
 *
 * The hardware implementation only supports ECC P256 curves for now.
 *
 * The password, user_id and remote_user_id must be smaller than the domain.
