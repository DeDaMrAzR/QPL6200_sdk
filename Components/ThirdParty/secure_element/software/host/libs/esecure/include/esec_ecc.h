/**
 * @brief eSecure Library: Ellyptic Curve crypto acceleration
 * @copyright Copyright (c) 2017-2018 Silex Insight. All Rights reserved
 * @file
 */

#ifndef ESEC_ECC_HEADER
#define ESEC_ECC_HEADER
...
/** Generate an elliptic curve signature for the message.
 *
...
/** Verify the elliptic curve signature for the given message. */
...
/** Derive an ECC public and private key pair from seed.
 *
 ...
 **/
uint32_t esec_ecc_import_key(uint32_t domainspec, const uint8_t* material, struct esec_key_storage storage, int flags,
                             struct esec_ecc_key* key);
 ...
 * domain is used.
 */
extern bool esec_ecc_is_domain_included(const struct esec_ecc_key* key);
...
/** Generate an elliptic curve signature for the message digest.
 *
 * The signature is a coordinate pair. The signature buffer should be large
...
/** Verify the elliptic curve signature for the given message digest. */
