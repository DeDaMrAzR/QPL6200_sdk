...
int mbedtls_internal_sha256_process(mbedtls_sha256_context* ctx, const unsigned char data[64])
{
    ...
    ret = esec_hash_update(ctx->is224 ? ESEC_HASH_ALGO_SHA224 : ESEC_HASH_ALGO_SHA256,
                           (const uint8_t *)ctx->state, data, 64, (uint8_t *)ctx->state);
                              ...
                              size_t ilen)
{
    size_t fill;
    uint32_t left;
    uint16_t hash_algo = ctx->is224 ? ESEC_HASH_ALGO_SHA224 : ESEC_HASH_ALGO_SHA256;
    ...
    while(ilen >= 64)
    {
        ...
        ret = esec_hash_update(hash_algo, (const uint8_t *)ctx->state, input, steplen, (uint8_t *)ctx->state);
...
int mbedtls_sha256_finish(mbedtls_sha256_context* ctx, unsigned char* output)
{
    ...
    memcpy(output,
           ctx->state,
           ctx->is224 ? ESEC_HASH_SIZE_SHA224 : ESEC_HASH_SIZE_SHA256);
