...
static int mbedtls_ecp_mul_accel( mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
             const mbedtls_mpi *m, const mbedtls_ecp_point *P,
             int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    {
        uint32_t ret = MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
            /* Common sanity checks */
    ...
    if( ( ret = mbedtls_ecp_check_privkey( grp, m ) ) != 0 ||
        ...
        uint8_t point_buff[64];
        MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &P->X, point_buff, 32) );
        MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &P->Y, point_buff + 32, 32) );

        uint8_t scalar_buff[32];
        MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary(m, scalar_buff, 32) );

        uint8_t result_buff[64];
        uint32_t result = esec_ecc_secp256r1_point_mul(scalar_buff, point_buff, result_buff);

        MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &R->X, result_buff, 32) );
        MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &R->Y, result_buff + 32, 32) );
             ...
             mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    if(rs_ctx == NULL)
    {
        return mbedtls_ecp_mul_accel(grp, R, m, P, f_rng, p_rng);
    }
    else
    {
...
#define ECC_MAX_OPERAND_SZ 32
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &P->X, p_bytes, klen) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &P->Y, p_bytes + klen, klen) );

    MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( n, n_bytes, klen) );

    MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &Q->X, q_bytes, klen) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_write_binary( &Q->Y, q_bytes + klen, klen) );

    status = esec_ecc_secp256r1_point_muladd(m_bytes, p_bytes, n_bytes, q_bytes, r_bytes);

    if (status != ESEC_OKAY)
        return MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &R->X, r_bytes, 32) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &R->Y, r_bytes + 32, 32) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_lset( &R->Z, 1) );
