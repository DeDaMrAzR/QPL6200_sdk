...
static Bool BleHost_ForwardLegacyAdvReportEvent(gpHci_LeAdvertisingReportEventParams_t* pReports, Bool directed);
    ...
    switch (pBuf->opCode)
    {
        ...
        case gpHci_OpCodeLeReadResolvingListSize:
        {
            ...
            if(hciCb.secCback)
            {
                ...
                for(uint8_t randCount = 0; randCount < HCI_RESET_RAND_CNT; randCount++)
                {
                    HciLeRandCmd();
                }
            }
...
Bool BleHost_ForwardLegacyAdvReportEvent(gpHci_LeAdvertisingReportEventParams_t* pReports, Bool directed)
{
    hciEvt_t evt;
    UIntLoop i;
...
Bool WcBleHost_gpHci_CommandCompleteHandler(gpHci_EventCode_t eventCode,gpHci_CommandCompleteParams_t * pBuf)
{
    ...
    switch(pBuf->opCode)
    {
        ...
        case gpHci_OpCodeVsdDisablePeripheralLatency:
...
Bool gpHci_LeAdvertisingReportEvent(gpHci_LeAdvertisingReportEventParams_t* pReports)
{
    return BleHost_ForwardLegacyAdvReportEvent(pReports, false);
}

Bool gpHci_LeDataLengthChangeEvent(gpHci_LeDataLengthChangeEventParams_t * LEDataLengthChangeEvent)
{
    hciEvt_t evt;
...
Bool gpHci_LeReadRemoteFeaturesCompleteEvent(gpHci_LeReadRemoteFeaturesCompleteEventParams_t * LEReadRemoteFeaturesComplete)
{
    hciEvt_t evt;
...
Bool gpHci_LeRemoteConnectionParameterRequestEvent(gpHci_LeRemoteConnectionParameterRequestEventParams_t * LERemoteConnectionParameterRequest)
{
    hciEvt_t evt;
...
Bool gpHci_LeDirectAdvertisingReportEvent(gpHci_LeDirectAdvertisingReportEventParams_t* pReports)
{
    return BleHost_ForwardLegacyAdvReportEvent((gpHci_LeAdvertisingReportEventParams_t*)pReports, true);
}
...
Bool gpHci_LePhyUpdateCompleteEvent(gpHci_LePhyUpdateCompleteEventParams_t* LEPhyUpdateComplete)
{
    hciEvt_t evt;
...
{
    GP_LOG_SYSTEM_PRINTF("gpHci_LeConnectionlessIqReportEvent not supported",0);
    GP_ASSERT_SYSTEM(false);
    return true;
}
...
Bool gpHci_VsdFilterAcceptListModifiedEvent(gpHci_VsdFilterAcceptListModifiedEventParams_t* filterAcceptListModifiedParams)
{
