...
#define PSA_CRYPTO_ASSERT GP_ASSERT_DEV_EXT
...
#define KEY_ID_RANGE(key_id)  ((key_id >= 1 && key_id <= (NUMBER_OF_KEY_SLOTS)))
#define CHECK_KEY_ID(key_id) { if (!KEY_ID_RANGE(key_id)) return PSA_ERROR_INVALID_HANDLE; }
/*
...
int32_t get_key_index(psa_key_id_t key_id);
psa_key_id_t get_psa_key_id(int32_t key_index);
psa_esec_key* get_psa_esec_key(psa_key_id_t key_id);
