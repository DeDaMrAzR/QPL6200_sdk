...
typedef struct
{
  ...
  bool_t      useLegacyPdu[DM_NUM_ADV_SETS];   /*!< Whether to use legacy advertising PDUs with extended
...
typedef struct
{
  ...
  bool_t      startAdvAfterConnectionClose;    /*!< Start advertising after closing connection */
  wsfTimerTicks_t    securityRequestTimeout;   /*!< Timeout in ms before sending security request */
} appSlaveCfg_t;
...
typedef struct
{
  bool_t      waitForSec;                      /*!< TRUE to wait for a secure connection before initiating
                                                    discovery */
  bool_t      readDbHash;                      /*!< TRUE to try and read peer's database hash rather than
