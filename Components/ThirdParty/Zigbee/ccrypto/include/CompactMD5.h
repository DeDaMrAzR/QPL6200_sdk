// CompactMD5.h : header file
//
// Copyright(C) 2005 A. Honarbacht. All rights reserved.
//
// This code is based on an MFC version created back in 2001, which in turn 
// was derieved from an MFC implementation published on CodeGuru. This 2005 
// version is optimized for embedded applications with small memory footprint
//
// Refer to the 2001 implementation for further details.
...
class CMD5Calculator
{
	...
	protected:
		void Transform(const unsigned char abData[64]);
		
		static unsigned int RotateLeft(unsigned int x, int n);
		
		static void TransformFF(unsigned int &nA, const unsigned int nB, 
			const unsigned int nC, const unsigned int nD, const unsigned int nX, 
			const unsigned int nS, const unsigned int nT);
			
		static void TransformGG(unsigned int &nA, const unsigned int nB, 
			const unsigned int nC, const unsigned int nD, const unsigned int nX, 
			const unsigned int nS, const unsigned int nT);
		
		static void TransformHH(unsigned int &nA, const unsigned int nB, 
			const unsigned int nC, const unsigned int nD, const unsigned int nX, 
			const unsigned int nS, const unsigned int nT);
		
		static void TransformII(unsigned int &nA, const unsigned int nB, 
			const unsigned int nC, const unsigned int nD, const unsigned int nX, 
			const unsigned int nS, const unsigned int nT);

	protected:
		// Input buffer
		unsigned char m_abData[64];
		
		// Number of bits, modulo 2^64 (lsb first)
		unsigned int m_adwCount[2];
};
