    ...
    for(UInt8 i=0; i<GP_HAL_BLE_MAX_NR_OF_FILTER_ACCEPT_LIST_ENTRIES; i++)
    {
        ...
        if((stateMask != 0x00) && (matchMask & stateMask))
        {
            ...
            gpHal_BleGetFilterAcceptListEntry(i, &entry);
            {
                ...
                vsdParams.subEventCode = gpHci_VsdMetaSubEventCodeFilterAcceptListModified;
                GP_LOG_PRINTF_ADDRESS("[WL] Removed", entry.address);
                MEMCPY(&vsdParams.params.filterAcceptListModifiedEvent.address, &entry.address, sizeof(BtDeviceAddress_t));
                vsdParams.params.filterAcceptListModifiedEvent.isAdded = false;
...
gpHci_Result_t gpBle_ClearWL(gpBle_WlEntryType_t type)
{
    ...
    switch(type)
    {
        case gpBle_WlEntryRegular:
            ...
            if(gpBleConfig_IsLeMetaVSDEventMasked(gpHci_VsdMetaSubEventCodeFilterAcceptListModified))
            {
...
gpHci_Result_t gpBle_AddDeviceToWL(gpBle_WlEntryType_t type, gpHci_FilterAcceptListAddressType_t addressType, BtDeviceAddress_t* pAddress)
{
    ...
    if (status == gpHal_ResultSuccess && event == true)
    {
            ...
            MEMSET(&vsdParams, 0, sizeof(vsdParams));
            vsdParams.subEventCode = gpHci_VsdMetaSubEventCodeFilterAcceptListModified;
            MEMCPY(&vsdParams.params.filterAcceptListModifiedEvent.address, pAddress, sizeof(BtDeviceAddress_t));
            GP_LOG_PRINTF_ADDRESS("[WL] Adding", (*pAddress));
            vsdParams.params.filterAcceptListModifiedEvent.isAdded = true;
            gpBle_SendVsdMetaEvent(&vsdParams);
    }
    else
    {
