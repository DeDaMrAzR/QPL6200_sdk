...
gpHal_BleTxPhy_t BlePhy_Hci2HalTx(gpHci_Phy_t phy)
{
    ...
    if(txPhy == gpHal_BleTxPhyCoded125kb)
    {
        if(gpBleConfig_GetPreferredCodedPhyCoding() == gpHci_PhyWithCoding_Coded500kb)
        {
            txPhy = gpHal_BleTxPhyCoded500kb;
        }
    }
...
gpHci_Phy_t BlePhy_HalRx2Hci(gpHal_BleRxPhy_t phy)
{
    ...
    switch(phy)
    {
        /** Note: TX phy on purpose, as the coding is included (and that is not the case in the typedef for the RX phy) */
...
gpHci_Phy_t BlePhy_HalTx2Hci(gpHal_BleTxPhy_t phy)
{
    gpHci_Phy_t p = phy + 1;
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    if(phy == gpHal_BleTxPhyCoded500kb)
    {
        p--;
    }
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    if(all_phys & GP_HCI_PHY_MASK_CODED)
    {
        if(gpBleConfig_GetPreferredCodedPhyCoding() == gpHci_PhyWithCoding_Coded500kb)
        {
            return gpHal_BleTxPhyCoded500kb;
        }
        else
        {
            return gpHal_BleTxPhyCoded125kb;
        }
    }
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
