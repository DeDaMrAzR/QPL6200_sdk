...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
#define BLEDATACOMMON_SLOWEST_SUPPORTED_PHY_WITH_CODING         (gpHci_PhyWithCoding_Coded125kb)
#else
...
typedef struct {
    ...
    gpHci_PhyMask_t preferredPhyModesRx;
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    // Local copy of connection interval, used as tmp overrule when decreasing the connection interval on coded PHY.
    UInt16 intervalOverruleUnit;
#endif //GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
#if defined(GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED) &&  defined(GP_DIVERSITY_BLE_PERIPHERAL)
static UInt32 BleDataCommon_GetIntervalPortionAvailable(Ble_IntConnId_t connId);
static UInt16 Ble_GetEffectiveMaxTxTimeAvailable(Ble_IntConnId_t connId);
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED && (GP_DIVERSITY_BLE_CENTRAL || GP_DIVERSITY_BLE_PERIPHERAL)
...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
        case gpHci_Phy_Coded:
        {
            if(phyOptions == gpHci_PhyOptions_S2Coded)
            {
                phyWithCoding = gpHci_PhyWithCoding_Coded500kb;
            }
            else
            {
                phyWithCoding = gpHci_PhyWithCoding_Coded125kb;
            }
            break;
        }
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
gpHal_BleTxPhy_t BleDataCommon_HciPhyToHalTxPhy(gpHci_Phy_t hciPhy, gpHci_PhyOptions_t phyOptions)
{
    ...
    switch(hciPhy)
    {
        ...
        case gpHci_Phy_Coded:
        {
            if(phyOptions == gpHci_PhyOptions_S2Coded)
            {
                halPhy = gpHal_BleTxPhyCoded500kb;
            }
            else
            {
                halPhy = gpHal_BleTxPhyCoded125kb;
            }
            break;
        }
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
#if defined(GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED) &&  defined(GP_DIVERSITY_BLE_PERIPHERAL)
UInt32 BleDataCommon_GetIntervalPortionAvailable(Ble_IntConnId_t connId)
{
    ...
    if(Ble_DataLinkContext[connId].intervalOverruleUnit != BLE_CONN_INTERVAL_INVALID)
    {
        connIntervalUs = BLE_TIME_UNIT_1250_TO_US(Ble_DataLinkContext[connId].intervalOverruleUnit);
    }
    else
    {
    ...
    portionOccupiedUs = gpBleDataCommon_GetIntervalPortionOccupiedUs(connId);

    if(portionOccupiedUs > connIntervalUs)
    {
        // If negative, portion available is 0
        return 0;
    }

    return (connIntervalUs - portionOccupiedUs);
}

UInt16 Ble_GetEffectiveMaxTxTimeAvailable(Ble_IntConnId_t connId)
{
    return min(BLE_EFFECTIVE_MAX_TX_TIME_UNCODED(connId), BleDataCommon_GetIntervalPortionAvailable(connId));
}
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED && (GP_DIVERSITY_BLE_CENTRAL || GP_DIVERSITY_BLE_PERIPHERAL)
...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    // By default, we don't need an overrule interval
    Ble_DataLinkContext[connId].intervalOverruleUnit = BLE_CONN_INTERVAL_INVALID;
#endif //GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
UInt16 gpBle_GetEffectiveMaxTxTime(Ble_IntConnId_t connId)
{
    ...
    if(txPhy == gpHci_Phy_Coded)
    {
        // coded
        return max(BLE_INITIAL_MIN_TX_TIME_CODED, Ble_GetEffectiveMaxTxTimeAvailable(connId));
    }
    else
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    {
...
UInt16 gpBle_GetEffectiveMaxRxTime(Ble_IntConnId_t connId)
{
    ...
    hciPhy = gpBle_GetEffectivePhyRxType(connId);

    if(hciPhy == gpHci_Phy_Coded)
    {
        // coded
        return max(BLE_INITIAL_MIN_TX_TIME_CODED, BLE_EFFECTIVE_MAX_RX_TIME_UNCODED(connId));
    }
    else
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    {
...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
// Allow to set an overrule connection interval, to be used as input for calculating connEffectiveMaxTxTime
void gpBleDataCommon_SetConnectionIntervalOverrule(Ble_IntConnId_t connId, UInt16 intervalOverruleUnit)
{
    if(!Ble_DataIsConnectionIdValid(connId))
    {
        GP_ASSERT_DEV_INT(false);
        return;
    }

    Ble_DataLinkContext[connId].intervalOverruleUnit = intervalOverruleUnit;
}
#endif //GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
#endif // GP_DIVERSITY_BLE_2MBIT_PHY_SUPPORTED
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
        case gpHci_PhyWithCoding_Coded125kb:
        case gpHci_PhyWithCoding_Coded500kb:
        {
            hciPhy = gpHci_Phy_Coded;
            break;
        }
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
#ifdef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
    COMPILE_TIME_ASSERT(gpHci_PhyWithCoding_Coded125kb == (gpHal_BleTxPhyCoded125kb + 1));
    COMPILE_TIME_ASSERT(gpHci_PhyWithCoding_Coded500kb == (gpHal_BleTxPhyCoded500kb + 1));
#endif //GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
