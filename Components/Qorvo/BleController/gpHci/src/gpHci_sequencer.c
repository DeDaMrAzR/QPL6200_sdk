...
#include "gpHci_SubeventsVsdMeta.h"
...
#ifndef GP_BLE_DIVERSITY_HCI_MAXISOSDUSIZE
#define GP_BLE_DIVERSITY_HCI_MAXISOSDUSIZE 251
#endif //GP_BLE_DIVERSITY_HCI_MAXISOSDUSIZE
...
static Bool gpHci_VsdMetaHandler(gpHci_EventCode_t eventCode, gpHci_VsdMetaEventParams_t* pBuf)
{
    ...
    if(gpBleConfig_IsLeMetaVSDEventMasked(pBuf->subEventCode))
    {
        ...
        {
            case gpHci_VsdMetaSubEventCodeFilterAcceptListModified:
            {
                txSuccess = gpHci_VsdFilterAcceptListModifiedEvent(&pBuf->params.filterAcceptListModifiedEvent);
                break;
            }
            default:
...
static Bool gpHci_SendHCiEventFrameToHost(gpBle_EventServer_t eventServerId, gpBle_EventBufferHandle_t eventHandle)
{
    ...
    switch(eventCode)
    {
        ...
        {
            txSuccess = gpHci_LeMetaEvent(eventCode, (gpHci_LeMetaEventParams_t*)&pBuf->metaEventParams);
            break;
        }
