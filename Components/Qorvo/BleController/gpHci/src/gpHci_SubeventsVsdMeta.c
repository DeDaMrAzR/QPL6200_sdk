...
/*****************************************************************************
 *                    gpHci_SubeventsVsdMeta Indications
 *****************************************************************************/
...
#define subEventCode               dataBuf[2]
    ...
    subEventCode = gpHci_VsdMetaSubEventCodeLogging;
    lengthEvent = 1;

    // Transmit packet
    retVal = DATA_REQUEST(3, dataBuf, GP_HCI_COMM_ID | GP_COM_COMM_ID_HW_4);
    return retVal;
...
#define eventProcessedParamsPacket_tsLastPacketReceived     dataBuf[1 + 1 + 1 + 0 + 1 + (2) * 1 + (4) * 1]
    ...
    // Serialize header
    eventCode = gpHci_EventCode_VsdMeta;
    subEventCode = gpHci_VsdMetaSubEventCodeForwardProcessedMessages;
    lengthEvent = 12;
...
#define lengthEvent                dataBuf[1]
#define subEventCode               dataBuf[2]
#define filterAcceptListModifiedEventPacket                 (&dataBuf[1 + 1 + 1 + 0])
#define filterAcceptListModifiedEventPacket_address         dataBuf[1 + 1 + 1 + 0]
#define filterAcceptListModifiedEventPacket_isAdded         dataBuf[1 + 1 + 1 + 0 + (6) * 1]

    // Serialize payload
    MEMCPY(&(filterAcceptListModifiedEventPacket_address), (UInt8*)&(filterAcceptListModifiedEvent->address), (6) * 1);
    filterAcceptListModifiedEventPacket_isAdded = filterAcceptListModifiedEvent->isAdded;

    // Serialize header
    eventCode = gpHci_EventCode_VsdMeta;
    subEventCode = gpHci_VsdMetaSubEventCodeFilterAcceptListModified;
    lengthEvent = 8;
...
#undef lengthEvent
#undef subEventCode
#undef filterAcceptListModifiedEventPacket
#undef filterAcceptListModifiedEventPacket_address
#undef filterAcceptListModifiedEventPacket_isAdded
}
