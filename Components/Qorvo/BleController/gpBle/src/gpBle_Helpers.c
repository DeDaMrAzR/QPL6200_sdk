...
{
    if(expectedEarliestEc == expectedLastEc)
    {
        return false;
    }

    return gpBle_IsEcEarlierOrEqual(expectedEarliestEc, expectedLastEc);
}

// Check if expectedEarliestEc occured before or at the same moment as expectedLastEc (by calculating the EC distance)
...
{
    gpPd_ReadWithUpdate(pPdLoh, sizeof(gpHci_ChannelMap_t), pChannelMap->channels);
...
void gpBle_ReadConnectIndFromPd(gpPd_Loh_t* pPdLoh, Ble_ConnReqPdu_t* pConnReq)
{
    ...
    gpBle_ReadChannelMapFromPdu(pPdLoh, &pConnReq->llData.channelMap);
