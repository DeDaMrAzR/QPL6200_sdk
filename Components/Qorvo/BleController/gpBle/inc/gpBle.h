...
#define GPBLE_CONN_COMPLETE_BUFFER_OVERHEAD     ( offsetof(gpBle_EventBuffer_t, payload) + \
                                                  ...
                                                  offsetof(gpHci_LeMetaInfo_t,connectionCompleteEvent) )
...
#define GPBLECHANNELMAPMANAGER_HANDLE_INVALID       0xFF

#define GPBLECRYPTO_INIT_VECTOR_LENGTH                  8
#define GPBLECRYPTO_INIT_VECTOR_PART_LENGTH             (GPBLECRYPTO_INIT_VECTOR_LENGTH >> 1)
...
#define BLE_BM32_SET(bf,bm,idx,v)       bf &= ~bm; bf |= ((v) << idx)
...
typedef UInt8 gpBleChannelMapHandle_t;

typedef struct {
    UInt32 lsbs;
    UInt8 msbAndDirection;
} BleCrypto_PacketCounter_t;

typedef struct {
    BleCrypto_PacketCounter_t packetCounterIn;
    BleCrypto_PacketCounter_t packetCounterOut;
    UInt8 initVector[GPBLECRYPTO_INIT_VECTOR_LENGTH];
    UInt8 sessionKey[GP_HCI_ENCRYPTION_KEY_LENGTH];
} gpBleCrypto_CcmLinkContext_t;
...
Bool gpBle_SendLeMetaEventWithPayload(gpHci_EventCbPayload_t* pPayload);
...
Bool gpBle_IsEcEarlierOrEqual(UInt16 expectedEarliestEc, UInt16 expectedLastEc);
