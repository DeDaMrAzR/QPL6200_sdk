...
#error "One of both roles should be defined"
#endif //
...
Ble_LlcpLinkContext_t* Ble_GetLinkContext(Ble_IntConnId_t connId)
{
    ...
    if(BLE_IS_INT_CONN_HANDLE_VALID(Ble_LinkInfo[connId & ~(GPHAL_BLE_VIRTUAL_CONN_MASK)].connId))
    {
...
gpBleLlcp_FeatureStatus_t gpBleLlcp_IsFeatureSupported(gpHci_ConnectionHandle_t connectionHandle, gpBleConfig_FeatureId_t featureId)
{
    ...
    if(pContext->featuresExchangedStatus == gpBleLlcp_FeatureStatus_ExchangedSuccess)
    {
        ...
        if(BIT_TST64(pContext->featureSetLink, featureId))
        {
            return gpBleLlcp_FeatureStatus_Supported;
        }
        else
    ...
    else
    {
        ...
        if(BIT_TST64(pContext->allowedProcedures, featureId) == 0)
        {
    ...
    UInt32 firstTheoreticalConnEventTs, UInt16 masterSca, UInt8 nrOfSkippedIntervals, UInt32 InitialWindowSize)
{
    ...
    pConnEventInfo->channelMapPtr= gpRtIl_BleChannelMapHandleToOffset(chanMapHandle);
...
void Ble_LlcpStartConnectionEstablishmentSlave(Ble_IntConnId_t connId, Ble_ConnEstablishParams_t* pConnEstablishParams)
{
    ...
    gpBleChannelMapHandle_t channelMapHandle;
    ...
    channelMapHandle = gpBleChannelMapManager_AllocateHandle(&pConnEstablishParams->connReqPdu.llData.channelMap);

    GP_ASSERT_DEV_INT(gpBleChannelMapManager_IsHandleInUse(channelMapHandle));
...
void Ble_LlcpResetConnection(Ble_LlcpLinkContext_t* pContext, Bool permanentConnection)
{
    if(permanentConnection)
    {
        ...
        gpBleChannelMapManager_RemoveUser(pContext->channelMapHandle);
...
void gpBleLlcp_GetConnectionChannelMap(Ble_IntConnId_t connId, gpHci_ChannelMap_t* pChannelMap)
{
    ...
    gpBleChannelMapManager_GetChannelMap(pContext->channelMapHandle, pChannelMap->channels);
}

gpBleChannelMapHandle_t gpBleLlcp_GetConnectionChannelMapHandle(gpHci_ConnectionHandle_t hciHandle)
{
    Ble_LlcpLinkContext_t* pContext;

    pContext = Ble_GetLinkContext(gpBleLlcp_HciHandleToIntHandle(hciHandle));
    ...
    return pContext->channelMapHandle;
}
...
void gpBleLlcp_Init(gpHal_BleCallbacks_t* pCallbacks)
{
    ...
    for(i = 0; i < BLE_LLCP_MAX_NR_OF_CONNECTIONS; i++)
    {
        ...
        Ble_LinkInfo[i].channelMapHandleNext = GPBLECHANNELMAPMANAGER_HANDLE_INVALID;
    }
...
void gpBleLlcp_Reset(Bool firstReset)
{
    ...
    for(i = 0; i < BLE_LLCP_MAX_NR_OF_CONNECTIONS; i++)
    {
        Ble_LlcpLinkContext_t* pContext = &Ble_LinkInfo[i];
        ...
        if(Ble_LinkInfo[i].channelMapHandleNext != GP_HAL_BLE_CHANNEL_MAP_INVALID)
        {
            gpBleChannelMapManager_RemoveUser(pContext->channelMapHandleNext);
            pContext->channelMapHandleNext = GP_HAL_BLE_CHANNEL_MAP_INVALID;
        }
    }
...
void Ble_TriggerCbConnectionComplete(Ble_LlcpLinkContext_t* pContext, gpHci_LeConnectionCompleteEventParams_t* pParams)
{
    ...
    MEMCPY(&pEventBuf->payload.metaEventParams.params.connectionCompleteEvent, pParams, sizeof(gpHci_LeConnectionCompleteEventParams_t));
...
gpHci_Result_t gpBleLlcp_GetAnchorPointFromEventCounterInPast(Ble_IntConnId_t connId, UInt16 eventCounter, UInt32* tAnchor)
{
    ...
    if(!gpBle_IsEcEarlierOrEqual(eventCounter, connMetrics.eventCounterLastRx))
    {
...
void gpBleLlcp_AclChannelMapPreInstantPassed(Ble_LlcpLinkContext_t* pContext, Ble_LlcpProcedureContext_t* pProcedure)
{
    if(pProcedure->result == gpHci_ResultSuccess)
    {
        // If the handle is not valid, the connection will be stuck in the pause state. Add this assert here to get notified when this happens.
        GP_ASSERT_SYSTEM(gpBleChannelMapManager_IsHandleInUse(pContext->channelMapHandleNext));

        gpHal_BleUpdateChannelMap(pContext->connId, gpRtIl_BleChannelMapHandleToOffset(pContext->channelMapHandleNext));

        gpBleChannelMapManager_SwitchChannelMap(&pContext->channelMapHandle, &pContext->channelMapHandleNext, NULL);
    }
    else
    {
        gpBleChannelMapManager_RemoveUser(pContext->channelMapHandleNext);
...
gpHci_Result_t gpBle_VsdDisablePeripheralLatency(gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf)
{
    gpHci_ConnectionHandle_t HciHandle;
    Ble_IntConnId_t connId;
    BLE_SET_RESPONSE_EVENT_COMMAND_COMPLETE(pEventBuf->eventCode);

    MEMCPY(&HciHandle, &pParams->VsdDisablePeripheralLatency.connHandle, sizeof(gpHci_ConnectionHandle_t));
    connId = gpBleLlcp_HciHandleToIntHandle(HciHandle);

    gpBleLlcp_ProhibitSlaveLatency(connId, pParams->VsdDisablePeripheralLatency.disabled, Ble_ProhibitSlaveLatency_HostStack);
