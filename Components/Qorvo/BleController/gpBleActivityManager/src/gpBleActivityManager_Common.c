...
void BleActivityManager_ProcessEstablished(BleActivityManager_EventProcessed_t* pContext, Ble_MonitorLinkContext_t* pMonitorContext, gpHal_ConnEventMetrics_t* pMetrics)
{
    ...
    // window widening only applies for peripheral (ACL and CIS) connections
    if (!pContext->centralConnection)
    {
        if(pMetrics->winwidening_limit_reached)
        {
            pMonitorContext->slaveNbrConsecutiveLargeCorrWindows += 1;

            // The windowDuration and the winwidening_limit_reached flag reflect the total correlator window size for the next event (cfr SW arch doc $5.5.1)
            // Only drop the link after we observed 2 consecutive events where the window widening limit was exceeded
