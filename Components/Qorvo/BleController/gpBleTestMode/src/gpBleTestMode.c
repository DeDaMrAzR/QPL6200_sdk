...
#ifdef GP_DIVERSITY_BLE_TEST_MODE_END_RETURN_NR_TX
            GP_ASSERT_DEV_EXT(Ble_TestModeContext.numberOfTxPackets >= testModeEnd.nrOfPackets);
            testModeEnd.nrOfPackets = Ble_TestModeContext.numberOfTxPackets - testModeEnd.nrOfPackets;
#else
...
gpHci_Result_t Ble_ReceiverTestStartChecker(gpHci_VsdLeReceiverTest_v3Command_t* pTest)
{
    ...
    if(pTest->expectedCteLengthUnit > 0)
    {
        ...
        GP_LOG_PRINTF("BleMask_HasPhy CTE: %u",0,pTest->phyMask.mask);
        if(BleMask_HasPhy(pTest->phyMask, gpHci_Phy_Coded))
        {
            GP_LOG_PRINTF("CTE not allowed on phy gpHci_Phy_Coded",0);
            return gpHci_ResultCommandDisallowed;
        }
    ...
    if(pTest->antenna > gpHal_AntennaSelection_Ant1)
    {
...
#ifndef GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
...
gpHal_BleTxPhy_t Ble_TransmitterTxPhyToHalPhy(gpHci_PhyWithCoding_t txTestPhy)
{
    ...
    switch (txTestPhy)
    {
        ...
        case gpHci_PhyWithCoding_Coded125kb:
            rv = gpHal_BleTxPhyCoded125kb;
            break;
        case gpHci_PhyWithCoding_Coded500kb:
            rv = gpHal_BleTxPhyCoded500kb;
            break;
#endif // GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED
        default:
            break;
...
gpHci_Result_t BleTestMode_GenericRxTestFunction(BleTestMode_TestModeVersionId_t versionId, gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#if defined(GP_DIVERSITY_BLE_DISABLE_TESTV3_CMDS)
