...
#if defined(GP_DIVERSITY_BLE_2MBIT_PHY_SUPPORTED) && defined(GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED)
    .featureMask = BM(gpBleConfig_FeatureIdLe2MbitPhy) | BM(gpBleConfig_FeatureIdLeCodedPhy),
#elif defined(GP_DIVERSITY_BLE_2MBIT_PHY_SUPPORTED)
...
#endif
#if defined(GP_DIVERSITY_BLE_CODED_PHY_SUPPORTED)
    else if(phys.mask & GP_HCI_PHY_MASK_CODED)
    {
        return gpHci_Phy_Coded;
    }
#endif
    else
    {
...
void Ble_LlcpPhyUpdateScheduleCompleteEvent(gpHci_ConnectionHandle_t connHandle, gpHci_Result_t result, gpHci_Phy_t txPhy, gpHci_Phy_t rxPhy)
{
    ...
    payload.metaEventParams.params.phyUpdateCompleteEvent.status = result;
    payload.metaEventParams.params.phyUpdateCompleteEvent.connectionHandle = connHandle;
    payload.metaEventParams.params.phyUpdateCompleteEvent.txPhy = txPhy;
    payload.metaEventParams.params.phyUpdateCompleteEvent.rxPhy = rxPhy;
...
{
    gpHci_LeDataLengthChangeEventParams_t params;
    Ble_LlcpPhyUpdateData_t * pData;
      ...
      )
    {
        ...
        MEMCPY(&payload.metaEventParams.params.dataLengthChangeEvent, &params, sizeof(gpHci_LeDataLengthChangeEventParams_t));
        gpBle_ScheduleEvent(0, gpHci_EventCode_LeMeta, &payload);
