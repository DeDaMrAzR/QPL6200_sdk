...
void Ble_LlcpFeaturesNotifyHost(Ble_LlcpLinkContext_t* pContext, gpHci_Result_t result)
{
    ...
    MEMSET(&params.metaEventParams.params.readRemoteFeaturesCompleteEvent, 0, sizeof(gpHci_LeReadRemoteFeaturesCompleteEventParams_t));

    params.metaEventParams.subEventCode = gpHci_LeMetaSubEventCodeReadRemoteFeaturesComplete;
    params.metaEventParams.params.readRemoteFeaturesCompleteEvent.status = result;
    params.metaEventParams.params.readRemoteFeaturesCompleteEvent.connectionHandle = pContext->hciHandle;
    GP_LOG_PRINTF(" LlcpFeaturesNotifyHost result 0x%x " ,2, result);
    if(result == gpHci_ResultSuccess)
    {
        MEMCPY(params.metaEventParams.params.readRemoteFeaturesCompleteEvent.features, (UInt8*)&pContext->featureSetLink, GP_HCI_FEATURE_SET_SIZE);
    }
