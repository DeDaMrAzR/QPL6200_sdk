        ...
        case gpBleLlcp_OpcodeRejectExtInd:
        {
            if(pPdLoh->length >= 1)
            {
                ...
                if(rejectOpcode == pProcedure->currentTxPdu)
                {
                    if(pPdLoh->length >= 2)
                    {
                        // Only copy the resultcode from the LL_REJECT_EXT_IND when the procedure was successful till now.
                        // If we already registered a failure, we report that original resultcode
                        if ( (gpHci_ResultSuccess == pProcedure->result) ||
                             (gpHci_ResultInvalid == pProcedure->result) )
                        {
