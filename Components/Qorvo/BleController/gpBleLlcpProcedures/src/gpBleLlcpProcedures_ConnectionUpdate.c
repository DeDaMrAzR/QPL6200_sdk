...
void BleLlcpProcedures_ScheduleUpdateCompleteEvent(Ble_LlcpLinkContext_t* pContext, Ble_LlcpProcedureContext_t* pProcedure)
{
    ...
    payload.metaEventParams.params.connectionUpdateCompleteEvent.status = pProcedure->result;
    payload.metaEventParams.params.connectionUpdateCompleteEvent.connectionHandle = pContext->hciHandle;

    if(pProcedure->result == gpHci_ResultSuccess)
    {
        // Only add those parameters in case of success
        payload.metaEventParams.params.connectionUpdateCompleteEvent.connInterval = pConnUpdatePdu->interval;
        payload.metaEventParams.params.connectionUpdateCompleteEvent.connLatency = pConnUpdatePdu->pdu.latency;
        payload.metaEventParams.params.connectionUpdateCompleteEvent.supervisionTo = pConnUpdatePdu->pdu.timeout;
    }
