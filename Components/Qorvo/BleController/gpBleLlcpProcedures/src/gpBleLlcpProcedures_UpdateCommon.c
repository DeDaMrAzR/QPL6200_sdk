...
void Ble_LlcpPreInstantPassed(Ble_IntConnId_t connId, Bool scheduleInstantPassed)
{
    ...
    // this will trigger a wakup to decrease the priority again, we skip
    // the priority increase/decrease if there is only one activity
    if (1 < gpBleActivityManager_GetNrOfActivities())
    {
