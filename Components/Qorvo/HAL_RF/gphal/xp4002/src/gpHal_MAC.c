...
COMPILER_ALIGNED(GP_WB_MAX_MEMBER_SIZE) UInt8 gpHal_ZB_RawMode_VsIE_Pool[GP_HAL_ZB_RAW_VSIE_SIZE*GPHAL_ZB_VSIE_POOLSIZE] LINKER_SECTION(".rt_fast_ram");
...
// gpHalMac_SetRxOnWhenIdle relies on the fact that we have at most 8 channel indices.
GP_COMPILE_TIME_VERIFY(gpHal_ChannelSlotIdentifier_Invalid <= 8);
...
static const ROM gpHal_PbmTxOptions_t gpHalMac_PbmTxOptionsDefault FLASH_PROGMEM = {
    ...
    12,                                            // tx_power_setting
    0,                                             // phy_mode
    GP_WB_ENUM_AK_ANTSEL_INT_USE_PORT_FROM_DESIGN, // antsel_int
    GP_WB_ENUM_AK_ANTSEL_EXT_USE_PORT_FROM_DESIGN, // antsel_ext
    false,                                         // ext_pa_required
    0x3,                                           // min_be
    0x5,                                           // max_be
    0x4,                                           // max_csma_backoffs;
    0,                                             // poll_req_max_wait_time
...
static UInt8 gpHal_MacQueueUsage = 0;
static UInt8 gpHal_activeRawVsIeIndex;
const Int8 gpHal_RxSensitivity = -100; // -100 dBm
...
static void gpHal_InitMacQueueUsage(void);

/*****************************************************************************
 *                    Static Function Definitions
 *****************************************************************************/

STATIC INLINE void gpHalMac_InitMacFilter(void)
{
    ...
    GP_WB_WRITE_RT_ZB_ACK_MAX_FRAME_LEN(45);
#endif //defined(GP_HAL_DIVERSITY_RAW_ENHANCED_ACK_RX)

    GP_WB_WRITE_RT_ZB_ACK_TURNAROUND(160); // jodc: due to update of other warm-up times.
}
...
STATIC INLINE UInt8 gpHalMac_GetInternalRxChannel(gpHal_ChannelSlotIdentifier_t srcId)
{
    ...
    {
        case 0:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_0();
            break;
        case 1:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_1();
            break;
        case 2:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_2();
            break;
        case 3:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_3();
            break;
        case 4:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_4();
            break;
        case 5:
            channel = GP_WB_READ_AK_PLME_ACTIVE_CHANNEL_5();
            break;
        default:
...
void gpHalMac_SetTxOptions(UInt8 pbmEntry, gpHal_PbmTxOptions_t* pTxOptions)
{
    ...
    u16 = 0x0;
    GP_WB_SET_ZB_FORMAT_T_FIRST_BOFF_IS_0_TO_CSMA_OPTIONS(u16, pTxOptions->first_boff_is_0);
    GP_WB_SET_ZB_FORMAT_T_CSMA_CA_ENABLE_TO_CSMA_OPTIONS(u16, pTxOptions->csma_ca_enable);
    GP_WB_SET_ZB_FORMAT_T_TREAT_CSMA_FAIL_AS_NO_ACK_TO_CSMA_OPTIONS(u16, pTxOptions->treat_csma_fail_as_no_ack);
    GP_WB_SET_ZB_FORMAT_T_MIN_BE_TO_CSMA_OPTIONS(u16, pTxOptions->min_be);
    GP_WB_SET_ZB_FORMAT_T_MAX_BE_TO_CSMA_OPTIONS(u16, pTxOptions->max_be);
    GP_WB_SET_ZB_FORMAT_T_MAX_CSMA_BACKOFFS_TO_CSMA_OPTIONS(u16, pTxOptions->max_csma_backoffs);
    GP_WB_SET_ZB_FORMAT_T_RETRANSMIT_ON_CCA_FAIL_TO_CSMA_OPTIONS(u16, pTxOptions->retransmit_on_cca_fail);
    GP_WB_WRITE_ZB_FORMAT_T_CSMA_OPTIONS(optsbase, u16);

    u8 = 0;
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_IDX_TO_CHANNEL_FLOW(u8, 0);
    /* on unsuccessful transmission on ch0, packet will be re-transmitted on ch0a, then ch0b, ch0c, ch0a etc  */
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0A_EN_TO_CHANNEL_FLOW(u8, pTxOptions->channel_ch0a_en);
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0B_EN_TO_CHANNEL_FLOW(u8, pTxOptions->channel_ch0b_en);
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0C_EN_TO_CHANNEL_FLOW(u8, pTxOptions->channel_ch0c_en);
    /* if packet transmitted successfully, change to channel ch1 after transmit */
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH1_EN_TO_CHANNEL_FLOW(u8, pTxOptions->channel_ch1_en);
    GP_WB_WRITE_ZB_FORMAT_T_CHANNEL_FLOW(optsbase, u8);

    u16 = 0;
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0A_TO_CHANNELS(u16, pTxOptions->channel_ch0a);
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0B_TO_CHANNELS(u16, pTxOptions->channel_ch0b);
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH0C_TO_CHANNELS(u16, pTxOptions->channel_ch0c);
    GP_WB_SET_ZB_FORMAT_T_CHANNEL_CH1_TO_CHANNELS(u16, pTxOptions->channel_ch1);
    GP_WB_WRITE_ZB_FORMAT_T_CHANNELS(optsbase, u16);

    u16 = 0x0;
    GP_WB_SET_ZB_FORMAT_T_ACKED_MODE_TO_FLOW_CTRL(u16, pTxOptions->acked_mode);
    GP_WB_SET_ZB_FORMAT_T_RX_WD_START_ON_FP_TO_FLOW_CTRL(u16, 0);
    GP_WB_SET_ZB_FORMAT_T_ATTENUATOR_SEL_TO_FLOW_CTRL(u16, GP_WB_ENUM_AK_ATTENUATOR_SEL_NO_ATTENUATION);
    // clip the value so it does not overwrite elements of the ZB_FORMAT_T
    if(pTxOptions->max_frame_retries > 15)
    {
        pTxOptions->max_frame_retries = 15;
    }
    GP_WB_SET_ZB_FORMAT_T_MAX_FRAME_RETRIES_TO_FLOW_CTRL(u16, pTxOptions->max_frame_retries);
    GP_WB_SET_ZB_FORMAT_T_NUM_CHANNEL_SLOTS_TO_FLOW_CTRL(u16, pTxOptions->num_channel_slots);
    GP_WB_WRITE_ZB_FORMAT_T_FLOW_CTRL(optsbase, u16);

    // reset the counters
    u8 = 0x0;
    GP_WB_SET_ZB_FORMAT_T_RETRY_CNT_TO_FLOW_STATUS(u8, 0);
    GP_WB_SET_ZB_FORMAT_T_CCA_CNT_TO_FLOW_STATUS(u8, 0);
    GP_WB_WRITE_ZB_FORMAT_T_FLOW_STATUS(optsbase, u8);

    GP_WB_WRITE_ZB_FORMAT_T_ANTENNA(optsbase, pTxOptions->antenna);
    GP_WB_WRITE_ZB_FORMAT_T_TX_POWER_CONFIG(optsbase, pTxOptions->tx_power_setting);

#if defined(GP_HAL_DIVERSITY_EXT_MODE_SIGNALS) 
     u16 = 0x0;
#if defined(GP_HAL_DIVERSITY_EXT_MODE_SIGNALS)
    GP_WB_SET_ZB_FORMAT_T_MODE_CTRL_TO_EXTERNAL_PA_FEM_SETTINGS(u16, pTxOptions->phy_mode);
#endif
    GP_WB_WRITE_ZB_FORMAT_T_EXTERNAL_PA_FEM_SETTINGS(optsbase, u16);
#endif

#if defined(GP_DIVERSITY_DEVELOPMENT) && defined(GP_BSP_RF_DEBUG_TX_SUPPORTED)
#ifndef GP_HAL_DIVERSITY_EXT_MODE_SIGNALS
    GP_WB_WRITE_ZB_FORMAT_T_MODE_CTRL(optsbase, BM(GP_BSP_RF_DEBUG_TX_MODE_CTRL_BIT));
#endif
#endif
...
static UInt32 gpHalMac_CalculatePollReqMaxWaitTime(gpPad_Handle_t padHandle)
{
    gpPad_Attributes_t padAttributes;
    gpPad_GetAttributes(padHandle, &padAttributes);
#define pOptions (&padAttributes)

    // Formula 802.15.4-2006
    // { sum[pow(2, minBE + k), k=0..m-1] + (pow(2, maxBE) - 1) * (maxCSMA_backoffs - m) } * aUnitBackoffPeriod
    // + phyMaxFrameDuration with minBe, maxBE and maxCSMA_backoffs stack dependent m = min(maxBE - minBE,
    // maxCSMA_backoffs) aUnitBackoffPeriod = 20 symbols phyMaxFrameDuration = (127 + 6) * 2 symbols
    UInt8 m = min((pOptions->maxBE - pOptions->minBE), pOptions->maxCsmaBackoffs);
    UInt32 gpHal_PollReqMaxWaitTime = 0;
    UIntLoop i;
    for(i = 0; i < m; i++)
    {
        gpHal_PollReqMaxWaitTime += (1 << (i + pOptions->minBE));
    }
    gpHal_PollReqMaxWaitTime += ((1 << pOptions->maxBE) - 1) * (pOptions->maxCsmaBackoffs - m);
    gpHal_PollReqMaxWaitTime *= 20;            // aUnitBackOffPeriod
    gpHal_PollReqMaxWaitTime += (127 + 6) * 2; // phyMaxFrameDuration
    gpHal_PollReqMaxWaitTime *= 16;            // symbols => us
    return gpHal_PollReqMaxWaitTime;
}

static void gpHalMac_PreparePollRequestPBM(gpHal_Address_t optsbase, gpPad_Handle_t padHandle)
{
#ifdef GP_DIVERSITY_GPHAL_NO_FP_BIT_NEEDED_FOR_RESPONSE_WINDOW
    GP_WB_WRITE_ZB_FORMAT_T_RX_WD_START_ON_FP(optsbase, false);
#else
    GP_WB_WRITE_ZB_FORMAT_T_RX_WD_START_ON_FP(optsbase, true);
#endif // GP_DIVERSITY_GPHAL_NO_FP_BIT_NEEDED_FOR_RESPONSE_WINDOW

    UInt32 gpHal_PollReqMaxWaitTime = gpHalMac_CalculatePollReqMaxWaitTime(padHandle);
    GP_WB_WRITE_ZB_FORMAT_T_RX_WD_DURATION(optsbase, gpHal_PollReqMaxWaitTime);
    GP_WB_WRITE_ZB_FORMAT_T_RX_WD_ENABLE(optsbase, true);
}

void gpHal_CalibrateMacChannel(gpHal_ChannelSlotIdentifier_t srcId, UInt8 macChannel)
{
    UInt16 arg = (srcId)  | ((macChannel - 11) << 8);
    gpHal_IpcTriggerCommand(GP_WB_ENUM_AK_RT_CMD_TYPE_SX_CAL_RT, sizeof(arg), (UInt8 *)&arg);
}

static void gpHalMac_SetRxOnWhenIdle(gpHal_ChannelSlotIdentifier_t srcId, UInt8 channel, Bool enable)
{
    gpHal_Result_t result;
    UInt8 rxOnWhenIdle;

    if(srcId >= gpHal_ChannelSlotIdentifier_Invalid)
    {
        GP_ASSERT_SYSTEM(false);
        return;
    }

    rxOnWhenIdle = GP_WB_READ_RT_RX_ON_WHEN_IDLE_CH();

    if(enable)
    {
        BIT_SET(rxOnWhenIdle, srcId);
        GP_WB_WRITE_U8(GP_WB_RT_RX_CH0_ADDRESS + srcId, channel - 11);
    }
    else
    {
        BIT_CLR(rxOnWhenIdle, srcId);
    }

    GP_WB_WRITE_RT_RX_ON_WHEN_IDLE_CH(rxOnWhenIdle);

    result = gpHal_IpcTriggerCommand(GP_WB_ENUM_RT_CMD_TYPE_UPDATE_RX_ON_WHEN_IDLE, 0, NULL);
...
UInt8 gpHalMac_GetChannel_LowTXPower(UInt8 *low_channel)
{
    ...
    rxOn = GP_WB_READ_AK_RTM_TRC_RX_ON_WHEN_IDLE_CH();
    for (idx=0; idx<6; idx++)
    {
        if (BIT_TST(rxOn, idx))
        {
...
{
    {
        GP_WB_WRITE_AK_RX_EN_PIP(pipmode);
    }
}
...
void gpHal_SetCCAThreshold(void)
{
    ...
    GP_WB_WRITE_RT_ZB_CCA_ED_THRESH(protoEDThreshold);
}
...
void gpHalMac_InitDefaultTransmitPowers(void)
{
    MEMSET(gpHal_pDefaultTransmitPowerTable, GPHAL_MAX_TRANSMIT_POWER, sizeof(gpHal_pDefaultTransmitPowerTable));
}
...
void gpHalMac_SetAckTransmitPowers(void)
{
    ...
    gpHalMac_ApplyAckTransmitSettings(gpHal_SourceIdentifier_0, channel);
    gpHalMac_ApplyAckTransmitSettings(gpHal_SourceIdentifier_1, channel);
    gpHalMac_ApplyAckTransmitSettings(gpHal_SourceIdentifier_2, channel);
}
...
void gpHalMac_ApplyAckTransmitSettings(gpHal_SourceIdentifier_t srcId, UInt8 channel)
{
    ...
    UInt32 ackPbmOptsBase = ak_rap_rt_from_ptr(GP_WB_READ_RT_ACK_ZB_FORMAT_T_PTR());
    GP_WB_WRITE_ZB_FORMAT_T_TX_POWER_CONFIG(ackPbmOptsBase, txOptions.tx_power_setting);
...
void gpHalMac_InitAckPbm(void)
{
    ...
    if (!GP_HAL_MAC_STATE_GET_ACKPBM_INITIALIZED())
    {
        ...
        optsbase = GP_HAL_PBM_ENTRY2ADDR_OPT_BASE(pbmHandleAck);
        GP_WB_WRITE_RT_ACK_ZB_FORMAT_T_PTR(ak_rap_rt_to_ptr(optsbase));
        GP_WB_WRITE_ZB_FORMAT_T_FRAME_BUFFER_START(optsbase, GP_HAL_PBM_ENTRY2ADDR_DATA_BASE(pbmHandleAck));
        GP_WB_WRITE_ZB_FORMAT_T_FRAME_BUFFER_SIZE(optsbase, GP_HAL_RESERVED_PBM_SIZE);
        GP_WB_WRITE_ZB_FORMAT_T_FRAME_OFFSET(optsbase, 0);
        GP_WB_WRITE_ZB_FORMAT_T_FRAME_LEN(optsbase, 0); //updated by RT
...
{
    const UInt8* deviceAddress = gpHal_InfoPageGetMacAddress();

    GP_ASSERT_SYSTEM(deviceAddress);
...
void gpHalMac_InitMacAddress(void)
{
    ...
    GP_WB_WRITE_MACFILT_DST_PAN_ID_TABLE_SIZE(GP_HAL_MAC_NUMBER_OF_RX_SRCIDS);
    for (srcId = 0; srcId < GP_HAL_MAC_NUMBER_OF_RX_SRCIDS; ++srcId)
    {
...
void gpHal_InitMAC(void)
{
    ...
    gpHalMac_InitMacFilter();

    gpHalMac_InitMacAddress();

    gpHalMac_InitRxConfig();

    gpHalMac_InitDefaultTransmitPowers();
    ...
    gpHal_activeRawVsIeIndex = 1;
    gpHal_SetEnhAckVSIE(0, NULL);

    gpHal_InitMacQueueUsage();

    gpHal_InitMAC_RxWindows();
}
...
{
    UInt8 radioState;
    radioState=GP_WB_READ_AK_PLME_TRX_STATE_STATUS();

    switch(radioState)
    {
        case GP_WB_ENUM_AK_PHY_STATE_TRX_OFF:
        {
            radioState = gpHal_ResultTrxOff;
            break;
        }
        case GP_WB_ENUM_AK_PHY_STATE_TX_ON:
        {
            radioState = gpHal_ResultTxOn;
            break;
        }
        case GP_WB_ENUM_AK_PHY_STATE_RX_ON:
        {
...
static void gpHalMac_FillInTxOptions(UInt8 pbmHandle, gpPad_Handle_t padHandle)
{
    gpHal_PbmTxOptions_t txOptions;
...
#define pOptions (&padAttributes)
    ...
    // tx retry in case of cca fail, but keep cca_fail return code.
    txOptions.retransmit_on_cca_fail = pOptions->retransmitOnCcaFail;
    ...
    txOptions.acked_mode = GP_WB_READ_ZB_FORMAT_T_ACKED_MODE(optsbase);
    if(!txOptions.channel_ch0b_en && !txOptions.channel_ch0c_en)
    {
        ...
        // retransmit_on_cca_fail is only usefull if max_frame_retries is set.
        if((txOptions.acked_mode) || (txOptions.retransmit_on_cca_fail))
        {
...
static void gpHal_InitMacQueueUsage(void)
{
    gpHal_MacQueueUsage = 0;
}

void gpHal_IncrementMacQueueUsage(void)
{
    gpHal_MacQueueUsage++;
    GP_ASSERT_DEV_INT(gpHal_MacQueueUsage < GP_MM_RAM_PBM_OPTS_NR_OF);
    COMPILE_TIME_ASSERT(GP_MM_RAM_PBM_OPTS_NR_OF < UINT8_MAX);
}

void gpHal_DecrementMacQueueUsage(void)
{
    if(gpHal_MacQueueUsage > 0)
    {
        gpHal_MacQueueUsage--;
    }
}
...
void gpHal_DataRequest_base(UInt8 pbmHandle)
{
    ...
    GP_STAT_SAMPLE_TIME();
    UInt16 optsOffset = ak_rap_rt_to_ptr(GP_HAL_PBM_ENTRY2ADDR_OPT_BASE(pbmHandle));
...
{
    zb_mgr_zb_timed_tx_t TimedTx;
    TimedTx.exec_time = exec_time;
    TimedTx.optsOffset = ak_rap_rt_to_ptr(GP_HAL_PBM_ENTRY2ADDR_OPT_BASE(pbmHandle));
    gpHal_IpcTriggerCommand(GP_WB_ENUM_RT_CMD_TYPE_ZB_TIMED_TX, sizeof(zb_mgr_zb_timed_tx_t), (UInt8 *)&TimedTx);
}
...
gpHal_Result_t gpHal_DataRequest(gpHal_DataReqOptions_t *dataReqOptions, gpPad_Handle_t padHandle, gpPd_Loh_t pdLoh)
{
    ...
    GP_WB_WRITE_ZB_FORMAT_T_ACKED_MODE(optsbase, ackRequest);
    GP_WB_WRITE_ZB_FORMAT_T_FRAME_BUFFER_START(optsbase, GP_HAL_PBM_ENTRY2ADDR_DATA_BASE(pbmHandle));
    GP_WB_WRITE_ZB_FORMAT_T_FRAME_BUFFER_SIZE(optsbase, GP_PD_BUFFER_SIZE_ZIGBEE);
    GP_WB_WRITE_ZB_FORMAT_T_FRAME_OFFSET(optsbase, pdLoh.offset);
    GP_WB_WRITE_ZB_FORMAT_T_FRAME_LEN(optsbase, pdLoh.length);

    //Set all pad settings to a certain PBM
    gpHalMac_FillInTxOptions(pbmHandle, padHandle);

    // See rework in K8x families
    // Writing this setting after gpHalMac_FillInTxOptions is required, otherwise it gets overwritten
    GP_WB_WRITE_ZB_FORMAT_T_CHANNEL_IDX(optsbase, dataReqOptions->srcId);

#ifdef GP_HAL_DIVERSITY_RAW_FRAME_ENCRYPTION
    GP_WB_WRITE_ZB_FORMAT_T_ENC_ENABLE(optsbase, dataReqOptions->rawEncryptionEnable);
    GP_WB_WRITE_ZB_FORMAT_T_ENC_DONE(optsbase, false);
    if(dataReqOptions->rawEncryptionEnable == true)
    {
        ...
        GP_WB_WRITE_ZB_FORMAT_T_AUX_OFFSET(optsbase, dataReqOptions->rawAuxOffset);
        GP_WB_WRITE_ZB_FORMAT_T_ENCR_OFFSET(optsbase, dataReqOptions->rawDataOffset);
        GP_WB_WRITE_ZB_FORMAT_T_ENC_KEEP_FRAMECOUNTER(optsbase, dataReqOptions->rawKeepFrameCounter);
    }
    GP_WB_WRITE_ZB_FORMAT_T_CSL_IE_OFFSET(optsbase, dataReqOptions->rawCslIeOffset);
#else
    GP_WB_WRITE_ZB_FORMAT_T_ENC_ENABLE(optsbase, false);
    GP_WB_WRITE_ZB_FORMAT_T_ENC_DONE(optsbase, false);
    GP_WB_WRITE_ZB_FORMAT_T_CSL_IE_OFFSET(optsbase, 0);
#endif //def GP_HAL_DIVERSITY_RAW_FRAME_ENCRYPTION
    ...
    switch(dataReqOptions->macScenario)
    {
        ...
        {
            gpHalMac_PreparePollRequestPBM(optsbase, padHandle);
            {
            ...
            GP_WB_WRITE_ZB_FORMAT_T_FCS_INSERT_DIS(optsbase, 0);
            break;
        }
        case gpHal_MacManualCrc_NoRetries:
        {
            GP_WB_WRITE_ZB_FORMAT_T_MAX_FRAME_RETRIES(optsbase, 0);
            GP_WB_WRITE_ZB_FORMAT_T_FCS_INSERT_DIS(optsbase, 1);
            break;
        }
    ...
    {
        // We don't suppport retries in Timed-Tx.
        GP_ASSERT_DEV_INT(gpPad_GetTxMaxFrameRetries(padHandle) == 0);
        // Prepare the packet for timed transmission
        gpHal_DataRequest_PrepareTimedTx(pbmHandle);
    }
    else
    {
...
static void gpHalMac_SwitchChannels(UInt8* channelList, Bool forceUpdate)
{
    ...
    for (slot = 0; slot < GP_HAL_MAC_NUMBER_OF_RX_SRCIDS; ++slot)
    {
        if (BIT_TST(changedChannels, slot)) //Change needed
        {
            ...
            {
                //Rx On
                gpHalMac_SetRxOnWhenIdle(slot, channel, true);
                gpHalMac_SetAckTransmitPowers();
            }
            else
            {
                //Rx Off
                gpHalMac_SetRxOnWhenIdle(slot, GP_HAL_MULTICHANNEL_INVALID_CHANNEL, false);
            }
        }
    }
...
void gpHal_SetEnhAckVSIE(UInt8 vsIeLen, UInt8* pVsIeData)
{
    ...
    GP_WB_WRITE_ZBFILT_RAWMODE_VSIE_PTR(ak_rap_rt_to_ptr(new_ptr));
...
void gpHal_SetRxModeConfig(gpHal_RxModeConfig_t* receiverMode2Configure)
{
    ...
    if(receiverMode2Configure->rxMode == gpHal_RxModeMultiStandard)
    {
        ...
        GP_WB_WRITE_AK_PLME_RX_MULTI_STD_MODE(GP_WB_ENUM_AK_MULTI_STD_MODE_ZB_BLE_CONCURRENT);
    }
    else
    {
        // Prioritize BLE
        GP_WB_WRITE_AK_PLME_RX_MULTI_STD_MODE(GP_WB_ENUM_AK_MULTI_STD_MODE_BLE_PRIORITY);
    }
#else
    ...
    // TODO - there are multiple (N)ormal, (Low power) modes, which is the right one to use here
    switch ((UInt8)receiverMode2Configure->rxMode)
    {
        case gpHal_RxModeHighSensitivity:
        {
            rapRxMode = RAP_RX_MODE_N_A0;
            break;
        }
        case gpHal_RxModeLowPower:
        {
            rapRxMode = RAP_RX_MODE_L_A0;
            break;
        }
        case gpHal_RxModeMultiChannel:
        {
            rapRxMode = RAP_RX_MODE_M_A0;
            break;
        }
#ifdef GP_HAL_DIVERSITY_MULTISTANDARD_LISTENING_MODE
        case gpHal_RxModeMultiStandard:
    ...
    if (receiverMode2Configure->antennaMode == gpHal_AntennaModeAntenna1)
    {
        COMPILE_TIME_ASSERT( (RAP_RX_MODE_N_A0+1) == RAP_RX_MODE_N_A1 );
        COMPILE_TIME_ASSERT( (RAP_RX_MODE_L_A0+1) == RAP_RX_MODE_L_A1 );
        COMPILE_TIME_ASSERT( (RAP_RX_MODE_M_A0+1) == RAP_RX_MODE_M_A1 );
        rapRxMode+=1;
    }
    ...
    if(receiverMode2Configure->rxMode != gpHal_RxModeMultiStandard)
    {
        ...
        if(!SA_MODE())
        {
            ...
            {
                COMPILE_TIME_ASSERT( (RAP_RX_MODE_N_A0+3) == RAP_RX_MODE_N_AD );
                COMPILE_TIME_ASSERT( (RAP_RX_MODE_L_A0+3) == RAP_RX_MODE_L_AD );
                COMPILE_TIME_ASSERT( (RAP_RX_MODE_M_A0+3) == RAP_RX_MODE_M_AD );
                rapRxMode+=3;
            }
        }
    }
    ...
    // NRT only sets the ZB Rx mode (note that RT manages the BLE Rx mode)
    ak_rap_rx_zb_set_mode(rapRxMode);
    ak_rap_rx_antenna_set_mode(rapRxMode);
...
Bool gpHal_CheckNoLock(void)
{
    ...
    return GP_WB_READ_AK_SX_UNMASKED_TX_OUT_OF_LOCK_INTERRUPT();
}
...
UInt8 gpHal_GetLastUsedChannel(UInt8 PBMentry)
{
    ...
    if(gpPd_BufferTypeZigBee == gpPd_GetPdType(gpPd_GetPdFromPBM(PBMentry)))
    {
        ...
        gpHal_SourceIdentifier_t srcId = GP_WB_READ_ZB_FORMAT_T_CHANNEL_IDX(pbmOptAddress);
        channel = gpHal_GetRxChannel(srcId);
    }

    return (channel == 0) ? GP_HAL_MULTICHANNEL_INVALID_CHANNEL : channel;
}

Int8 gpHal_GetRadioSensitivity(void)
{
    return gpHal_RxSensitivity;
}
