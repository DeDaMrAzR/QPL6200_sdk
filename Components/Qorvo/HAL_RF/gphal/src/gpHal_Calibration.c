...
qDrvTimerXL_t gpHal_CalibrationTimer;
#endif
...
#ifdef HAL_DIVERSITY_V2
static void gpHal_CalibrationPending(void* arg, qDrvTimerXL_Event_t event)
#else
...
#ifdef HAL_DIVERSITY_V2
    qResult_t res;
    res = qDrvTimerXL_Alloc(&gpHal_CalibrationTimer,qDrvTimerXL_Mode16Bit);
    GP_ASSERT_DEV_INT(res == Q_OK);

    qDrvTimerXL_Config_t cfg = {
        .countMode = qDrvTimerXL_CountModePrescaled,
        .prescaler = CALIBRATION_TIMER_PRESCALER,
    };

    res = qDrvTimerXL_Init(&gpHal_CalibrationTimer, &cfg, gpHal_CalibrationPending, NULL, Q_DRV_IRQ_PRIO_DEFAULT);
    GP_ASSERT_DEV_INT(res == Q_OK);

    res = qDrvTimerXL_OverflowIrqEnable(&gpHal_CalibrationTimer, true);
    GP_ASSERT_DEV_INT(res == Q_OK);
    res = qDrvTimerXL_Start(&gpHal_CalibrationTimer);
    GP_ASSERT_DEV_INT(res == Q_OK);
#else
#ifdef GP_DIVERSITY_GPHAL_XP4002
    /* Configure Timer to interrupt at the requested rate. */
    halTimerXL_initTimer(HAL_CALIBRATION_TIMER, CALIBRATION_TIMER_PRESCALER, HAL_TIMER_CLKSELINTCLK,
                         (1000000 * (64 >> HAL_GET_MCU_CLOCK_SPEED()) / CALIBRATION_TIMER_RATE_HZ /
                          (1 << CALIBRATION_TIMER_PRESCALER) / 2) -
                             1UL,
                         gpHal_CalibrationPending, true);
    halTimerXL_startTimer(HAL_CALIBRATION_TIMER);
#else
    /* Configure Timer to interrupt at the requested rate. */
...
#if defined(GP_COMP_HALCORTEXM4) && (!defined(GP_DIVERSITY_GPHAL_XP4002))
...
#elif defined(GP_DIVERSITY_GPHAL_XP4002)
        ...
        gpHal_cbCalibrationHandler_t cbHandler)
{
    ...
    if (HalCalibration_NrOfTasks == 0)
    {
        ...
        static const qDrvTsens_Callbacks_t cbs = {
            .aboveMaximum = NULL,
            .belowMinimum = NULL,
            .fifoNotEmpty = NULL
        };
    ...
    lastMeasuredTemperature = qDrvTsens_ResultGetBlocking();
#else  // #if defined(GP_DIVERSITY_GPHAL_XP4002)
        lastMeasuredTemperature = 0;
#endif // #if defined(GP_DIVERSITY_GPHAL_XP4002)
...
#if defined(GP_DIVERSITY_GPHAL_XP4002)
        GP_WB_WRITE_APPUC_DISABLE_FCLK_GATING(1);
#else
...
void gpHal_CalibrationSetInterval(UInt8 taskId, UInt32 interval)
{
    UInt32 currentTime;
    HAL_TIMER_GET_CURRENT_TIME_1US(currentTime);

    if(taskId < GP_HAL_CALIBRATION_MAX_TASKS)
    {
        HalCalibration_TaskInfo[taskId].task.calibrationPeriod = interval;
        HalCalibration_TaskInfo[taskId].nextCalibrationTime = currentTime + interval;
    }
}
