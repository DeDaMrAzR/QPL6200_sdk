...
#define GPHAL_BLE_WHITELIST_ID_TO_OFFSET(id)             (GP_HAL_PHYSICAL_TO_ARM(gpHal_BleWl_WhitelistMemory) + GP_HAL_BLE_WHITELIST_ENTRY_SIZE * id)
...
static UInt8
    gpHal_BleWl_WhitelistMemory[GP_HAL_BLE_WHITELIST_ENTRY_SIZE *
                                GP_HAL_BLE_MAX_NR_OF_FILTER_ACCEPT_LIST_ENTRIES] LINKER_SECTION(".rt_fast_ram");

GP_COMPILE_TIME_VERIFY(
    GP_HAL_BLE_MAX_NR_OF_FILTER_ACCEPT_LIST_ENTRIES >= GP_HAL_BLE_MAX_NR_OF_WL_CONTROLLER_SPECIFIC_ENTRIES);

/*****************************************************************************
 *                    Static Function Prototypes
 *****************************************************************************/

static Bool gpHal_IsFilterAcceptListRangeValid(UInt8 rangeStart, UInt8 rangeStop);
static void gpHal_BleAddToWhiteList(UInt8 index, gpHal_FilterAcceptListEntry_t* pEntry);
static void gpHal_RemoveFromFilterAcceptList(UInt8 index, gpHal_FilterAcceptListEntry_t* pEntry);
...
void gpHal_BleWlInit(void)
{
    ...
    GP_HAL_REGISTER_AKRAM_REGION(gpHal_BleWl_WhitelistMemory);

    wlOffset = GPHAL_BLE_RAM_ADDRESS_TO_START_OFFSET(GPHAL_BLE_WHITELIST_ID_TO_OFFSET(0));
    GP_WB_WRITE_BLEFILT_WHITELIST_BASE_ADDRESS((UInt16)wlOffset);
...
gpHal_Result_t gpHal_BleAddDeviceToFilterAcceptList(gpHal_FilterAcceptListEntry_t* pEntry, UInt8 rangeStart, UInt8 rangeStop)
{
    ...
    if(!gpHal_IsFilterAcceptListRangeValid(rangeStart, rangeStop))
    {
...
UInt8 gpHal_BleGetFilterAcceptListEntryIndex(gpHal_FilterAcceptListEntry_t* pEntry, UInt8 rangeStart, UInt8 rangeStop)
{
    ...
    return gpHal_BleFindFilterAcceptListEntry(pEntry->addressType, &pEntry->address, rangeStart, rangeStop);
}
...
gpHal_Result_t gpHal_BleRemoveDeviceFromFilterAcceptList(gpHal_FilterAcceptListEntry_t* pEntry, UInt8 rangeStart, UInt8 rangeStop)
{
    ...
    if(currentEntryIndex != GP_HAL_BLE_WL_ID_INVALID)
    {
        gpHal_RemoveFromFilterAcceptList(currentEntryIndex, pEntry);
        return gpHal_ResultSuccess;
    }
...
void gpHal_BleClearFilterAcceptList(UInt8 rangeStart, UInt8 rangeStop)
{
    ...
    GP_ASSERT_DEV_EXT(gpHal_IsFilterAcceptListRangeValid(rangeStart, rangeStop));
    ...
    for(i = rangeStart; i <= rangeStop; i++)
    {
        if (GPHAL_BLE_WHITELIST_ENTRY_VALID(i))
        {
            ...
            if (GP_HAL_BLE_WL_ID_INVALID == new_idx && gpHal_IsFilterAcceptListRangeValid(rangeStop+1, GP_HAL_BLE_MAX_NR_OF_FILTER_ACCEPT_LIST_ENTRIES-1) )
            {
...
gpHal_Result_t gpHal_BleUpdateFilterAcceptListEntryState(UInt8 addressType, BtDeviceAddress_t* pAddress, UInt8 stateMask_mask, UInt8 stateMask_value, Bool createIfNotExist)
{
    ...
    if(idx != GP_HAL_BLE_WL_ID_INVALID) // entry exists
    {
        ...
        {
            if(regular_cnt < (UInt8)(GP_HAL_BLE_MAX_NR_OF_FILTER_ACCEPT_LIST_ENTRIES -
                                     GP_HAL_BLE_MAX_NR_OF_WL_CONTROLLER_SPECIFIC_ENTRIES))
            {
                GP_LOG_PRINTF("updating entry...", 0);
