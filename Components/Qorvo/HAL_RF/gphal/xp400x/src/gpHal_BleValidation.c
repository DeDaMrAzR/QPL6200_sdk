...
// RTIL passes pointer to access address and assumes this is the start of the validation settings
// If you want to change this, make sure to adapt all occurences of gpHal_BleValidationPopulateParameters
GP_COMPILE_TIME_VERIFY(GP_WB_VALIDATION_RX_ACCESS_ADDRESS_ADDRESS == 0);
...
static void gpHal_BleCalculateAddrAndSignValues(gpHal_BleValidationParameters_t* pValidation, const gpHal_BleValidationInputParameters_t * inParams);
...
static void gpHal_BleValidationWriteParameters(gpHal_BleValidationRx_t* pValidationRx, gpHal_BleValidationParameters_t* pValidationOut);
void gpHal_BleValidationDump(UInt32 accessAddress, gpHal_BleValidationParameters_t* pValidationOut);
...
// Taken from digital team (gen_bt_lp_validation_data in hal.py)
// Ideally, this is a function maintained by digital team
void gpHal_BleCalculateAddrAndSignValues(gpHal_BleValidationParameters_t* pValidation, const gpHal_BleValidationInputParameters_t * inParams)
{
    UInt8 validationN = pValidation->validationStartIndex;
    UInt32 aa = inParams->accessAddress;
    UInt8 phase = 1;
    UInt8 i;
    UInt32 validationString;
    UInt32 tmp;
    UInt8 seq_state;
    UInt8 sequence;
    ...
    if(validationN == 0)
    {
        validationString = !(aa & 0x01) + ((aa & 0x01) << 1) + (!(aa & 0x01) << 2);
    }
    else if(validationN == 1)
    {
        validationString = (aa & 0x01) + (!(aa & 0x01) << 1) + ((aa & 0x01) << 2);
    }
    else if(validationN == 2)
    {
        validationString = !(aa & 0x01) + ((aa & 0x01) << 1) + ((aa & 0x02) << 1);
    }
    else
    {
        validationString = BIT_TST(aa, validationN - 3) + (BIT_TST(aa, validationN - 2) << 1) + (BIT_TST(aa, validationN - 1) << 2);
    }

    tmp = (aa >> pValidation->validationStartIndex) & 0x7F;
    validationString |= (tmp << 3);
    phase = 1;
    seq_state = (validationString >> 1) & 0x03;
    seq_state = ((seq_state & 0x01) << 1) | ((seq_state & 0x02) >> 1);
    sequence = 0x80 | ((validationString >> 3) & 0x7F);

    for(i = 0; i < GPHAL_BLE_NR_OF_ADDR_SIGN_COMBINATIONS + 1; i++)
    {
        UInt8 seq = BIT_TST(sequence, i);

        if(i > 0)
        {
            pValidation->validationAddr[i - 1] = ((seq_state * 2) + seq) + 8 * (phase % 2);
            pValidation->validationAddr[i - 1] = pValidation->validationAddr[i - 1] / 2;
            pValidation->validationSign[i - 1] = phase / 2;
        }

        phase = (phase + (2 * (seq_state/2) - 1));
        phase = phase % 4;
        seq_state = (seq_state % 2) * 2 + seq;
    }
}
...
void gpHal_BleValidationWriteParameters(gpHal_BleValidationRx_t* pValidationRx, gpHal_BleValidationParameters_t* pValidationOut)
{
    gpHal_Address_t validationRxAddress = (gpHal_Address_t)GP_HAL_PHYSICAL_TO_ARM(pValidationRx);
    UInt8 fakePreambleIndex;

    GP_WB_WRITE_VALIDATION_RX_START_IDX(validationRxAddress, pValidationOut->validationStartIndex);
    GP_WB_WRITE_VALIDATION_RX_THRESH(validationRxAddress, pValidationOut->validationThresh);
    GP_WB_WRITE_VALIDATION_RX_FAKE_PREAMBLE_PRESENT(validationRxAddress, pValidationOut->fakePreambleFlag);

    if(pValidationOut->fakePreambleStartIndex == GPHAL_BLE_PREAMBLE_MATCHING_INDEX_INVALID)
    {
        fakePreambleIndex = 0;
    }
    else
    {
        fakePreambleIndex = pValidationOut->fakePreambleStartIndex;
    }

    GP_WB_WRITE_VALIDATION_RX_FAKE_PREAMBLE_START_IDX(validationRxAddress, fakePreambleIndex);

    // FIXME: loop?
    // Write addr and sign bits
    GP_WB_WRITE_VALIDATION_RX_ADDR_0(validationRxAddress, pValidationOut->validationAddr[0]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_0(validationRxAddress, pValidationOut->validationSign[0]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_1(validationRxAddress, pValidationOut->validationAddr[1]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_1(validationRxAddress, pValidationOut->validationSign[1]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_2(validationRxAddress, pValidationOut->validationAddr[2]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_2(validationRxAddress, pValidationOut->validationSign[2]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_3(validationRxAddress, pValidationOut->validationAddr[3]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_3(validationRxAddress, pValidationOut->validationSign[3]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_4(validationRxAddress, pValidationOut->validationAddr[4]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_4(validationRxAddress, pValidationOut->validationSign[4]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_5(validationRxAddress, pValidationOut->validationAddr[5]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_5(validationRxAddress, pValidationOut->validationSign[5]);
    GP_WB_WRITE_VALIDATION_RX_ADDR_6(validationRxAddress, pValidationOut->validationAddr[6]);
    GP_WB_WRITE_VALIDATION_RX_SIGN_6(validationRxAddress, pValidationOut->validationSign[6]);
}

void gpHal_BleValidationDump(UInt32 accessAddress, gpHal_BleValidationParameters_t* pValidationOut)
{
    GP_LOG_SYSTEM_PRINTF("Validation parameters for AA: 0x%lx",0, (long unsigned int)accessAddress);
    GP_LOG_SYSTEM_PRINTF(" - startIdx: %u",0, pValidationOut->validationStartIndex);
    GP_LOG_SYSTEM_PRINTF(" - threshold: %u",0, pValidationOut->validationThresh);
    GP_LOG_SYSTEM_PRINTF(" - fake preamble: %u (%u)",0, pValidationOut->fakePreambleFlag, pValidationOut->fakePreambleStartIndex);
    GP_LOG_SYSTEM_PRINTF("- addr[0] = %u",0, pValidationOut->validationAddr[0]);
    GP_LOG_SYSTEM_PRINTF("- sign[0] = %u",0, pValidationOut->validationSign[0]);
    GP_LOG_SYSTEM_PRINTF("- addr[1] = %u",0, pValidationOut->validationAddr[1]);
    GP_LOG_SYSTEM_PRINTF("- sign[1] = %u",0, pValidationOut->validationSign[1]);
    GP_LOG_SYSTEM_PRINTF("- addr[2] = %u",0, pValidationOut->validationAddr[2]);
    GP_LOG_SYSTEM_PRINTF("- sign[2] = %u",0, pValidationOut->validationSign[2]);
    GP_LOG_SYSTEM_PRINTF("- addr[3] = %u",0, pValidationOut->validationAddr[3]);
    GP_LOG_SYSTEM_PRINTF("- sign[3] = %u",0, pValidationOut->validationSign[3]);
    GP_LOG_SYSTEM_PRINTF("- addr[4] = %u",0, pValidationOut->validationAddr[4]);
    GP_LOG_SYSTEM_PRINTF("- sign[4] = %u",0, pValidationOut->validationSign[4]);
    GP_LOG_SYSTEM_PRINTF("- addr[5] = %u",0, pValidationOut->validationAddr[5]);
    GP_LOG_SYSTEM_PRINTF("- sign[5] = %u",0, pValidationOut->validationSign[5]);
    gpLog_Flush();
}
...
void gpHal_BleGetValidationParameters(gpHal_BleValidationParameters_t* pValidation, const gpHal_BleValidationInputParameters_t *inParams)
{
    ...
    //Next, calculate if a fake preamble is present or not (needs validationStartIndex value)
    gpHal_BleCalculateFakePreambleIndex(pValidation, inParams);

    // As a last step, calculate the validation addr and sign values
    gpHal_BleCalculateAddrAndSignValues(pValidation, inParams);
}

Bool gpHal_BleValidationPopulateParameters(gpHal_BleValidationRx_t* pValidationRx, gpHal_BleRxPhy_t rxPhy)
{
    gpHal_BleValidationInputParameters_t input;
    gpHal_BleValidationParameters_t validationOutput;

    if(rxPhy == gpHal_BleRxPhy1Mb)
    {
        GPHAL_BLE_INIT_VALIDATION_INPUT_NDR(input, pValidationRx->access_address);
    }
    else if(rxPhy == gpHal_BleRxPhy2Mb)
    {
        GPHAL_BLE_INIT_VALIDATION_INPUT_HDR(input, pValidationRx->access_address);
    }
    else
    {
        // Nothing needed for other PHYs, we should not enter here
        GP_ASSERT_DEV_INT(false);
        return false;
    }

    gpHal_BleGetValidationParameters(&validationOutput, &input);

    if(pValidationRx != NULL)
    {
        gpHal_BleValidationWriteParameters(pValidationRx, &validationOutput);
    }

    // gpHal_BleValidationDump(pValidationRx->access_address, &validationOutput);

    return validationOutput.isReliableAccessAddress;
}
