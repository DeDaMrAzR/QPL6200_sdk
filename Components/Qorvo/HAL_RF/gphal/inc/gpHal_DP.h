...
typedef struct {
    UInt16 panId;
    Bool  dataPendingMode;
    UInt8 shortLen;
    UInt8 shortMaxLen;
    UInt8 extLen;
    UInt8 extMaxLen;
} gpHalDP_StackLookupEntry;
...
/** @brief Set the Frame pending mode per stack
 *
 *   @param  enable        The data pending mode to set
 *   @param  id            The stack id for which the data packet pending is set
 *   @return result        The return parameter indicating success or the failure code
 *
 */
gpHal_Result_t gpHal_SetFramePendingAckStack(Bool enable, UInt8 id);

/** @brief Get the Frame pending mode per stack
 *
 *   @param  id            The stack id for which the data packet pending is requested
 *   @return result        A boolean indicating if the stack has pending data packets in the list.
 *
 */
