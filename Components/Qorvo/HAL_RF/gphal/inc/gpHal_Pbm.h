 ...
 *
 *  @param  PBMentry The index of the PBM buffer to transmit
 ...
 *
 *  @param address The base address of the PBM where the data should be read from.  This value should be obtained
...
/** @brief Query the RSSI value of the ACK packet related to the specified TX pbm entry.
 *
 * This function returns the RSSI of the received ACK packet, the passed pbm entry needs to be for a TX packet.
 * @param PBMentry the TX pbm entry containing the requested data.
 * @note if transmission is not in ACKED MODE or the ACK is not received, 0 is returned
 */
GP_API Int8 gpHal_GetTxAckRSSI(UInt8 PBMentry);
 ...
 * @param PBMentry  the pbm entry containing the requested data.
 * @note retry counter 0 means - first time successfully transmitted
 */
...
/** @brief Function for writing one byte to a PBM buffer. The destination is the data segment of the PBM. This segment
 * is handled as a cyclic buffer.
 *
...
/** @brief Function for writing one byte to a PBM buffer. The destination is the data segment of the PBM.
 *
 *  This function writes one byte to a PBM buffer defined by the parameter address.
 *  This function should be used in order to update the payload of a PBM buffer.
 *
