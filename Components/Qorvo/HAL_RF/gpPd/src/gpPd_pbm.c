...
static void Pd_SetTxAckLqi(gpPd_Handle_t pdHandle, gpPd_Lqi_t lqi);
static gpPd_Rssi_t Pd_GetTxAckRssi(gpPd_Handle_t pdHandle);
static void Pd_SetTxAckRssi(gpPd_Handle_t pdHandle, gpPd_Rssi_t rssi);
...
static gpPd_Rssi_t Pd_GetTxAckRssi(gpPd_Handle_t pdHandle)
{
    return gpHal_GetTxAckRSSI(pdHandle);
}
...
{
    if(Pd_GetFrameControlFromTxAckAfterRx(pdHandle) != 0)
    {
...
static gpPd_Handle_t Pd_CopyPd(gpPd_Handle_t pdHandle)
{
    ...
    {
        MEMCPY(&gpPd_Descriptors[pdHandleCopy].attr, &gpPd_Descriptors[pdHandle].attr,
               sizeof(gpPd_Descriptors[pdHandle].attr));
...
static void Pd_DataIndication(UInt8 pbmHandle, gpPd_Offset_t pbmOffset, UInt16 pbmLength, gpPd_Loh_t* p_PdLoh,
