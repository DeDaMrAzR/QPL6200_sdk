...
#define GP_TEST_NR_OF_CHANNELS              16

#define VALID_SLEEP_MODE                    true
#define INVALID_SLEEP_MODE                  false
#define CONVERT_CHANNEL_TO_IDX(channel)     (channel - 11)

#define GP_TEST_MAX_NB_OF_MAC_RETRIES ((UInt8)15)

#ifndef GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
...
gpTest_Statistics_t gpTest_Statistics[IEEE_802_15_4_TOTAL_CHANNELS];

gpTest_AntennaStatistics_t gpTest_AntennaStatistics[GP_TEST_NUMBER_OF_ANTENNAS];
...
static Int8 pDefaultTxPowers[GP_TEST_NR_OF_CHANNELS];
...
void gpTest_MacInit(void)
{
    ...
    padInitAttributes.ccaFailureAsNoAck = 0;
...
void gpTest_MacStart(void)
{
    ...
    gpTest_SetShortAddress(0xFFFF, gpHal_SourceIdentifier_1);
    gpTest_SetShortAddress(0xFFFF, gpHal_SourceIdentifier_2);
    gpTest_SetPanId(0xCAFE, gpHal_SourceIdentifier_0);
    gpTest_SetPanId(0xCAFE, gpHal_SourceIdentifier_1);
    gpTest_SetPanId(0xCAFE, gpHal_SourceIdentifier_2);
    // Enable interrupts
...
gpTest_Result_t gpTest_MacSetNumberOfRetries(UInt8 retries)
{
    if(retries < GP_TEST_MAX_NB_OF_MAC_RETRIES)
    {
...
gpTest_Result_t gpTest_MacSetTxPowerChannel(gpHal_Channel_t channel, Int8 transmitPower)
{
    UIntLoop chIdx;
    ...
    if(GPHAL_MAX_TRANSMIT_POWER < transmitPower || GPHAL_MIN_TRANSMIT_POWER > transmitPower)
    {
        return gpHal_ResultInvalidParameter;
    }

    chIdx = CONVERT_CHANNEL_TO_IDX(channel);
    ...
    for(stackId = 0; stackId < GP_DIVERSITY_NR_OF_STACKS; stackId++)
    {
        ...
        gpMacCore_TxPower_t pMaxTransmitPowerTable[GP_TEST_NR_OF_CHANNELS];

        gpMacCore_GetRegionalDomainSettings(&BlockedChannelMask, pMaxTransmitPowerTable, stackId);
        if(pMaxTransmitPowerTable[chIdx] < transmitPower)
        {
            return gpHal_ResultInvalidParameter;
        }
    }
    ...
    pDefaultTxPowers[chIdx] = transmitPower;
    // When configured as default the silicon will look up the Tx power for the according channel in its default Tx
    // Power list
    gpHal_SetDefaultTransmitPowers(pDefaultTxPowers);
    // When configured as default the silicon will look up the Tx power for the according channel in its default Tx
    // Power list
    gpPad_SetTxPower(gpTest_PadHandle, gpHal_DefaultTransmitPower);

    return gpHal_ResultSuccess;
}

gpTest_Result_t gpTest_MacGetTxPowerChannel(gpHal_Channel_t channel, Int8* pTransmitPower)
{
    if(!GP_MACCORE_CHECK_CHANNEL_VALID(channel) || pTransmitPower == NULL)
    {
        return gpHal_ResultInvalidParameter;
    }

    *pTransmitPower = (Int8)gpHal_GetDefaultTransmitPower(channel);
    if(*pTransmitPower == GPHAL_DEFAULT_TRANSMIT_POWER)
    {
        *pTransmitPower = 0x7f;
    }
    return gpHal_ResultSuccess;
...
UInt8 gpTest_MacGetAverageLQI(void)
{
    ...
    UInt8 lqi;
    UInt16 sumLQI = 0;
    for(chIdx = 0; chIdx < IEEE_802_15_4_TOTAL_CHANNELS; chIdx++)
    {
        gpTest_MacGetLQIForChannel(chIdx + 11, &lqi);
        sumLQI += lqi;
    }
    return sumLQI / IEEE_802_15_4_TOTAL_CHANNELS;
}

gpTest_Result_t gpTest_MacGetLQIForChannel(UInt8 channel, UInt8* pLqi)
{
    if(!GP_MACCORE_CHECK_CHANNEL_VALID(channel) || pLqi == NULL)
    {
        return gpHal_ResultInvalidParameter;
    }
    if(gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].ReceivedPacketCounterD != 0)
    {
        *pLqi = ((UInt8)(gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].CumulativeLQI /
                         gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].ReceivedPacketCounterD));
    }
    else
    {
        return *pLqi = 0;
...
Int8 gpTest_MacGetAverageRSSI(void)
{
    ...
    for(chIdx = 0; chIdx < IEEE_802_15_4_TOTAL_CHANNELS; chIdx++)
    {
        gpTest_MacGetRSSIForChannel(chIdx + 11, &rssi);
        sumRSSI += rssi;
    }
    return sumRSSI / IEEE_802_15_4_TOTAL_CHANNELS;
}

gpTest_Result_t gpTest_MacGetRSSIForChannel(UInt8 channel, Int8* pRssi)
{
    if(!GP_MACCORE_CHECK_CHANNEL_VALID(channel) || pRssi == NULL)
    {
        return gpHal_ResultInvalidParameter;
    }
    if(gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].ReceivedPacketCounterD != 0)
    {
        *pRssi = ((Int8)(gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].CumulativeRssi /
                         gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)].ReceivedPacketCounterD));
    }
    else
    {
...
void gpTestHal_cbMacDataConfirm(UInt8 status, gpPd_Loh_t pdLoh, UInt8 lastChannelUsed)
{
    ...
    {
        gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(lastChannelUsed)].PacketsSentOK++;
        gpTest_TxPacketsOK++;
    }
    else
    {
        gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(lastChannelUsed)].PacketsSentError++;
...
void gpTestHal_cbMacDataIndication(gpPd_Loh_t pdLoh, gpHal_RxInfo_t *rxInfo)
{
    ...
    UInt8 length;
#ifndef GP_HAL_DIVERSITY_READ_RECEIVING_ANTENNA
    NOT_USED(rxInfo);
#endif
    ...
    gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(rxInfo->rxChannel)].ReceivedPacketCounterC++;
#ifndef GP_TEST_DIVERSITY_DISABLE_EXPECTED_RX_PACKET_CHECK
    if(pdLoh.length == MacRxExpectedPacket_dataLength)
#endif
    {
        gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(rxInfo->rxChannel)].ReceivedPacketCounterL++;
        ...
        { //always
...
#endif
            {
                ...
                gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(rxInfo->rxChannel)].ReceivedPacketCounterD++;

                Lqi = gpPd_GetLqi(pdLoh.handle);
                Rssi = gpPd_GetRssi(pdLoh.handle);
                Channel = gpPd_GetRxChannel(pdLoh.handle);

                gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(Channel)].CumulativeLQI += Lqi;
                gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(Channel)].CumulativeRssi += Rssi;
#ifdef GP_HAL_DIVERSITY_READ_RECEIVING_ANTENNA
                if (rxInfo->rxReceivingAntenna == 0)
                {
                    gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(Channel)].RxAntenna0++;
                }

                gpTest_AntennaStatistics[rxInfo->rxReceivingAntenna].ReceivedPacketCounter++;
                gpTest_AntennaStatistics[rxInfo->rxReceivingAntenna].CumulativeRssi += Rssi;
#endif
            }
#ifndef GP_TEST_DIVERSITY_DISABLE_EXPECTED_RX_PACKET_CHECK
            else if (gpTest_SendOptions & GP_TEST_TXOPTIONS_INCREMENTINGCTR)
...
void Test_SendData(void)
{
    ...
    {
        gpTest_cbDataConfirm(gpTestHalResult,
                             gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(gpTest_CurrentTxChannel)].PacketsSentOK,
                             gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(gpTest_CurrentTxChannel)].PacketsSentError);
    }
    gpTest_macScenario = gpHal_MacDefault;
...
{
    UInt8 chIdx;
    gpTest_Statistics_t sumStat = {0};

    for(chIdx = 0; chIdx < IEEE_802_15_4_TOTAL_CHANNELS; chIdx++)
    {
        sumStat.ReceivedPacketCounterC += gpTest_Statistics[chIdx].ReceivedPacketCounterC;
        sumStat.ReceivedPacketCounterL += gpTest_Statistics[chIdx].ReceivedPacketCounterL;
        sumStat.ReceivedPacketCounterD += gpTest_Statistics[chIdx].ReceivedPacketCounterD;
        sumStat.PacketsSentOK += gpTest_Statistics[chIdx].PacketsSentOK;
        sumStat.PacketsSentError += gpTest_Statistics[chIdx].PacketsSentError;
        sumStat.CumulativeRssi += gpTest_Statistics[chIdx].CumulativeRssi;
        sumStat.CumulativeLQI += gpTest_Statistics[chIdx].CumulativeLQI;
        sumStat.RxAntenna0 += gpTest_Statistics[chIdx].RxAntenna0;
    }
    sumStat.CumulativeRssi /= sumStat.ReceivedPacketCounterD;
    sumStat.CumulativeLQI /= sumStat.ReceivedPacketCounterD;
    MEMCPY(Statistics, &sumStat, sizeof(gpTest_Statistics_t));
}

gpTest_Result_t gpTest_MacGetStatisticsForChannel(gpTest_Statistics_t* Statistics, UInt8 channel)
{
    if(!GP_MACCORE_CHECK_CHANNEL_VALID(channel) || Statistics == NULL)
    {
        return gpHal_ResultInvalidParameter;
    }
    MEMCPY(Statistics, &gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)], sizeof(gpTest_Statistics_t));

    return gpHal_ResultSuccess;
}

void gpTest_MacResetStatistics(void) { MEMSET(&gpTest_Statistics, 0, sizeof(gpTest_Statistics)); }

gpTest_Result_t gpTest_MacResetStatisticsForChannel(UInt8 channel)
{
    if(!GP_MACCORE_CHECK_CHANNEL_VALID(channel))
    {
        return gpHal_ResultInvalidParameter;
    }
    MEMSET(&gpTest_Statistics[CONVERT_CHANNEL_TO_IDX(channel)], 0, sizeof(gpTest_Statistics_t));

    return gpHal_ResultSuccess;
}

gpTest_Result_t gpTest_MacGetAntennaStatistics(gpTest_AntennaStatistics_t* ant0Statistics,
                                               gpTest_AntennaStatistics_t* ant1Statistics)
{
    if(NULL == ant0Statistics || NULL == ant1Statistics)
    {
        return gpHal_ResultInvalidParameter;
    }

    MEMCPY(ant0Statistics, &gpTest_AntennaStatistics[0], sizeof(gpTest_AntennaStatistics_t));
    MEMCPY(ant1Statistics, &gpTest_AntennaStatistics[1], sizeof(gpTest_AntennaStatistics_t));

    return gpHal_ResultSuccess;
}

void gpTest_MacResetAntennaStatistics(void) { MEMSET(&gpTest_AntennaStatistics, 0, sizeof(gpTest_AntennaStatistics)); }
