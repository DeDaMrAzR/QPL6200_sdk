...
#define GP_TEST_NUMBER_OF_ANTENNAS 2
...
typedef struct gpTest_AntennaStatistics_s {
    /** Received packet counter */
    UInt16 ReceivedPacketCounter;
    /** Cumulative rssi from indications with matching data **/
    Int32 CumulativeRssi;
} gpTest_AntennaStatistics_t;
 ...
 *  Possible results are:
 *  - gpTest_ResultSuccess
 *  - gpTest_ResultInvalidParameter (not in MAC retries range)
 */
gpTest_Result_t gpTest_SetNumberOfRetries(UInt8 numRetries);
...
/** @brief Sets the transmit power on channel.
 *
 *  This function sets the transmit power on channel.
 *
 *  @param channel (gpHal_Channel_t) The channel setting
 *  @param transmitPower (Int8) The transmitpower setting (in dBm -70..3dBm).
 *
 *  @return result (gpTest_Result_t)
 *  Possible results are:
...
/** @brief Gets the transmit power on channel.
 *
 *  This function gets the transmit power on channel.
 *
 *  @param channel (gpHal_Channel_t) The channel setting
 *  @param transmitPower (Int8*) The pointer to returned transmitpower value (in dBm -70..3dBm).
 *
 *  @return result (gpTest_Result_t)
 *  Possible results are:
 *  - gpTest_ResultSuccess
 *  - gpTest_ResultInvalidParameter (not in correct range)
 */
gpTest_Result_t gpTest_GetTxPowerChannel(gpHal_Channel_t channel, Int8* transmitPower);
 ...
 *  * Possible values:
 *  - gpHal_ResultSuccess
 *  - gpHal_ResultInvalidParameter: An invalid combination of parameters was applied.
 */
gpTest_Result_t gpTest_GetStatisticsForChannel(gpTest_Statistics_t* Statistics, UInt8 channel);

/** @brief This function returns the current antennas statistics.
 *
 *  This function returns the current antennas statistics.
 *
 *  @param ant0Statistics           Pointer to a antenna 0 statistics.
 *  @param ant1Statistics           Pointer to a antenna 1 statistics.
 *
 *  @return void
 */
void gpTest_GetAntennaStatistics(gpTest_AntennaStatistics_t* ant0Statistics,
                                 gpTest_AntennaStatistics_t* ant1Statistics);

/** @brief This function resets the gpTest antenna statistics.
 *
 *  @return void
 */
...
/** @brief This function returns the Link Quality Indication on specific channel.
 *
 *  When frames are received that match the data in the buffer pointed to by gpTest_SavedDataPointer,
 *  the LQI for that frame is obtained and added to a running total value.
 *  When queried, an LQI on specific channel is returned, which is calculated by dividing the running total by
 *  the number of received matching frames.
 *
 *  @param channel                Channel number. Possible values are: 11 ... 26
 *  @param pLqi (UInt8*)          The pointer to returned LQI value. Possible values are: 0 ... 0x7f
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
...
/** @brief This function returns the Received Signal Strength Indication on specific channel.
 *
 *  When frames are received that match the data in the buffer pointed to by gpTest_SavedDataPointer,
 *  the RSSI for that frame is obtained and added to a running total value.
 *  When queried, an RSSI on specific channel is returned, which is calculated by dividing the running total
 *  by the number of received matching frames.
 *
 *  @param channel                channel number (11 - 26).
 *  @param pRssi (Int8*)          The pointer to returned RSSI value.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
...
extern void gpTest_SetTxModulation(gpTest_BleTxPhy_t modulation);
extern gpTest_Result_t gpTest_SetRxPhyMask(UInt8 rxPhyMask);
...
/** @brief This function reads the different RX modes. All parameters are expected to be non-NULL.
 *
 *  @param enableMultiStandard         Allows concurrent listening on ZigBee and BLE channels (not compatible with the
 * other two options).
 *  @param enableMultiChannel          Allows listening to multiple ZigBee channels simultaneously (not compatible with
 * the other two options)
 *  @param enableHighSensitivity       Allows for higher sensitivity ZigBee reception (not compatible with the other two
 * options)
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 ...
 *
 * @param enable (Bool). Possible values:
 * - true     Enable the AGC (again).
 * - false    Disable the AGC.
 *
 * @return void
 */
void gpTest_EnableAgc(Bool enable);

/** @brief This function returns status of AGC enable
 *
 * @param void
 * @return Bool
 */
...
/**
 *
 * Map the 32 kHz clock to the selected gpio.
 * Allowed gpio pins on xp4001: 10 up until 15
 *
 * @param gpio      Output pin to receive the selected clock.
 * @param clockType Select one of the available clockTypes to be output
 * * Possible vlaues:
 *  - 0:     64 kHz RC clock divided down to 32 khz
 *  - 1:     32 kHz Xtal osc
 *  - 2:     32MHz xtal clock divided down to 32 KHz
 *  - 0xFF:  To disable clock output again.
 *
 * Warning: this function enables or disables the 32 MHz sleep clock, according to the selected clocktype.
 **/
 ...
 * Can be used to revert the state afterwards to the original state.
 *
 * @param gpio                Output pin for which the setting must be retrieved
 * @param *alternateEnabled   Reference to the state being retrieved. Possible values:
 * - true      Alternate function was enabled on the respective gpio.
 * - false     Alternate function was disabled on the respective gpio.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *
 * @param gpio                Output pin for which the setting must be configured.
 * @param alternateEnabled    Possible values:
 * - true     Enable the alternate function.
 * - false    Disable the alternate function.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *  Retrieve the current alternate selection for a certain gpio.
 *  Can be used to revert afterwards to the original state.
 *
 *  @param gpio                Output pin for which the setting must be retrieved
 *  @param *alternateSelection Reference to the state being retrieved.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
...
/**
 *
 *  Configure the Alternate Selection for a certain gpio.
 *
 *  @param gpio                Output pin for which the setting must be configured.
 *  @param alternateSelection  Value to set.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *
 *  @param gpio              Output pin for which the current debug function is retrieved.
 *  @param *debugSelection   Reference to the ID of the current debug function.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *
 *  @param gpio              Output pin for which to configure the function.
 *  @param debugSelection    Value of the ID for the debug function to configure.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
...
/**
 *
 *  Check whether the application is in the Low Temperature mode
 * *
 *  @return Bool
 *  * Possible values:
 *  - true if in Low Temperature mode
 *  - false otherwise
 *
 **/
...
/**
 *
 *  Get the zone max temperature of the Xtal capacitor temperature compensation.
 *
 *  @param zoneId               The zone identificator (starting from 1).
 *  @param pOutMaxTemp          Pointer to the max temperature.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *
 **/
gpHal_Result_t gpTest_Xtal32MHzTrimGetZoneMaxTemp(UInt8 zoneId, Int8* pOutMaxTemp);

/**
 *
 *  Get the zone setting of the Xtal capacitor temperature compensation.
 *
 *  @param zoneId               The zone identificator (starting from 1).
 *  @param pOutSetting          Pointer to the setting.
 *
 *  @return gpTest_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *
 **/
gpHal_Result_t gpTest_Xtal32MHzTrimGetZoneSetting(UInt8 zoneId, UInt8* pOutSetting);

/**
 *
 *  Configure the XTAL32M capacitor trimming algorithm.
 *
 *  @param pInConfig            Pointer to the struct with the zones configuration.
 *
...
/**
 *
 *  Get size of load capacitor settings table.
 *
 *  @return size of load capacitor coefficient value table
 *
 **/
...
/**
 *
 *  Get load capacitor settings of the silicon.
 *
 *  @param tempIndex            Temperature coefficient index.
 *  @return load capacitor coefficient value
 *
 **/
 ...
 *  Set DCDC mode.
 *
 *  @param dcdcOn                 DCDC mode enable flag
 *
 *  @return gpHal_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 *  - gpHal_ResultInvalidParameter
 *  - gpHal_ResultCCAFailure
 *
 **/

/**
 *
 * Get trim cap values for 32 MHz clock.
 *  @param *valueP      Pointer to trim cap P
 *  @param *valueN      Pointer to trim cap N
 *
 *  @return gpHal_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
 ...
 *  Get DCDC mode.
 *
 *  @param pStandbyPowerMode     Pointer to the standby power mode.
 *  @param pActivePowerMode      Pointer to the active power mode
 *  @param pSleepMode            Pointer to the sleep mode
 *
 *  @return gpHal_Result_t
 *  * Possible values:
 *  - gpHal_ResultSuccess
