...
#include "gpLog.h"
    ...
    ((!defined(GP_DIVERSITY_BOOTLOADER) || defined(GP_DIVERSITY_BOOTLOADER_BUILD)) ||                                  \
     defined(GP_DIVERSITY_APPUC_FW_HEADER))
#include "q_firmware_header.h"
#elif defined(GP_COMP_HALCORTEXM4) 
...
#if !(defined(GP_DIVERSITY_GPHAL_XP4002) && !defined(GP_DIVERSITY_BOOTLOADER))
...
{
#if defined(GP_DIVERSITY_GPHAL_XP4002) && !defined(GP_DIVERSITY_BOOTLOADER)
    const gpVersion_SoftwareInfo_t* sw_info = get_sw_info();

    MEMCPY_P(swVersion, &sw_info->version, sizeof(gpVersion_ReleaseInfo_t));

#elif defined(GP_COMP_HALCORTEXM4) 
#if defined(GP_DIVERSITY_APPUC_FW_HEADER)
    const appuc_fw_header_t* header = hal_get_appuc_header();
    MEMCPY_P(swVersion, &(header->appuc_fw_version_info.app_version), sizeof(gpVersion_ReleaseInfo_t));
...
{
#if defined(GP_DIVERSITY_GPHAL_XP4002) && !defined(GP_DIVERSITY_BOOTLOADER)
    const gpVersion_SoftwareInfo_t* info = get_sw_info();

    MEMCPY_P(swInfo, info, sizeof(gpVersion_SoftwareInfo_t));

#elif defined(GP_COMP_HALCORTEXM4) 

#if defined(GP_DIVERSITY_APPUC_FW_HEADER)
    const appuc_fw_header_t* header = hal_get_appuc_header();
    MEMCPY_P(swInfo, &(header->appuc_fw_version_info), sizeof(gpVersion_SoftwareInfo_t));
#elif defined(GP_DIVERSITY_LOADED_USER_LICENSE)
    const loaded_userlicense_t* license = hal_get_loaded_user_license();
