...
#if defined(GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS) && GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0
static gpSched_PreProcessIdleCallback_t sched_PreProcessOnIdleCallbacks[GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS]; /**<
                                                                                                Array with functions to
                                                                                                call when the scheduler
                                                                                                is on wakeup from sleep
                                                                                              */
#endif /* GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0 */
...
#if defined(GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS) && GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0
void gpSched_RegisterPreProcessOnIdleCallback(gpSched_PreProcessIdleCallback_t callback)
{
    UInt32 i;

    GP_ASSERT_DEV_EXT(callback != NULL);

    for(i = 0; i < GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS; ++i)
    {
        if(sched_PreProcessOnIdleCallbacks[i] == NULL)
        {
            sched_PreProcessOnIdleCallbacks[i] = callback;
            break;
        }
    }
}

void gpSched_PreProcessIdle(void)
{
    UInt32 i;

    // Call all registered external pre-process callbacks
    /* Note:
     * The scheduler is not checked between scheduling of the external CBs
     * to fairly divide idle time between all registered CBs
     */
    for(i = 0; i < GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS; ++i)
    {
        if(sched_PreProcessOnIdleCallbacks[i])
        {
            sched_PreProcessOnIdleCallbacks[i]();
        }
    }
}
#endif /* GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0 */
...
#if defined(GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS) && GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0
    MEMSET(sched_PreProcessOnIdleCallbacks, 0, sizeof(sched_PreProcessOnIdleCallbacks));
#endif /* GP_SCHED_NR_OF_PREPROCESS_IDLE_CALLBACKS > 0 */
