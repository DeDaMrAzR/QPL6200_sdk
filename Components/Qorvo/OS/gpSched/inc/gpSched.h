...
typedef void (*gpSched_PreProcessIdleCallback_t)(void);

/**
 * @brief Register a function that should be called when nothing is scheduled.
 *        This is also referred to as pre processing.
 *        The callback should be:
 *        - non-blocking
 *        - Use the scheduler for deffered work (using gpSched_ScheduleEvent)
 *
 * @param  callback   Function that will be triggered when the scheduler(OS) is idle.
 */
GP_API void gpSched_RegisterPreProcessOnIdleCallback(gpSched_PreProcessIdleCallback_t callback);
/**
 * @brief Execute all registered pre-process on idle callback functions
 *
 */
GP_API void gpSched_PreProcessIdle(void);
