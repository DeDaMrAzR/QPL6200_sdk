 ...
 *
 * gpMacCore.h
 *   This file contains the definitions of the public functions and enumerations of the gpMacCore.(based on
 *   IEEE802.15.4-2006)
 *
 *
 ...
 *
 * To transmit a data package the originator needs to:
 ...
 *
 * The receiving device should handle an incoming packet as follows:
...
// phy definitions
/** @brief The minimal value that the header overhead can be in a frame (2 bytes framecontrol + 1 byte sequence number +
 *  2 bytes FCS), as specified in the IEEE802.15.4-2006 spec. */
#define GP_MACCORE_MIN_FRAME_OVERHEAD 5 /* 2 framecontrol + 1 sequence number + 2 FCS */
/** @brief The maximal number of bytes the header overhead can be in a beacon, as specified in the IEEE802.15.4-2006
 *  spec. */
...
/** @brief The maximal PHY length of a packet (in bytes), as specified in the IEEE802.15.4-2006 spec without the CRC
 *  bytes. */
...
/** @brief The maximum number of octets added by the MAC sublayer to the PSDU without security, as specified in the IEEE
 *  802.15.4 spec */
#define GP_MACCORE_MAX_MPDU_UNSECURED_OVERHEAD 25 // 23 bytes MAC Header + 2 bytes MAC footer
/** @brief The maximum number of octets that can be transmitted in the MAC Payload field of an unsecured MAC frame that
 *  will be guaranteed not to exceed aMaxPHYPacketSize, as specified in the IEEE 802.15.4 spec */
 ...
 * As it's defined in symbols, 960 needs to be multiplied by 16us to get time in us = 15360us */
 ...
 * A scan duration of 4 will give +- 260ms. */
...
/** @brief Maximum security header byte size, consisting of security control (1), frame contorl (4) and keyIdentifier
 *  (0/1/5/9). */
...
#define GP_MACCORE_MAX_MAC_SECURED_HEADER_SIZE                                                                         \
 ...
 * As specified in IEEE802.15.4-2015 paragraph 7.3.3 Acknowledgment frame format. The
 * current max value is the value with:
 * - Extended Dest addressing, PanID compression, Aux header,
 * - One Thread CSL IE, VS IE and MIC.
 */
#define GP_MACCORE_MAX_PHY_ENHACK_PACKET_SIZE 37

/** @brief Max duration in us for transmitting a frame and receiving the corresponding ACK, without the CSMA time */
#define GP_MACCORE_MAX_TXFRAME_PLUS_RXACK_TIME(ack_size)                                                               \
    ((GP_MACCORE_RX_TO_TX_TURNAROUNDTIME * GP_MACCORE_SYMBOL_DURATION) +                                               \
     ((GP_MACCORE_PHYHEADER_OCTETS + GP_MACCORE_MAX_PHY_PACKET_SIZE) * 8 / GP_MACCORE_PHY_BITS_PER_SYMBOL *            \
      GP_MACCORE_SYMBOL_DURATION) +                                                                                    \
     (GP_MACCORE_TX_TO_RX_TURNAROUNDTIME * GP_MACCORE_SYMBOL_DURATION) +                                               \
     ((GP_MACCORE_PHYHEADER_OCTETS + ack_size) * 8 / GP_MACCORE_PHY_BITS_PER_SYMBOL * GP_MACCORE_SYMBOL_DURATION))

#define GP_MACCORE_MAX_TXFRAME_PLUS_RXNORMALACK_TIME                                                                   \
    GP_MACCORE_MAX_TXFRAME_PLUS_RXACK_TIME(GP_MACCORE_MAX_PHY_ACK_PACKET_SIZE)
#define GP_MACCORE_MAX_TXFRAME_PLUS_RXENHACK_TIME                                                                      \
    GP_MACCORE_MAX_TXFRAME_PLUS_RXACK_TIME(GP_MACCORE_MAX_PHY_ENHACK_PACKET_SIZE)
...
/** @brief Send packet as an indirect transmission. Packet will be buffered until polled successfully or expired. */
...
#define GP_MACCORE_NO_SECURITY_SPECIFIED(pSecOptions)                                                                  \
    ((pSecOptions) == NULL || (pSecOptions)->securityLevel == gpEncryption_SecLevelNothing)
...
// regular check macro's (to be used anywhere, cannot be omitted during compilation)
#define GP_MACCORE_CHECK_ADDRESSMODE_NONE(mode) ((mode) != gpMacCore_AddressModeNoAddress)
#define GP_MACCORE_CHECK_ADDRESSMODE_VALID(mode)                                                                       \
    ((mode) != gpMacCore_AddressModeReserved && (mode) <= gpMacCore_AddressModeExtendedAddress)
#define GP_MACCORE_CHECK_ADDRESSINFO_VALID(info)                                                                       \
    ((info) != NULL && GP_MACCORE_CHECK_ADDRESSMODE_VALID((info)->addressMode))
#define GP_MACCORE_CHECK_ADDRESSMODE_PRESENT(mode)                                                                     \
    ((mode) == gpMacCore_AddressModeShortAddress || (mode) == gpMacCore_AddressModeExtendedAddress)
#define GP_MACCORE_CHECK_SCANTYPE_VALID(type)                                                                          \
    (((type) != gpMacCore_ScanTypePassive && (type) <= gpMacCore_ScanTypeOrphan) ||                                    \
     ((type) >= gpMacCore_ScanTypeRSSI && (type) <= gpMacCore_ScanTypeAIMasked))
...
// special check-if macro's that return true when check is false (to be used inside if statement, can be omitted when
// assert level does not match)
...
#define GP_MACCORE_CHECK_IF_ADDRESSMODE_NOT_PRESENT(mode)                                                              \
    (GP_MACCORE_CHECK_IF(GP_MACCORE_CHECK_ADDRESSMODE_PRESENT(mode)))
#define GP_MACCORE_CHECK_IF_SCANTYPE_INVALID(type) (GP_MACCORE_CHECK_IF(GP_MACCORE_CHECK_SCANTYPE_VALID(type)))
#define GP_MACCORE_CHECK_IF_SCANDURATION_INVALID(duration)                                                             \
    (GP_MACCORE_CHECK_IF(GP_MACCORE_CHECK_SCANDURATION_VALID(duration)))
...
//@{
// PHY results
/** @brief The requested operation was completed successfully. For instance if a transmission was requested, this value
 *  indicates a successful transmission. */
#define gpMacCore_ResultSuccess 0x0
// Overall results
/** @brief The requested operation failed for a generic reason. */
#define gpMacCore_ResultFailed 0xc0
// MAC results
...
/** @brief The key purportedly applied by the originator of the received frame is not allowed to be used with that frame
 *  type according to the key usage policy of the recipient. */
#define gpMacCore_ResultImproperKeyType 0xdc
/** @brief The security level purportedly applied by the originator of the received frame does not meet the minimum
 *  security level required/expected by the recipient for that frame type. */
#define gpMacCore_ResultImproperSecurityLevel 0xdd
/** @brief The received frame was purportedly secured using security based on IEEE Std 802.15.4-2003, and such security
...
/** @brief The requested GTS transmission failed because the specified GTS either did not have a transmit GTS direction
 *  or was not defined. */
#define gpMacCore_ResultInvalidGTS 0xe6
/** @brief A request to purge an MSDU from the transaction queue was made using an MSDU handle that was not found in the
 *  transaction table. */
...
/** @brief A request to send data was unsuccessful because neither the source address parameters nor the destination
 *  address parameters were present. */
#define gpMacCore_ResultInvalidAddress 0xf5
/** @brief A receiver enable request was unsuccessful because it specified a number of symbols that was longer than the
 *  beacon interval. */
#define gpMacCore_ResultOnTimeTooLong 0xf6
/** @brief A receiver enable request was unsuccessful because it could not be completed within the current superframe
 *  and was not permitted to be deferred until the next superframe. */
#define gpMacCore_ResultPastTime 0xf7
/** @brief The device was instructed to start sending beacons based on the timing of the beacon transmissions of its
 *  coordinator, but the device is not currently tracking the beacon of its coordinator. */
#define gpMacCore_ResultTrackingOff 0xf8
/** @brief An attempt to write to a MAC PIB attribute that is in a table failed because the specified table index was
 *  out of range. */
#define gpMacCore_ResultInvalidIndex 0xf9
/** @brief A scan operation terminated prematurely because the number of PAN descriptors stored reached an
...
/** @brief A request to perform a scan operation failed because the MLME was in the process of performing a previously
 *  initiated scan operation. */
#define gpMacCore_ResultScanInProgress 0xfc
/** @brief The device was instructed to start sending beacons based on the timing of the beacon transmissions of its
 *  coordinator, but the instructed start time overlapped the transmission time of the beacon of its coordinator. */
#define gpMacCore_ResultSuperframeOverlap 0xfd
/** @typedef gpMacCore_Result_t
 * @brief The gpMacCore_Result_t type defines the result as defined by the IEEE802.15.4 MAC. Not all the  possible
 * values are used by the non-beaconed MAC.
 */
...
#define gpMacCore_AttributeCurrentChannel 0x0 /* The RF channel to be used for all following tx and rx */
/** @brief The 5 most significant bits (MSBs) (b27,... ,b31) of ChannelsSupported shall be reserved and set to 0, and
 *  the 27 LSBs (b0,b1, ... b26) shall indicate the status(1=available, 0=unavailable) for each of the 27 valid
 *  channels.
 */
#define gpMacCore_AttributeChannelsSupported 0x1 /* Supported channel mask from bit 0 to bit 27 */
/** @brief The 2 MSBs represent the tolerance on the transmit power: 00 = +- 1 dB 01 = +- 3 dB 10 = +- 6 dB The 6 LSBs
 *  represent a signed integer in twos-complement format, corresponding to the nominal transmit power of the device in
 *  decibels relative to 1 mW. The lowest value of phyTransmitPower shall be interpreted as less than or equal to +-32
 *  dBm. */
#define gpMacCore_AttributeTransmitPower 0x2 /* Transmit power in mdB */
...
// mac attr
/** @brief The maximum number of symbols to wait for an acknowledgment frame to arrive following a transmitted data
 *  frame. */
...
/** @brief The 16 bit short address assigned to the coordinator with which the device is associated. A value of 0xfffe
 *  indicates that the coordinator is only using its 64 bit extended address. A value of 0xffff indicates that this
 *  value is unknown. */
...
/** @brief The minimum value of the backoff exponent in the CSMA-CA algorithm. Note that if this value is set to 0,
 *  collision avoidance is disabled during the first iteration of the algorithm. Also note that for the slotted version
 *  of the CSMA-CA algorithm with battery life extension enabled, the minimum value of the backoff exponent will be the
 *  lesser of 2 and the value of macMinBE. */
#define gpMacCore_AttributeMinBE 0x4F
/** @brief The 16 bit identifier of the PAN (Personal Area Network) on which the device is operating. If this value is
 *  0xffff, the device is not associated to a network. */
...
/** @brief The 16 bit address that the device uses to communicate in the PAN. If the device is a PAN coordinator, this
 *  value shall be chosen before a PAN is started. Otherwise, the address is allocated by a coordinator during
 *  association. A value of 0xfffe indicates that the device has associated but has not been allocated a short address.
 * A value of 0xffff indicates that the device is not associated to a network. */
...
/** @brief Indication of whether a coordinator is currently allowing association. A value of TRUE indicates that
 *  association is permitted. */
#define gpMacCore_AttributeAssociationPermit 0x41
/** @brief The contents of the beacon payload. The length of the beacon payload is specified through the 'beacon payload
 *  length' attribute. */
...
/** @brief Specification of how often the coordinator transmits a beacon. The macBeaconOrder, BO, and the beacon
 *  interval, BI, are related as follows: for 0 <= BO <= 14, BI = aBaseSuperframeDuration * 2BO symbols. If BO = 15, the
...
/** @brief This indicates whether the MAC sublayer is in a promiscuous (receive all) mode. A value of TRUE indicates
 *  that the MAC sublayer accepts all frames received from the PHY, regardless of the source/destination address fields.
 */
#define gpMacCore_AttributePromiscuousMode 0x51
/** @brief This specifies the length of the active portion of the superframe, including the beacon frame. The superframe
 *  order, SO, and the superframe duration, SD, are related as follows: for 0 <= SO <= BO <= 14, SD =
 *  aBaseSuperframeDuration*2SO symbols. If SO = 15, the superframe will not be active after the beacon. */
#define gpMacCore_AttributeSuperFrameOrder 0x54
/** @brief The maximum time (in superframe periods) that a transaction is stored by a coordinator and it is indicated in
 *  the beacon that data is pending. */
 ...
 *  A value of TRUE indicates the device has associated through the PAN coordinator. Otherwise, the value is set to
 *  FALSE. */
 ...
 * @brief The gpMacCore_Attribute_t type defines the attribute type. The value could be an IEEE802.15.4 MAC attribute
 *  or a non-standard attributes that was added for flexibility. Only the PIB attributes that are used in this mac could
 *  be used, the non-used PIB attributes are not implemented to save memory.
 */
typedef UInt8 gpMacCore_Attribute_t;

/** @enum gpMacCore_ScanType_t */
//@{
/** @brief The energy detect scan, can be used to detect the amount of energy in the air. It is only used on FFD's (Full
 *  Functional Devices). */
#define gpMacCore_ScanTypeED 0
/** @brief The Active Scan will send out a beacon request. FFDs in the POS (Personal Operating Space) of the device will
 *  respond by sending out a beacon. */
#define gpMacCore_ScanTypeActive 1
/** @brief The passive scan is not implemented. */
#define gpMacCore_ScanTypePassive 2
/** @brief The orphan scan is used when a node lost his coordinator (i.e. when it has changed panId or channel). Using
 *  the orphan scan the device can locate his coordinator. */
...
/** @brief (Wifi Aware) Absolute Interference as a measure of the energy (0 = -95dBm, 254 = -45dBm, 255 = RDS Blocked
 *  Channel). */
#define gpMacCore_ScanTypeAI 0x82
/** @brief (Wifi Aware)  Absolute Interference Masked as a measure of the energy masked with Min Interference Level (0 =
 *  -95dBm, 254 = -45dBm, 255 = RDS Blocked Channel). */
#define gpMacCore_ScanTypeAIMasked 0x83
/** @brief Stop an ongoing scan. The confirm for the scan in progress will be triggered with the results up to that
 *  point. */
#define gpMacCore_ScanTypeStop 0xFF
/** @typedef gpMacCore_ScanType_t
 * @brief The gpMacCore_ScanType_t type defines the scanType as described in the IEEE802.15.4 MAC. Since this is a
 * non-beaconed MAC, the passive scan is not a valid value.
 */
 ...
 * @brief The gpMacCore_PanId_t type specifies a panId (Personal Area Network Identifier). The panId is a unique
 * identifier of the network. The length of the panId is 2 bytes.
 */
 ...
 * @brief The gpMacCore_PanDescriptor structure specifies the parameters of the PanDescriptor as is defined in the
 * IEEE802.15.4-2006 spec.
...
typedef struct gpMacCore_PanDescriptor {
    ...
    /** The quality of the link. This is an unsigned integer, ranging from 0 to 255. Lower values represent lower link
     *  quality. */
    UInt8 linkQuality;
...
#define gpMacCore_DataPendingModeForNonNeighbourDevices                                                                \
    1 /* for use with indirect transmit to unknown devices e.g. needed by Zigbee 3.0 */
 ...
 * removed. The callback functions (specified using the gpMacCore_RegisterCallbackFunctions function) are registered.
 ...
 */
GP_API void gpMacCore_DataRequest(gpMacCore_AddressMode_t srcAddrMode, gpMacCore_AddressInfo_t* pDstAddrInfo,
 ...
 * - Call gpMacCore_DataRequest() with option GP_MACCORE_TX_OPT_TIMEDTX to queue the packet;
 * - Call gpMacCore_ScheduleTimedTx() to schedule transmission
 ...
 *                          gpMacCore_DataRequest().
 * @param timingOptions     Specification of scheduled transmit time.
 * @param stackId           The identifier of the stack
 */
GP_API gpMacCore_Result_t gpMacCore_ScheduleTimedTx(gpPd_Handle_t pdHandle, gpMacCore_TxTimingOptions_t timingOptions,
                                                    gpMacCore_StackId_t stackId);
 ...
 *
 * @param scanType              This parameter defines which of the 3 implemented scans needs to be performed: ED,
 *                              active or orphan scan.
 * @param scanChannels          This parameter defines which channels need to be scanned. It is a bitmask where bit 0
 *                              must be '1' to select channel 0, bit 26 '1' to select channel 26, etc. Since the chip
 *                              works in the 2.4GHz band, only channels 11 till 26 can be selected. If other channels
 *                              are selected, they are ignored and only the selected channels between channel 11 and
 *                              channel 26 are scanned.
 ...
 *                              16us.
 * @param resultListSize        The length of the resultList (in bytes). If an ED scan is done, this should equal the
 *                              amount of channels. If resultListSize is higher than the amount of channels that needs
 *                              to be scanned there is no problem. If the value is lower, the scan is terminated when
 *                              the list is full. If an active scan is selected, resultListSize is the amount of
 *                              PANDescriptors that can be saved. If it is an orphan scan, resultListSize is 0.
 * @param pEdScanResultList     This is a pointer to an array where the result of the scan can be saved. If an ED scan
 *                              is done, the size is resultListSize. If an active scan is issued the size is
 *                              resultListSize*sizeof(gpMacCore_PanDescriptor_t).
 * @param stackId               The identifier of the stack
 */
GP_API void gpMacCore_ScanRequest(gpMacCore_ScanType_t scanType, UInt32 scanChannels, UInt8 scanDuration,
                                  UInt8 resultListSize, UInt8* pEdScanResultList, gpMacCore_StackId_t stackId);
 ...
 * @param stackId                   The identifier of the stack
 */
GP_API void gpMacCore_AssociateRequest(UInt8 logicalChannel, gpMacCore_AddressInfo_t* pCoordAddrInfo,
                                       UInt8 capabilityInformation, gpMacCore_StackId_t stackId);
 ...
 * @param stackId                   The identifier of the stack
 */
GP_API void gpMacCore_AssociateResponse(MACAddress_t* pDeviceAddress, UInt16 associateShortAddress,
                                        gpMacCore_Result_t status, gpMacCore_StackId_t stackId);
 ...
 * gpMacCore_AttributeShortAddress PIB attribute, the short address will be used as source address of the data request
 * command. If the gpMacCore_AttributeShortAddress PIB attribute is set to 0xffff or 0xfffe, the device will use its
 * extended address as source address in the data request command packet.
 *
 * @param coordAddrInfo     All the details regarding the coordinator
 * @param pSecOptions       structure with security options, if NULL no security is used.
 * @param stackId           The identifier of the stack
 */
GP_API void gpMacCore_PollRequest(gpMacCore_AddressInfo_t* coordAddrInfo, gpMacCore_Security_t* pSecOptions,
                                  gpMacCore_StackId_t stackId);
 ...
 * @param stackId           The identifier of the stack
 */
GP_API gpMacCore_Result_t gpMacCore_Start(gpMacCore_PanId_t panId, UInt8 logicalChannel, Bool panCoordinator,
                                          gpMacCore_StackId_t stackId);
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetCurrentChannel(channel, stackId)                                                                  \
    gpMacCore_SetCurrentChannel_STACKID(MACCORE_STACKID_MAP_2(channel, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetMultipleChannelTable(pChannel, stackId)                                                           \
    gpMacCore_SetMultipleChannelTable_STACKID(MACCORE_STACKID_MAP_2(pChannel, stackId))
 ...
 *
 * @param transmitPower     The transmission power to set in dBm, or one of gpMacCore_TxPowerMin, TxPowerMax,
 *                          TxPowerDefault.
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetTransmitPower(transmitPower, stackId)                                                             \
    gpMacCore_SetTransmitPower_STACKID(MACCORE_STACKID_MAP_2(transmitPower, stackId))
 ...
 * @param stackId                   The identifier of the stack
 */
#define gpMacCore_SetCoordExtendedAddress(pCoordExtendedAddress, stackId)                                              \
    gpMacCore_SetCoordExtendedAddress_STACKID(MACCORE_STACKID_MAP_2(pCoordExtendedAddress, stackId))
 ...
 * @param stackId                   The identifier of the stack
 */
#define gpMacCore_GetCoordExtendedAddress(pCoordExtendedAddress, stackId)                                              \
    gpMacCore_GetCoordExtendedAddress_STACKID(MACCORE_STACKID_MAP_2(pCoordExtendedAddress, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetCoordShortAddress(coordShortAddress, stackId)                                                     \
    gpMacCore_SetCoordShortAddress_STACKID(MACCORE_STACKID_MAP_2(coordShortAddress, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetPanCoordinator(panCoordinator, stackId)                                                           \
    gpMacCore_SetPanCoordinator_STACKID(MACCORE_STACKID_MAP_2(panCoordinator, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetMaxCsmaBackoffs(maxCsmaBackoffs, stackId)                                                         \
    gpMacCore_SetMaxCsmaBackoffs_STACKID(MACCORE_STACKID_MAP_2(maxCsmaBackoffs, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetRxOnWhenIdle(rxOnWhenIdle, stackId)                                                               \
    gpMacCore_SetRxOnWhenIdle_STACKID(MACCORE_STACKID_MAP_2(rxOnWhenIdle, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetShortAddress(shortAddress, stackId)                                                               \
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetAssociationPermit(associationPermit, stackId)                                                     \
    gpMacCore_SetAssociationPermit_STACKID(MACCORE_STACKID_MAP_2(associationPermit, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetBeaconPayload(pBeaconPayload, stackId)                                                            \
    gpMacCore_SetBeaconPayload_STACKID(MACCORE_STACKID_MAP_2(pBeaconPayload, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_GetBeaconPayload(pBeaconPayload, stackId)                                                            \
    gpMacCore_GetBeaconPayload_STACKID(MACCORE_STACKID_MAP_2(pBeaconPayload, stackId))
 ...
 * @param stackId               The identifier of the stack
 */
#define gpMacCore_SetBeaconPayloadLength(beaconPayloadLength, stackId)                                                 \
    gpMacCore_SetBeaconPayloadLength_STACKID(MACCORE_STACKID_MAP_2(beaconPayloadLength, stackId))
 ...
 */
#define gpMacCore_GetBeaconPayloadLength(stackId)                                                                      \
    gpMacCore_GetBeaconPayloadLength_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetPromiscuousMode(promiscuousMode, stackId)                                                         \
    gpMacCore_SetPromiscuousMode_STACKID(MACCORE_STACKID_MAP_2(promiscuousMode, stackId))
 ...
 * @param stackId                       The identifier of the stack
 */
#define gpMacCore_SetTransactionPersistenceTime(transactionPersistentTime, stackId)                                    \
    gpMacCore_SetTransactionPersistenceTime_STACKID(MACCORE_STACKID_MAP_2(transactionPersistentTime, stackId))
 ...
 */
#define gpMacCore_GetTransactionPersistenceTime(stackId)                                                               \
    gpMacCore_GetTransactionPersistenceTime_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetExtendedAddress(pExtendedAddress, stackId)                                                        \
    gpMacCore_SetExtendedAddress_STACKID(MACCORE_STACKID_MAP_2(pExtendedAddress, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_GetExtendedAddress(pExtendedAddress, stackId)                                                        \
    gpMacCore_GetExtendedAddress_STACKID(MACCORE_STACKID_MAP_2(pExtendedAddress, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetNumberOfRetries(numberOfRetries, stackId)                                                         \
    gpMacCore_SetNumberOfRetries_STACKID(MACCORE_STACKID_MAP_2(numberOfRetries, stackId))
...
/** @brief Sets the SecurityEnabled attribute
 *
 * @param securityEnabled   enable or disable stack security
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetSecurityEnabled(securityEnabled, stackId)                                                         \
    gpMacCore_SetSecurityEnabled_STACKID(MACCORE_STACKID_MAP_2(securityEnabled, stackId))
GP_API void gpMacCore_SetSecurityEnabled_STACKID(Bool securityEnabled MACCORE_STACKID_ARG_2);

/** @brief Gets the SecurityEnabled attribute
 *
 * @param stackId           The identifier of the stack
 *
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetBeaconStarted(BeaconStarted, stackId)                                                             \
    gpMacCore_SetBeaconStarted_STACKID(MACCORE_STACKID_MAP_2(BeaconStarted, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetTxAntenna(txAntenna, stackId)                                                                     \
    gpMacCore_SetTxAntenna_STACKID(MACCORE_STACKID_MAP_2(txAntenna, stackId))
...
/** @brief @brief This function manipulates the indication of beacons by the MAC
 *
 * @param enable            When enabled, beacons will be indicated by the MAC to the specific stack
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetIndicateBeaconNotifications(enable, stackId)                                                      \
    gpMacCore_SetIndicateBeaconNotifications_STACKID(MACCORE_STACKID_MAP_2(enable, stackId))
 ...
 */
#define gpMacCore_GetIndicateBeaconNotifications(stackId)                                                              \
    gpMacCore_GetIndicateBeaconNotifications_STACKID(MACCORE_STACKID_MAP_1(stackId))
GP_API Bool gpMacCore_GetIndicateBeaconNotifications_STACKID(MACCORE_STACKID_ARG_1);

/** @brief This function manipulates the poll indication by the MAC
 *
 * @param enable            When enable is true, poll indication will be forwarded by the MAC to the specific stack
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetForwardPollIndications(enable, stackId)                                                           \
    gpMacCore_SetForwardPollIndications_STACKID(MACCORE_STACKID_MAP_2(enable, stackId))
GP_API void gpMacCore_SetForwardPollIndications_STACKID(Bool enable MACCORE_STACKID_ARG_2);

/** @brief This function returns the parameter ForwardPollIndications
 *
 * @param stackId           The identifier of the stack.
 *
 * @return true if Forward Poll indications are enabled, else false
 */
#define gpMacCore_GetForwardPollIndications(stackId)                                                                   \
    gpMacCore_GetForwardPollIndications_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetLqiThresholdForTest(lqiThreshold, stackId)                                                        \
    gpMacCore_SetLqiThresholdForTest_STACKID(MACCORE_STACKID_MAP_2(lqiThreshold, stackId))
GP_API void gpMacCore_SetLqiThresholdForTest_STACKID(UInt8 lqiThreshold MACCORE_STACKID_ARG_2);

#define gpMacCore_GetLqiThresholdForTest(stackId)                                                                      \
    gpMacCore_GetLqiThresholdForTest_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetAddressModeOverrideForBeacons(addressMode, stackId)                                               \
    gpMacCore_SetAddressModeOverrideForBeacons_STACKID(MACCORE_STACKID_MAP_2(addressMode, stackId))
GP_API void gpMacCore_SetAddressModeOverrideForBeacons_STACKID(UInt8 addressMode MACCORE_STACKID_ARG_2);

#define gpMacCore_GetAddressModeOverrideForBeacons(stackId)                                                            \
    gpMacCore_GetAddressModeOverrideForBeacons_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetMacVersion(macVersion, stackId)                                                                   \
    gpMacCore_SetMacVersion_STACKID(MACCORE_STACKID_MAP_2(macVersion, stackId))
 ...
 * @param stackId           The identifier of the stack.
 *
 * @return gpMacCore_ResultSuccess on success
           gpMaCore_ResultInvalidParameter if @pAddrInfo or @stackId is invalid
           gpMacCore_ResultTransactionOverflow if no frame can be queued
 */
GP_API gpMacCore_Result_t gpMacCore_DataPending_QueueAdd(gpMacCore_AddressInfo_t* pAddrInfo,
                                                         gpMacCore_StackId_t stackId);
 ...
 *
 * @return gpMacCore_ResultSuccess on success
           gpMacCore_ResultInvalidParameter if @pAddrInfo or @stackId is invalid */
GP_API gpMacCore_Result_t gpMacCore_DataPending_QueueRemove(gpMacCore_AddressInfo_t* pAddrInfo,
                                                            gpMacCore_StackId_t stackId);
 ...
 *
 * @return gpMacCore_ResultSuccess on success
           gpMacCore_ResultInvalidParameter on failure
 */
 ...
 * @param stackId           The identifier of the stack.
 */
GP_API gpMacCore_Result_t gpMacCore_SetDataPendingMode(gpMacCore_DataPendingMode_t dataPendingMode,
                                                       gpMacCore_StackId_t stackId);
...
/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_SetKeyDescriptor(gpMacCore_KeyDescriptor_t* pKeyDescriptor,
                                                     gpMacCore_Index_t index);

/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_GetKeyDescriptor(gpMacCore_KeyDescriptor_t* pKeyDescriptor,
                                                     gpMacCore_Index_t index);
...
/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_SetDeviceDescriptor(gpMacCore_DeviceDescriptor_t* pDeviceDescriptor,
                                                        gpMacCore_Index_t index);

/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_GetDeviceDescriptor(gpMacCore_DeviceDescriptor_t* pDeviceDescriptor,
                                                        gpMacCore_Index_t index);
...
/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_SetSecurityLevelDescriptor(
    gpMacCore_SecurityLevelDescriptor_t* pSecurityLevelDescriptor, gpMacCore_Index_t index);

/** @brief Access method for the corresponding PIB attribute
 */
GP_API gpMacCore_Result_t gpMacCore_GetSecurityLevelDescriptor(
    gpMacCore_SecurityLevelDescriptor_t* pSecurityLevelDescriptor, gpMacCore_Index_t index);
 ...
 * Any attempt to transmit/listen on a blocked channel will fail with gpMacCore_ResultInvalidParameter. Transmit power
 * levels above maximum will be clipped to the maximum level.
 *
 ...
 * @param stackId                   The identifier of the stack.
 */
GP_API void gpMacCore_SetRegionalDomainSettings(UInt32 blockedChannelMask, gpMacCore_TxPower_t* pMaxTransmitPowerTable,
                                                gpMacCore_StackId_t stackId);
 ...
 * @param stackId                   The identifier of the stack.
 */
GP_API void gpMacCore_GetRegionalDomainSettings(
    UInt32* pBlockedChannelMask, gpMacCore_TxPower_t* pMaxTransmitPowerTable, gpMacCore_StackId_t stackId);
 ...
 * @param stackId                       The identifier of the stack.
 */
#define gpMacCore_EnableEnhancedFramePending(enableEnhancedFramePending, stackId)                                      \
    gpMacCore_EnableEnhancedFramePending_STACKID(MACCORE_STACKID_MAP_2(enableEnhancedFramePending, stackId))
 ...
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetStackInRawMode(rawModeEnabled, stackId)                                                           \
    gpMacCore_SetStackInRawMode_STACKID(MACCORE_STACKID_MAP_2(rawModeEnabled, stackId))
 ...
 * @param stackId               The identifier of the stack.
 */
#define gpMacCore_SetRawModeEncryptionKeys(encryptionKeyIdMode, encryptionKeyId, pCurrKey, stackId)                    \
    gpMacCore_SetRawModeEncryptionKeys_STACKID(                                                                        \
        MACCORE_STACKID_MAP_4(encryptionKeyIdMode, encryptionKeyId, pCurrKey, stackId))
GP_API void gpMacCore_SetRawModeEncryptionKeys_STACKID(gpMacCore_KeyIdMode_t encryptionKeyIdMode,
                                                       gpMacCore_KeyIndex_t encryptionKeyId,
                                                       UInt8* pCurrKey MACCORE_STACKID_ARG_2);
 ...
 * @param stackId           The identifier of the stack.
 */
#define gpMacCore_SetRawModeNonceFields(frameCounter, pExtendedAddress, seclevel, stackId)                             \
    gpMacCore_SetRawModeNonceFields_STACKID(MACCORE_STACKID_MAP_4(frameCounter, pExtendedAddress, seclevel, stackId))
GP_API void gpMacCore_SetRawModeNonceFields_STACKID(UInt32 frameCounter, MACAddress_t* pExtendedAddress,
                                                    UInt8 seclevel MACCORE_STACKID_ARG_2);
 ...
 */
#define gpMacCore_ConfigureEnhAckProbing(linkMetrics, pExtendedAddress, shortAddress, stackId)                         \
    gpMacCore_ConfigureEnhAckProbing_STACKID(                                                                          \
        MACCORE_STACKID_MAP_4(linkMetrics, pExtendedAddress, shortAddress, stackId))
GP_API gpMacCore_Result_t gpMacCore_ConfigureEnhAckProbing_STACKID(UInt8 linkMetrics, MACAddress_t* pExtendedAddress,
                                                                   UInt16 shortAddress MACCORE_STACKID_ARG_2);
#endif // GP_MACCORE_DIVERSITY_RAW_FRAMES
 ...
 * gpMacCore_SetNumberOfRetries. As such this function only changes the default MAC 802.15.4-2015 behavior to also
 * retransmit on a CCA fail.
 *
 * @param enable            Enables or disables the Retransmits.
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetRetransmitOnCcaFail(enable, stackId)                                                              \
    gpMacCore_SetRetransmitOnCcaFail_STACKID(MACCORE_STACKID_MAP_2(enable, stackId))
 ...
 */
#define gpMacCore_GetRetransmitOnCcaFail(stackId)                                                                      \
    gpMacCore_GetRetransmitOnCcaFail_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * is set through gpMacCore_SetNumberOfRetries. As such this function only changes the default MAC 802.15.4-2015
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetRetransmitRandomBackoff(enable, stackId)                                                          \
    gpMacCore_SetRetransmitRandomBackoff_STACKID(MACCORE_STACKID_MAP_2(enable, stackId))
 ...
 */
#define gpMacCore_GetRetransmitRandomBackoff(stackId)                                                                  \
    gpMacCore_GetRetransmitRandomBackoff_STACKID(MACCORE_STACKID_MAP_1(stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetMinBeRetransmit(minBERetransmit, stackId)                                                         \
    gpMacCore_SetMinBeRetransmit_STACKID(MACCORE_STACKID_MAP_2(minBERetransmit, stackId))
 ...
 * @param stackId           The identifier of the stack
 */
#define gpMacCore_SetMaxBeRetransmit(maxBERetransmit, stackId)                                                         \
    gpMacCore_SetMaxBeRetransmit_STACKID(MACCORE_STACKID_MAP_2(maxBERetransmit, stackId))
...
/** @brief Retrieve last message information of a specific short address
 *
 * @param  shortAddr        Short address to fetch information for. 0xFFFF can be used to fetch information for the last
 *                          packet (not specific to a short address). Disclaimer: When this API is called from a data
 *                          indication callback and 0xFFFF is used in the short address parameter, there is no guarantee
 *                          that the data refers to the actual packet for which the data indication callback is
 *                          generated. Therefore requesting data for the last packet is recommended to be used in a
 *                          controlled environment only.
 * @param  pDiagnostics     Pointer to return information in.
 *
 * @return result True if entry was found. False if diagnostics for the given short address was never received.
 */
GP_API Bool gpMacCore_DiagRxCntrGetLastMessage(UInt16 shortAddr, gpMacCore_DiagRxCntr_t* pDiagnostics);

/** @brief Retrieve average MAC retries for transmissions to a specific short address
 *
 * @param shortAddr         Short address to fetch information for. Use GP_MACCORE_STATISTICS_ALL_DEVICES for
 *                          information on all devices.
 * @param  pDiagnostics     Pointer to return information in.
 *
 * @return result True if entry was found. False if diagnostics for the given short address was never received.
 */
GP_API Bool gpMacCore_DiagTxCntrGetAvgRetryMessage(UInt16 shortAddr, gpMacCore_DiagTxCntr_t* pDiagnostics);
#endif /* defined(GP_MACCORE_DIVERSITY_DIAGCNTRS) */
...
/** @brief Calls the DataConfirm callback function. It is used to indicate the result of a data transmission to the next
 * higher layer.
 *
 ...
 * @param pdHandle          The msduHandle that was given with the dataRequest is returned with the dataConfirm. This
 *                          way, the next higher layer knows which packet generated the confirm.
 */
void gpMacCore_cbDataConfirm(gpMacCore_Result_t status, gpPd_Handle_t pdHandle);

/** @brief Calls the DataIndication callback function. It is used to inform the next higher layer a data packet is
 * received.
 *
 * This function indicates (to the next higher layer) that a data packet is received.
 *
 * It has more or less the same parameters than the type gpMacCore_cbCPSDataIndication_t, except that is has one
 * additional parameter handle and that the parameter pMsdu contains the address in the PBM memory where the data
 * payload is located.
 *
 ...
 *
 * This function informs the next higher layer that an associate request command was received, indicating a device
 * wishes to associate.
 *
 * @param pDeviceAddress        A pointer to the address of the device that requested the association. This will always
 *                              be an extended (64 bit) address.
 * @param capabilityInformation The operational capabilities of the device requesting association as specified in the
 *                              IEEE802.15.4-2006 specification.
 * @param stackId               The stack Id this indication is ment for
 ...
 *
 * This function is used to inform a device of its attempt to join a network. It indicates the results (joined or not)
 * of a previous issued AssociateRequest.
 *
 * @param assocShortAddress The short address the device may use if the association was successful.
 * @param status            The status of the association attempt. If this indicates a success, the assocShortAddress
 *                          field contains the short address the device may use.
 ...
 * @param status            The communications status.
 * @param stackId           The stack Id this indication is ment for
 * @param txTime            transmission/reception time of this message
 */
void gpMacCore_cbSecurityFailureCommStatusIndication(gpMacCore_AddressInfo_t* pSrcAddrInfo,
                                                     gpMacCore_AddressInfo_t* pDstAddrInfo, gpMacCore_Result_t status,
                                                     gpMacCore_StackId_t stackId, gpPd_TimeStamp_t txTime);

/** @ingroup MLME_SAP
 * @brief Calls the BeaconNotifyIndication function. Informs the next higher layer a beacon packet containing a payload
 * has been received.
 *
 * This function indicates a beacon with payload is received.
 *
 * @param bsn                   The sequence number of the beacon.
 * @param pPanDescriptor        A pointer to the PANDescriptor of the beacon, the values of this pointer should not be
 *                              changed.
 * @param stackId               The stack Id this indication is ment for
...
/** @brief Calls the ScanConfirm function, indicating the result of a previously issued ScanRequest to the next higher
 * layer.
 *
...
/** @brief Calls the OrphanIndication function. It is issued on a coordinator on receipt of an orphan notification
 * command.
 *
 * This function indicates a device is orphaned. It is issued on a coordinator on receipt of an orphan notification
 * command.
 *
...
/** @brief Calls the PollConfirm function. The PollConfirm inform the next higher layer about a previously issued
 * PollRequest.
 *
 * This function returns the status of a previously issued PollRequest.
 *
 * @param status            The status of the pollRequest. If success, the received data will be returned using the
 *                          DataIndication function.
 * @param coordAddrInfo     All the details of the coordinator address
 */
void gpMacCore_cbPollConfirm(gpMacCore_Result_t status, gpMacCore_AddressInfo_t* coordAddrInfo,
                             gpPd_TimeStamp_t txTime);

/** @brief Calls the PollIndication function. It is only invoked when the gpMacCore_SetForwardPollIndications() is
 * enabled.
 *
 * This callback is only used when the upper layer implements its own data pending mechanism.
 ...
 * @param rxTime            reception time of this message
 */
void gpMacCore_cbPollIndication(gpMacCore_AddressInfo_t* addressInfo, gpMacCore_StackId_t stackId,
 ...
 * This notification serves as bookkeeping information for the higher layer.
 *
 * @param addressInfo       The address info used by the originator
 * @param stackId           The stack Id this indication is ment for
 * @param rxTime            Time at which the packet was received (in us)
 * @param pdHandle          Handle of the queued packet that will be transmitted in response to this response. If no
 *                          data was pending, 0xFF is returned here.
 * @param fromNeighbour     True if the poll came from a device specified in the Neighbour table list - see the
 *                          gpMacCore_AddNeighbour() API calls.
 */
void gpMacCore_cbPollNotify(gpMacCore_AddressInfo_t* addressInfo, gpMacCore_StackId_t stackId, gpPd_TimeStamp_t rxTime,
                            gpPd_Handle_t pdHandle, Bool fromNeighbour);

/** @brief Calls the SecurityFrameCounterIndication function. It is invoked when the security framecounter window
 * exceeds the previous value with GP_MACCORE_FRAMECOUNTER_WINDOW.
 *
 ...
 */
GP_API void gpMacCore_EnableRxWindows(UInt32 dutyCycleOnTime, UInt32 dutyCyclePeriod, UInt16 recurrenceAmount,
 ...
 */
GP_API Int8 gpMacCore_GetRadioSensitivity(void);
 ...
 */
GP_API UInt8 gpMacCore_GetCslAccuracy(void);
 ...
 */
GP_API UInt8 gpMacCore_GetCslUncertainty(void);
