...
static const gpPad_Attributes_t ROM MacCore_DefaultPadAttributes FLASH_PROGMEM = {
    ...
    0,                                     // Bool ccaFailureAsNoAck;
};


#ifdef GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
STATIC gpMacCore_RegionalDomainSettings_t MacCore_RegionalDomainSettings[GP_DIVERSITY_NR_OF_STACKS];
...
#ifdef GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
/* Initialize Regional Domain Settings for all stacks. */
void MacCore_InitRegionalDomainSettings(void)
{
    UIntLoop stackId;

    for (stackId = 0; stackId < GP_DIVERSITY_NR_OF_STACKS; stackId++)
    {
        // Initially allow all channels and umlimited transmit power.
        const UIntLoop numchannel = sizeof(MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable) /
                                    sizeof(MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable[0]);
        UIntLoop ch;
        MacCore_RegionalDomainSettings[stackId].blockedChannelMask = 0;
        for (ch = 0; ch < numchannel; ch++)
        {
            MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable[ch] = GPHAL_MAX_TRANSMIT_POWER;
        }
    }
}

/* Return true if the specified channel is blocked according to Regional Domain Settings. */
Bool MacCore_IsRdsChannelBlocked(gpHal_Channel_t channel, gpMacCore_StackId_t stackId)
{
    GP_ASSERT_DEV_EXT(stackId < GP_DIVERSITY_NR_OF_STACKS);
    GP_ASSERT_DEV_EXT(GP_MACCORE_CHECK_CHANNEL_VALID(channel));
    return (!GP_MACCORE_CHECK_CHANNEL_VALID(channel) ||
            BIT_TST(MacCore_RegionalDomainSettings[stackId].blockedChannelMask, channel));
}

/* Return the maximum allowed power level according to Regional Domain Settings. */
gpMacCore_TxPower_t MacCore_GetRdsMaxTransmitPower(gpHal_Channel_t channel, gpMacCore_StackId_t stackId)
{
    GP_ASSERT_DEV_EXT(stackId < GP_DIVERSITY_NR_OF_STACKS);
    GP_ASSERT_DEV_EXT(GP_MACCORE_CHECK_CHANNEL_VALID(channel));
    if (GP_MACCORE_CHECK_CHANNEL_VALID(channel))
    {
        return MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable[channel - 11];
    }
    else
    {
...
#ifdef GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
void gpMacCore_SetRegionalDomainSettings(UInt32 blockedChannelMask, gpMacCore_TxPower_t* pMaxTransmitPowerTable, gpMacCore_StackId_t stackId)
{
    GP_ASSERT_DEV_EXT(stackId < GP_DIVERSITY_NR_OF_STACKS);
    if(pMaxTransmitPowerTable == NULL)
    {
        GP_ASSERT_DEV_EXT(false);
        return;
    }
    MacCore_RegionalDomainSettings[stackId].blockedChannelMask = blockedChannelMask;
    MEMCPY(MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable, pMaxTransmitPowerTable, sizeof(MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable));
}

void gpMacCore_GetRegionalDomainSettings(UInt32 *blockedChannelMask, gpMacCore_TxPower_t* pMaxTransmitPowerTable, gpMacCore_StackId_t stackId)
{
    GP_ASSERT_DEV_EXT(stackId < GP_DIVERSITY_NR_OF_STACKS);
    if(pMaxTransmitPowerTable == NULL)
    {
    ...
    *blockedChannelMask = MacCore_RegionalDomainSettings[stackId].blockedChannelMask;
    MEMCPY(pMaxTransmitPowerTable, MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable, sizeof(MacCore_RegionalDomainSettings[stackId].maxTransmitPowerTable));
}
#endif //GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
...
Int8 gpMacCore_GetTransmitPower_STACKID(MACCORE_STACKID_ARG_1)
{
    ...
    {
#ifdef GP_MACCORE_DIVERSITY_REGIONALDOMAINSETTINGS
        MacCore_GetRdsMaxTransmitPower(gpRxArbiter_GetStackChannel(MACCORE_STACKID_REF), MACCORE_STACKID_REF);
#else
...
void gpMacCore_SetNumberOfRetries_STACKID(UInt8 numberOfRetries MACCORE_STACKID_ARG_2)
{
    ...
    if(MACCORE_STACKID_REF == MacCoreGetRawStack())
    {
        gpPad_SetCcaFailureAsNoAck(gpMacCore_PIB[MACCORE_STACKID_REF].padHandle, true);
    }
    else
#endif // defined(GP_MACCORE_DIVERSITY_RAW_FRAMES)
    {
        gpPad_SetCcaFailureAsNoAck(gpMacCore_PIB[MACCORE_STACKID_REF].padHandle, false);
    }
}
...
void gpMacCore_StackRemoved(gpMacCore_StackId_t stackId)
{
    ...
    /* Note: we need another call in rx arbiter
    to disable/remove a stack */
