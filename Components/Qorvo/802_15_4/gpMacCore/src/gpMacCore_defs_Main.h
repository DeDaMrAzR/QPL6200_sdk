...
/* Initialize Regional Domain Settings. */
STATIC_FUNC void MacCore_InitRegionalDomainSettings(void);

/* Consult Regional Domain Settings. */
STATIC_FUNC Bool MacCore_IsRdsChannelBlocked(gpHal_Channel_t channel, gpMacCore_StackId_t stackId);
STATIC_FUNC gpMacCore_TxPower_t MacCore_GetRdsMaxTransmitPower(gpHal_Channel_t channel, gpMacCore_StackId_t stackId);
...
/** @brief Update the last received diagnostics counter information
 *
 * @param pdLoh     Pointer to gpPd_Loh_t struct containing the received packet information
 * @param shortAddr Short address to update information for
*/
void MacCore_DiagRxCntrUpdateLastMessage(gpPd_Loh_t *pdLoh, UInt16 shortAddr);

/** @brief Upate average MAC retries for transmissions to a specific short address
 *
 * Called after each transmission.
 *
 * @param retryCntr Retries for a single packet.
 * @param shortAddr Short address to update information for.
*/
...
STATIC_FUNC Bool MacCore_IsDataPending(Bool dataQueued, Bool isNeighbour, gpMacCore_StackId_t stackId);
