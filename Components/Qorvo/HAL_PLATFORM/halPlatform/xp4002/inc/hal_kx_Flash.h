...
#define FLASH_CHECK_ADDRESS(address) (GP_MM_CMA_NVM_LINEAR_START <= (address) && (address) < GP_MM_CMA_NVM_LINEAR_END)
#define FLASH_CHECK_ADDRESS_RANGE(address, len)                                                                        \
    (GP_MM_CMA_NVM_LINEAR_START <= (address) && ((address) + (len)) <= GP_MM_CMA_NVM_LINEAR_END)

/* Convert absolute address (in ARM memory space) to flash physical address */
#define FLASH_WB_ADDR_TO_PHYS(address) (((address)-GP_MM_CMA_NVM_LINEAR_START) / FLASH_WRITE_UNIT)
 ...
 *                 data needs to be a pointer into the RAM area. Writing directly from one part of flash to another is
 *not supported.
 **/
