...
#if defined(GP_DIVERSITY_GPHAL_XP4002)
    return GP_WB_READ_ES_SYSTEM_TIME_US();
#else
...
#define HAL_TIMER_GET_CURRENT_TIME_32US(time)   \
    do {                                        \
        (time) = hal_TimerGetCurrentTime_32us(); \
    } while (false)
// Macro returns a true 32-bit/32us timer - overflow will happen at 32-bit
#define HAL_TIMER_GET_CURRENT_TIME(time)  HAL_TIMER_GET_CURRENT_TIME_32US(time)
#else
