 ...
 *****************************************************************************/
#if defined(GP_DIVERSITY_GPHAL_XP4002)
#define GPIO_PIN(pin) pin
#else
#define GPIO_PIN(pin) gpios[pin]
#endif

#if defined(GP_DIVERSITY_GPHAL_XP4002)
typedef UInt8 hal_gpiodsc_t;
#else
typedef struct{
    UInt8 bitBandOffset;
...
/* enum for edge detect: */
#define HAL_GPIO_INTERRUPT_ON_RISING_EDGE   0x0
#define HAL_GPIO_INTERRUPT_ON_FALLING_EDGE  0x1
/* enum for level detect: */
#define HAL_GPIO_INTERRUPT_ON_LOW_LEVEL     0x0
#define HAL_GPIO_INTERRUPT_ON_HIGH_LEVEL    0x1

typedef void (*hal_cbGpioExti_t)(uint8_t);
void hal_gpioConfigureInterrupt(UInt8 gpioPerip, UInt8 gpio, UInt8 type, UInt8 level, hal_cbGpioExti_t cbExti);
void hal_gpioUnconfigureInterrupt(UInt8 gpioPerip, UInt8 gpio);
void hal_gpioSetIntLevelAndType(UInt8 gpio, UInt8 type, UInt8 level);
void hal_gpioDisableInt(UInt8 gpioPerip, UInt8 gpio);
void hal_gpioEnableInt(UInt8 gpioPerip, UInt8 gpio);
#else
...
#if defined(GP_HAL_ASP_USE_2048KHZ_CLOCK)
//2,048Mhz -> 32 kHz     (/4 left for SW to get to 8kHz)
#endif
#define HAL_ASP_DECIMATION_2048KHZ_32KHZ      64

#if defined(GP_HAL_ASP_USE_2048KHZ_CLOCK)
//2,048Mhz -> 64 kHz     (/4 left for SW to get to 16kHz)
#endif
...
#if defined(GP_HAL_ASP_USE_2048KHZ_CLOCK)
/* Final sample frequency for voice equals 2,048Mhz /32 /4 = 16kHz */
#endif
...
#if defined(GP_HAL_ASP_USE_2048KHZ_CLOCK)
/* Final sample frequency for voice equals 2,048Mhz /64 /4 = 16kHz */
#endif
...
#if defined(GP_HAL_ASP_USE_2048KHZ_CLOCK)
/* Special usecase : 2,048Mhz /22 /2 = 46.545kHz */
#define HAL_ASP_FIR_DECI_ULTRASOUND       (2)
#define HAL_CIC_DECIMATION_ULTRASOUND     (22)
#elif defined(GP_HAL_ASP_USE_4096KHZ_CLOCK)
/* Special usecase : 4,096Mhz /44 /2 = 46.545kHz */
#define HAL_ASP_FIR_DECI_ULTRASOUND       (2)
#define HAL_CIC_DECIMATION_ULTRASOUND     (44)
#else
/* Default frequency for ultrasound equals 3,072Mhz /32 /2 = 48kHz */
#define HAL_ASP_FIR_DECI_ULTRASOUND       (2)
#define HAL_CIC_DECIMATION_ULTRASOUND     (32)
#endif /* GP_HAL_ASP_USE_2048KHZ_CLOCK */
