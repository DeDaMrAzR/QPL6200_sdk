...
#ifdef GP_UTILS_DIVERSITY_CRC_TABLE
static const UInt16 ROM gpUtils_crc16Table[] = /* CRC lookup table polynomial 0xA001 */
{
...
#define _crc16_update(crc, a) (PGM_READ_WORD(&gpUtils_crc16Table[((crc) ^ (a)) & 0xff]) ^ ((crc) >> 8))
...
#endif //GP_UTILS_DIVERSITY_CRC_TABLE
...
{
#ifndef GP_UTILS_DIVERSITY_CRC_TABLE
    while (length--)
    {
        *pCrcValue = _crc16_kermit_update(*pCrcValue, *pData++);
    }
#else
    /* CRC16-CCITT/Kermit not implemented for table version */
