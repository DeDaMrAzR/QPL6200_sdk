...
static UInt32 Random_m_w =
    521288629UL; /* https://groups.google.com/forum/?fromgroups#!topic/sci.crypt/yoaCpGWKEk0[1-25]
                  */
static UInt32 Random_m_z =
    362436069UL; /* https://groups.google.com/forum/?fromgroups#!topic/sci.crypt/yoaCpGWKEk0[1-25]
                  */
/*****************************************************************************
...
void Random_MashUpSeed(void)
{
    ...
    for(seedWordCount = 0; seedWordCount < GP_RANDOM_RANDOMNESS_STRENGTH_WORDS; seedWordCount++)
    {
        ...
        Random_Seed[(RandomSeedWordOffset + seedWordCount) % GP_RANDOM_RANDOMNESS_STRENGTH_WORDS] ^=
            Random_PseudoRead32();
    }
}
...
void Random_GetLastSequence(UInt8* pBuffer, UInt8 nmbrRandomBytes)
{
    ...
    {
        pBuffer[randomByteIndex] = ((UInt8*)Random_Seed)[((RandomSeedWordOffset << 2) + randomByteIndex) %
                                                         (GP_RANDOM_RANDOMNESS_STRENGTH_WORDS << 2)];
    }
}
...
#ifndef GP_COMP_CHIPEMU
    uint32_t success = esec_get_random_bytes((uint8_t*)Random_Seed, (GP_RANDOM_RANDOMNESS_STRENGTH_WORDS << 2));
    if(success != ESEC_OKAY)
    {
        GP_ASSERT_SYSTEM(false);
    }
#else
    MEMSET(Random_Seed, 0, (GP_RANDOM_RANDOMNESS_STRENGTH_WORDS << 2));
#endif
...
void gpRandom_GetNewSequence(UInt8 nmbrRandomBytes, UInt8* pBuffer)
{
    ...
    {
        UInt8 byteDiff = nmbrRandomBytes - randomBytesGenerated;
        UInt8 bytesToGenerate = (GP_RANDOM_RANDOMNESS_STRENGTH_WORDS << 2) < byteDiff
                                    ? (GP_RANDOM_RANDOMNESS_STRENGTH_WORDS << 2)
                                    : byteDiff;
        Random_MashUpSeed();
...
{
#ifndef GP_COMP_CHIPEMU
    uint32_t success = esec_get_random_bytes((uint8_t*)pBuffer, nmbrRandomBytes);
    if(success != ESEC_OKAY)
    {
        GP_ASSERT_SYSTEM(false);
    }
#else
    gpRandom_CTR_DRBG_Generate(nmbrRandomBytes, pBuffer);
#endif
}
...
void gpRandom_Init(void)
{
    ...
    gpRandom_CTR_DRBG_Init();

    /* Do extra reseeding */
    gpRandom_CTR_DRBG_Reseed();
