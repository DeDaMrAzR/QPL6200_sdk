 ...
 *  - head     = [ Component ID | type=head | index ] [ key Hash | payload index 0 | ... | payload index 9 ] [
 * payload[0..229] ]
 ...
 */
typedef uint8_t qvCHIP_KvsType_t;
...
/** @brief Start of NVM area - linker script defined. */
 ...
 *  @param[out] hashBuffer  Byte buffer to return hash created from the key string
 */
static qvStatus_t qvCHIP_KvsCreateHash(const char* key, uint8_t* hashBuffer)
{
    ...
    GP_LOG_PRINTF("'%s' -> %02x%02x%02x%02x...", 0, key, hashBuffer[0], hashBuffer[1], hashBuffer[2], hashBuffer[3]);
...
static qvStatus_t qvCHIPKvs_BuildLookup(void)
{
    ...
    if(qvCHIP_KvsLookupHandle == gpNvm_LookupTable_Handle_Invalid)
    {
        ...
        uint8_t lookupMask[1] = {GP_COMPONENT_ID};
        uint8_t nrOfMatches;
        gpNvm_Result_t nvmResult;

        nvmResult =
            gpNvm_BuildLookupProtected(&qvCHIP_KvsLookupHandle, KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore,
                                       sizeof(lookupMask), lookupMask, GP_NVM_NBR_OF_UNIQUE_TOKENS, &nrOfMatches);
 ...
 */
static uint8_t qvCHIPKvs_FindFreeIndex(qvCHIP_KvsType_t type)
{
    gpNvm_Result_t nvmResult;

    qvCHIP_KvsToken_t tokenMask = {.componentId = GP_COMPONENT_ID, .type = type, .index = 0};
    ...
    do
    {
        ...
        nvmResult =
            gpNvm_ReadUniqueProtected(qvCHIP_KvsLookupHandle, KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, NULL,
                                      sizeof(tokenMask), (uint8_t*)&tokenMask, MAX_KVS_VALUE_LEN, &dataLen, NULL);
        if(nvmResult == gpNvm_Result_DataAvailable)
        {
 ...
 *  @param pKeyPayload     Pointer to struct representation of key NVM element byte payload.
 */
static void qvCHIP_KvsDumpKeyElement(uint8_t keyElementIndex, qvCHIP_KvsKeyPayload_t* pKeyPayload)
{
    GP_LOG_SYSTEM_PRINTF(
        "%u: %02x%02x%02x%02x... | %u, %u, ...", 0, keyElementIndex, pKeyPayload->keyHash[0], pKeyPayload->keyHash[1],
        pKeyPayload->keyHash[2], pKeyPayload->keyHash[3], pKeyPayload->payloadIndices[0],
 ...
 *                           This list is used as ordered to store parts sequentially.
 *
 * @return status Possible values from qvStatus_t:
 *                - QV_STATUS_NVM_ERROR - any failure in gpNvm calls
 *                - QV_STATUS_NO_ERROR
 */
static qvStatus_t qvCHIP_KvsAddHeadElement(uint8_t keyElementIndex, const uint8_t* keyHash, size_t valueSize,
                                           const void* value, const uint8_t* payloadIndices)
{
    gpNvm_Result_t nvmResult;

    // Fill in index for key element
    qvCHIP_KvsToken_t keyTokenMask = {
        .componentId = GP_COMPONENT_ID, .type = qvCHIP_KvsTypeHead, .index = keyElementIndex};
    ...
    if(value != NULL)
    {
        MEMCPY(&keyPayload.payload, (uint8_t*)value, MIN(valueSize, MAX_PAYLOAD_IN_HEAD_ELEM));
    ...
    // Write to NVM
    nvmResult = gpNvm_WriteProtected(KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, sizeof(keyTokenMask),
                                     (uint8_t*)&keyTokenMask, writeLen, (uint8_t*)&keyPayload);
    if(nvmResult != gpNvm_Result_DataAvailable)
    {
 ...
 *                - QV_STATUS_NO_ERROR
 */
static qvStatus_t qvCHIP_KvsFindKeyHeadInfo(const uint8_t* keyHash, uint8_t* pKeyIndex, uint8_t* pPayloadIndices,
                                            size_t maxReadSize, size_t* valueSize, uint8_t* value)
{
    ...
    *pKeyIndex = INVALID_KEY_INDEX;
    ...
    GP_LOG_PRINTF("Finding -> %02x%02x%02x%02x... l:%u", 0, keyHash[0], keyHash[1], keyHash[2], keyHash[3],
                  qvCHIP_KvsLookupHandle);
    ...
    do
    {
        ...
        uint8_t lookupLen;              // Only used for check
        qvCHIP_KvsKeyPayload_t dataKey; // Required for full info fetch
        uint8_t dataLen;                // Used to determine length of indices

        nvmResult =
            gpNvm_ReadNextProtected(qvCHIP_KvsLookupHandle, KVS_POOL_ID, NULL, GP_NVM_MAX_TOKENLENGTH, &lookupLen,
                                    (uint8_t*)&lookupMask, sizeof(dataKey), &dataLen, (uint8_t*)&dataKey);
        GP_LOG_PRINTF("r:%u mask:%u %u|%u|%u data:%u %02x%02x%02x%02x...", 0, nvmResult, lookupLen,
                      lookupMask.componentId, lookupMask.type, lookupMask.index, dataLen, dataKey.keyHash[0],
                      dataKey.keyHash[1], dataKey.keyHash[2], dataKey.keyHash[3]);
...
static qvStatus_t qvCHIP_KvsAddDataElements(size_t valueSize, const void* value, uint8_t* pPayloadIndices)
{
    ...
    while((iterator < MAX_KVS_PAYLOAD_EXTENSIONS) && (valueSize > 0))
    {
        qvCHIP_KvsToken_t dataTokenMask = {
            .componentId = GP_COMPONENT_ID, .type = qvCHIP_KvsTypeData, .index = pPayloadIndices[iterator]};
        ...
        nvmResult = gpNvm_WriteProtected(KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, sizeof(dataTokenMask),
                                         (uint8_t*)&dataTokenMask, MIN(valueSize, MAX_KVS_VALUE_LEN),
                                         &((uint8_t*)value)[iterator * MAX_KVS_VALUE_LEN]);
        if(nvmResult != gpNvm_Result_DataAvailable)
        {
            // Write failed - NVM full ?
 ...
 *                - QV_STATUS_NO_ERROR
 */
static qvStatus_t qvCHIP_KvsGetDataElements(const uint8_t* pPayloadIndices, uint8_t offsetBytes, size_t maxBytesToRead,
                                            size_t* readBytesSize, uint8_t* pReadBuffer)
{
    qvStatus_t qvStatus = QV_STATUS_NO_ERROR;
    gpNvm_Result_t nvmResult;
    uint8_t iterator = 0;
    ...
    while((iterator < MAX_KVS_PAYLOAD_EXTENSIONS) && (maxBytesToRead > 0))
    {
        ...
        nvmResult =
            gpNvm_ReadUniqueProtected(qvCHIP_KvsLookupHandle, KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, NULL,
                                      sizeof(tokenMask), (uint8_t*)&tokenMask, MIN(MAX_KVS_VALUE_LEN, maxBytesToRead),
                                      &dataLen, (uint8_t*)&pReadBuffer[*readBytesSize]);
        if(nvmResult == gpNvm_Result_Truncated)
        {
            qvStatus = QV_STATUS_BUFFER_TOO_SMALL;
            // We wait to go to _cleanup, because readBytesSize still needs to be updated
        }
        ...
        // At this point, if offsetBytes is greater than 0, it is surely less than MAX_KVS_VALUE_LEN, otherwise the
        // entire element would have been skipped; therefore, we need to skip the remaining offsetBytes Also, if
        // offsetBytes > 0 here, it means at this point, *readBytesSize is 0
        if(offsetBytes > 0)
        {
        ...
        if(qvStatus == QV_STATUS_BUFFER_TOO_SMALL)
        {
            goto _cleanup;
        }
    }
...
static qvStatus_t qvCHIP_KvsDeleteInternal(const uint8_t* keyHash)
{
    ...
    qvStatus = qvCHIP_KvsFindKeyHeadInfo(keyHash, &keyIndex, payloadIndices, MAX_KVS_VALUE_LEN, NULL, NULL);
    if(QV_STATUS_NO_ERROR != qvStatus)
    {
        goto _cleanup;
    }
    ...
    while(iterator < MAX_KVS_PAYLOAD_EXTENSIONS)
    {
        ...
        nvmResult =
            gpNvm_RemoveProtected(KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, sizeof(tokenMask), (uint8_t*)&tokenMask);
        if(nvmResult != gpNvm_Result_DataAvailable)
        {
            qvStatus = QV_STATUS_INVALID_DATA;
            goto _cleanup;
...
#ifdef QVCHIP_SECURE_STORAGE
static qvStatus_t qorvoEncrypt(uint8_t* cipher, const uint8_t* plain, size_t length)
{
    GP_LOG_PRINTF("Encryption increases data size from %u to %u to %u", 0, length,
                  length + QV_CRYPTO_META_DATA_SIZE - ESEC_STOR_AUTH_SIZE, length + QV_CRYPTO_META_DATA_SIZE);

    // Encrypting data
    struct esec_skey sKey = {}; // empty esec_skey struct, will be filled out by esec_import_skey

    // Random nonce
    uint8_t keyAuth[ESEC_STOR_AUTH_SIZE] = {0xAA, 0xBB, 0xAA, 0xBB, 0xAA, 0xBB, 0xAA, 0xBB};
    qvCHIP_RandomGetDRBG(ESEC_STOR_AUTH_SIZE, keyAuth);

    // encrypt
    uint32_t success = esec_import_skey(plain, length, (struct esec_key_storage)ESEC_STORAGE_WRAPPED(cipher, keyAuth),
                                        ESEC_KEY_EXPORTABLE, &sKey);

    if(success != ESEC_OKAY)
    {
        GP_LOG_SYSTEM_PRINTF("Encryption failed: %lu", 0, success);
        return QV_STATUS_INVALID_DATA;
    }

    length += QV_CRYPTO_META_DATA_SIZE - ESEC_STOR_AUTH_SIZE;

    // copy the auth data to the end of the cipher buffer
    MEMCPY(&cipher[length], keyAuth, ESEC_STOR_AUTH_SIZE);
    MEMSET(keyAuth, 0x00, ESEC_STOR_AUTH_SIZE); // clearing the memory
    return QV_STATUS_NO_ERROR;
}

static qvStatus_t qorvoDecrypt(uint8_t* plain, uint8_t* cipher, size_t length)
{
    // reconstructing the esec_skey structure
    uint8_t* keyAuth = &cipher[length - ESEC_STOR_AUTH_SIZE];
    struct esec_skey sKey = {.storage = ESEC_STORAGE_WRAPPED(cipher, keyAuth),
                             .keyspec = length - QV_CRYPTO_META_DATA_SIZE};

    uint32_t success = esec_export_skey(sKey, plain);
    if(success != ESEC_OKAY)
    {
        // should we delete the entry, since it is not recoverable?
        GP_LOG_SYSTEM_PRINTF("Decryption failed: %lu", 0, success);
        return QV_STATUS_INVALID_DATA;
    }
    return QV_STATUS_NO_ERROR;
}
...
void qvCHIP_NvmSetVariableSettings(void)
{
    ...
    uint8_t availableMaxTokenLength;
    ...
    GP_LOG_PRINTF("Settings old/new: Nvm Start:%lx/%lx #Uniq Token:%u/%u Token Length:%u/%u", 0,
                  (unsigned long)currentNvmStart, (unsigned long)&gpNvm_Start, currentNumberOfUniqueTokens,
                  GP_NVM_NBR_OF_UNIQUE_TOKENS, availableMaxTokenLength, GP_NVM_MAX_TOKENLENGTH);
    ...
    uint16_t currentNrOfSectors;
    uint8_t currentNrOfPools;
    uint8_t currentSectorsPerPool[4]; // Max from NVM implementation

    uint16_t newNrOfSectors = (GP_NVM_POOL_1_NBR_OF_PHY_SECTORS
    ...
    uint8_t newNrOfPools = GP_NVM_NBR_OF_POOLS;
    uint8_t newSectorsPerPool[GP_NVM_NBR_OF_POOLS] = {
...
qvStatus_t qvCHIP_KvsConsistencyCheck(void)
{
        ...
        if(nvmResult == gpNvm_Result_DataAvailable)
        {
            if(tokenMask.type == qvCHIP_KvsTypeHead)
            {
                ...
                if(kvsKey->payloadIndices[0] != INVALID_KEY_INDEX)
                {
                    ...
                    MEMCPY((uint8_t*)pHeadKeyPayloadIndices[nrOfHeadKeys], (uint8_t*)kvsKey->payloadIndices,
                           MAX_KVS_PAYLOAD_EXTENSIONS);
                }
                ...
                {
                    GP_LOG_SYSTEM_PRINTF(
                        "!!! KVS inconsistency: key shorter than maximum, should fit in the head element", 0);
                    GP_ASSERT_DEV_INT(false);
                }
                nrOfHeadKeys += 1;
            }
        ...
        for(uint8_t j = 0; j < MAX_KVS_PAYLOAD_EXTENSIONS; j++)
        {
            if(pHeadKeyPayloadIndices[i][j] != INVALID_KEY_INDEX)
            {
                ...
                for(uint8_t k = 0; k < nrOfDataKeys; k++)
                {
                    ...
                    {
                        found = true;
                        // Invalidate found index, to make sure it is not double referenced
    ...
    for(uint8_t k = 0; k < nrOfDataKeys; k++)
    {
        if(dataKeyIndexes[k] != INVALID_KEY_INDEX)
        {
            ...
            nvmResult =
                gpNvm_RemoveProtected(KVS_POOL_ID, gpNvm_UpdateFrequencyIgnore, tokenMaskLen, (uint8_t*)&tokenMask);
            if(nvmResult != gpNvm_Result_DataAvailable)
            {
                qvStatus = QV_STATUS_NVM_ERROR;
                goto _cleanup;
...
qvStatus_t qvCHIP_KvsPut(const char* key, const void* value, size_t valueSize, bool isSensitive)
{
    ...
    uint8_t* buffer = NULL;
#ifdef QVCHIP_SECURE_STORAGE
    size_t origValueSize = valueSize;
#endif // QVCHIP_SECURE_STORAGE
...
#ifdef QVCHIP_SECURE_STORAGE
    // Sensitive Data have meta data stored as well
    valueSize += (isSensitive) ? (QV_CRYPTO_META_DATA_SIZE) : 0;
#endif // def QVCHIP_SECURE_STORAGE
    ...
    if(isSensitive)
    {
        buffer = (void*)calloc(valueSize, sizeof(uint8_t));
        qvStatus = qorvoEncrypt((uint8_t*)buffer, (uint8_t*)value, origValueSize);
        if(QV_STATUS_NO_ERROR != qvStatus)
        {
            goto _cleanup;
        }
    ...
    qvStatus = qvCHIP_KvsFindKeyHeadInfo(keyHash, &keyIndex, payloadIndices, 0, NULL, NULL);
    ...
    if(valueSize > MAX_PAYLOAD_IN_HEAD_ELEM)
    {
        ...
        qvStatus =
            qvCHIP_KvsAddDataElements(valueSize - MAX_PAYLOAD_IN_HEAD_ELEM,
                                      (const void*)&((uint8_t*)buffer)[MAX_PAYLOAD_IN_HEAD_ELEM], payloadIndices);
        if(QV_STATUS_NO_ERROR != qvStatus)
        {
            goto _cleanup;
        }
    ...
    qvStatus =
        qvCHIP_KvsAddHeadElement(keyIndex, keyHash, MIN(valueSize, MAX_PAYLOAD_IN_HEAD_ELEM), buffer, payloadIndices);
    if(QV_STATUS_NO_ERROR != qvStatus)
    {
        goto _cleanup;
    }
                         ...
                         bool isSensitive)
{
    ...
    *readBytesSize = 0;
    ...
    if(isSensitive)
    {
        // We need a buffer for decryption
        buffer = calloc(valueSize, sizeof(uint8_t));
    }
    else
#endif // QVCHIP_SECURE_STORAGE
    {
        buffer = value; // we can directly retrieve the values in @value
    }

    // Retrieve list of payload elements and payload data from head info structure
    qvStatus =
        qvCHIP_KvsFindKeyHeadInfo(keyHash, &keyIndex, payloadIndices, valueSize, readBytesSize, (uint8_t*)buffer);
    if(qvStatus == QV_STATUS_BUFFER_TOO_SMALL && isSensitive)
    {
        // If the buffer is too small, we cannot retrieve all the data, and therefore
    ...
    // If offset is between 0 and data read length, we need to shift the data to the left and update offset and
    // readBytesSize
    if(offsetBytes > 0 && offsetBytes < *readBytesSize)
    {
        MEMCPY_INPLACE(buffer, (void*)&((uint8_t*)buffer)[offsetBytes], *readBytesSize - offsetBytes);
    ...
    if(payloadIndices[0] != INVALID_KEY_INDEX && valueSize > 0)
    {
        ...
        qvStatus = qvCHIP_KvsGetDataElements(payloadIndices, offsetBytes, valueSize, readBytesSize, (uint8_t*)buffer);
        if(qvStatus == QV_STATUS_BUFFER_TOO_SMALL && isSensitive)
        {
            // If the buffer is too small, we cannot retrieve all the data, and therefore
            // we won't be able to decrypt it.
    ...
    if(isSensitive)
    {
        qorvoDecrypt((uint8_t*)value, (uint8_t*)buffer, *readBytesSize);
        *readBytesSize -= QV_CRYPTO_META_DATA_SIZE;
    }
#endif // QVCHIP_SECURE_STORAGE
