...
qvStatus_t qvCHIP_Nvm_Backup(uint16_t key, const uint8_t* pRamLocation, uint16_t length)
{
    ...
    return qvCHIP_KvsPut(charKey, pRamLocation, length, qvCHIP_insensitive);
}
...
qvStatus_t qvCHIP_Nvm_Restore(uint16_t key, uint8_t* pRamLocation, uint16_t* length)
{
    ...
    status = qvCHIP_KvsGet(charKey, pRamLocation, *length, &readBytes, 0, qvCHIP_insensitive);
...
bool qvCHIP_Nvm_ValueExists(uint16_t key, uint16_t* length)
{
    ...
    return (qvCHIP_KvsGet(charKey, &dummy, 0, &dummy, 0, qvCHIP_insensitive) == QV_STATUS_BUFFER_TOO_SMALL);
}
